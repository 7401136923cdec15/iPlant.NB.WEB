require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/base/paging'], function ($zace, $com, $page) {
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            //物料同步
            $("body").delegate("#lmvt-synchronism", "click", function () {

                if(!confirm("此操作会同步ERP系统中物料、单位信息，将会耗费一定的时间，是否继续？")){
                    return;
                }
                $com.app.loading('数据同步中，请稍等。。。');
                model.com.getMeterialSyncUser({ }, function (resModule) {
                     alert("同步成功！！！");
                     $com.app.loaded();
                });
             });
            ...
            $("body").delegate("#lmvt-bopImport-inputErrorLog", "click", function () {
                ...
                var vdata = { 'header': '导入日志', 'href': './factory_model/FPCErrorSetting.html?id=' + 4, 'id': 'FPCErrorSettingBop', 'src': './static/images/menu/newfactoryModel/techniquePart.png' };
                window.parent.iframeHeaderSet(vdata);
                window.callFunctionTrigger("FPCErrorSettingBop", { ID: 4 });
            ...
            $("body").delegate("#input-file", "change", function () {
                ...
                model.com.postImportExcelMaterial(form, [function (res) {
            ...
            //             //alert(list.length);
            //             var CGBool = false;
            //             var CYBool = false;//常用计量单位
            //             var KCBool = false;//重量单位
            //             var SCBool = false;
            //             var XSBool = false;
            //             //var SBool = false;
            //             var SuBool = false;
            ...
            //                     }
            //                     if (item.ZLUnitID == ZCYUnitIDList[i].Name) {
            //                         item.ZLUnitID = ZCYUnitIDList[i].ID;
            //                         KCBool = true;
            //                     }
            ...
            //                 item.CYUnitID = item.CYUnitID;
            //                 item.MaterialType = item.MaterialType;
            //                 item.MaterialGroup = item.MaterialGroup;
            //                 item.Groes = item.Groes;
            //                 item.Normt = item.Normt;
            //                 item.Remark = item.Remark;
            //                 item.NetWeight = item.NetWeight;
            //                 item.GrossWeight = item.GrossWeight;
            //                 item.ZLUnitID = item.ZLUnitID;
            //                 item.MaterialName = item.Name + '\\' + item.Groes + '\\' + item.Remark;
            ...
            $("body").delegate("#zace-search-materialRecord", "click", function () {
                ...
                if (value == undefined || value == "" || value.trim().length < 1)
                    ...
                    $com.table.filterByLikeStringData($("#femi-materialRecord-tbody"), DataSearchAll, value, undefined, undefined, undefined, function (res) {
                        $("#femi-materialRecord-tbody").html($com.util.template(res, HTML.TableNode));
                    });
                }
        ...
        com: {
            ...
            //同步
            getMeterialSyncUser: function (data, fn, context) {
                var d = {
                    $URI: "/Material/SyncUser",
                    $TYPE: "get",
                };
            ...
            setMaterial: function () {
                setTimeout(function () {
                    model.com.getMeterialProgress({ ImportType: 4, ImportTime: mImportTime }, [function (res) {
                        if (!res)
                            return;
            ...
            refresh: function () {
                ...
                model.com.getmaterialRecord({ material_no: "", material_name: "", type_id: 0, status: 0, PageSize: 100000 }, function (res) {
                    if (!res)
                        return;
                    if (res && res.list) {
                        ...
                        $page.init($("#femi-materialRecord-tbody").closest("table"), _list, "", function (res) {
                            $("#femi-materialRecord-tbody").html($com.util.template(res, HTML.TableNode));
                        });
            ...
            //导入
            postImportExcelMaterial: function (data, fn, context) {
                var d = {
                    $URI: "/IPTStandard/ImportMaterial",
                    $TYPE: "post",
                    $SERVER: '/MESQMS',
                };

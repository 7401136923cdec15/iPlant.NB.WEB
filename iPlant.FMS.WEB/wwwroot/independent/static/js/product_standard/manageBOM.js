require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/base/bootstrap-treeview.min', '../static/utils/js/base/materialSelect'], function ($zace, $com, $tree, $materialSelect) {
    ...
    HTML = {
        TableBOMMode: [
            ...
            '<LI>更多<UL>',
            '<LI data-value="{{ID}}" class="lmvt-info">详情</LI>',
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            $("body").delegate(".lmvt-info", "click", function () {
                ...
                $("#femi-bomItem-tbody").html("<tr><td> <input type=\"checkbox\" class=\"femi-tb-checkbox\" style=\"margin: 1px 0px 1px\" value=\"{{functionID}}\" /> </td><td colspan=\"13\">请先点击需要查看的物料。。。</td></tr>");

                $(".zace-BomItem").show();
                $(".zace-Bom").hide();
                var $this = $(this);
                var WName = $this.find('td[data-title=BOMName]').attr('data-value');
                mBomId = Number($this.attr('data-value'));
        ...
        com: {
                ...
                if (list.length > 0)
                    $.each(list, function (i, item) {
                        if (item.children.length > 0) {
                            var Counts = item.children.length;
                            item.text = item.MaterialNo + "(" + item.MaterialName + ")";
                            item.nodes = item.children;
                            item.tags = [Counts];
                            model.com.StandardTreeBuild(item.children);
                        } else {
                            item.text = item.MaterialNo + "(" + item.MaterialName + ")";
                            // item.nodes = [];
                            // item.tags = [0];
            ...
            refreshBomTree: function () {
                ...
                model.com.getBomItemList({ bom_id: mBomId, IsList: false }, function (resBom) {
                    ...
                    if (resBom && resBom.list) {
                        ...
                        var retrunList = model.com.BuildeTree(_list);

                        var list = model.com.StandardTreeBuild(retrunList);
            ...
            refreshBomTreePro: function (list) {
                // bom_no:{String} , bom_name:{String} ,workshop_id:{int} ,type_id:{int} ,status:{int}
                // 
            ...
            renderTree: function (list) {
                ...
                $("#standardItem").treeview({
                    ...
                    onNodeSelected: function (event, data) {
                        ...
                        mPlaceID = data.PlaceID;
                        mPartPoint = data.PartPointID;

                        if (data.nodes && data.nodes.length > 0) {
                            model.com.refreshBomTreePro(data.nodes);
                        } else {
                            model.com.refreshBomTreePro([data]);
                        }

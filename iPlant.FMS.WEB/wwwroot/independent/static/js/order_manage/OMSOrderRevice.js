require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/paging', '../static/utils/js/base/base'], function ($alfie, $page, $com) {
    ...
    var mZCommitEndTime = $com.util.format("yyyy-MM-dd", new Date().getTime() + 24 * 3600 * 1000);
    var mCode = "";
    var mData;
    var wUser = [];
    var mFMCStation = [];
    ...
    var PartArrayNo = [{
        ...
        PartNo: "18HC0026",
        Customer: "18号线",
        WBSNo: "201808196398345",
        WorkName: "镟轮",
        CreateTime: "2018-08-19 14:48:38",
        PlanTime: "2018-08-19",
        PlanName: "王胜利",
        Title: "1.【工单号】有效，点击“下一步”完成导入。",
    }];
    ...
    HTML = {
        ...
        TableNode_no: [
            ...
            '<td style="min-width: 50px;color:{{Color}}" data-title="WBSNo" data-value="{{WBSNo}}">{{WBSNo}}</td>',
            '</tr>',
        ].join(""),
        OrderDetailTemp: [
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            $("body").delegate("#alfie-add-level", "click", function () {
                ...
                $("body").append($com.modal.show(DEFAULT_VALUE_D, KEYWORD_order, "新增工单", function (rst) {
                    //调用插入函数然后用load刷新数据源
                    ...
                    var _data = {
                        ...
                        PartNo: rst.PartNo,
                        WBSNo: rst.PartNo,
                        PlanReceiveDate: $com.util.format('yyyy-MM-dd', rst.PlanReceiveDate),
                        PlanFinishDate: $com.util.format('yyyy-MM-dd', rst.PlanFinishDate),
            ...
            $("body").delegate("#commit_next", "click", function () {
                if (FinishDate.length > 0) {
                    model.com.OMSOrderUpdateList({
                        ...
                        $(".ImportFinish").text(FinishDate.length);
                        $(window).resize();
                    });
                } else {
                    alert("未查到可导入工单！");
                ...
                model.com.postImportExcel(form, function (res) {
                    ...
                    if (list) {
                        ...
                        model.com.postCheckList({
                            ...
                            FinishDate = res.list;
                            var unFinishDate = res.info;
                            for (var i = 0; i < FinishDate.length; i++) {
                                FinishDate[i].Color = "green";
                                FinishDate[i].PlanReceiveDate = $com.util.format("yyyy-MM-dd", FinishDate[i].PlanReceiveDate);
                                FinishDate[i].WID = i + 1;
                            }
                            for (var i = 0; i < unFinishDate.length; i++) {
                                unFinishDate[i].Color = "red";
                                unFinishDate[i].WID = i + 1;
                                unFinishDate[i].PlanReceiveDate = $com.util.format("yyyy-MM-dd", unFinishDate[i].PlanReceiveDate);
                            }
                            $(".OrderAll").hide();
                            $(".zacePencilDone").hide();
                            $(".zacePencilDone-Import").show();
                            ...
                            $("#femi-no-tbody-item").html($com.util.template(unFinishDate, HTML.TableNode_no));
                            $("#femi-yes-tbody-item").html($com.util.template(FinishDate, HTML.TableNode_no));
                            $(".ImportNum").text(FinishDate.length);
                            $(".unImportNum").text(unFinishDate.length);
                            $(window).resize();
                            $com.app.loaded();
                        });
                    }
        ...
        run: function () {
            ...
            model.com.StatusCount({
                CommandID: -1, FactoryID: -1, WorkShopID: -1, LineID: -1, StatusList: [-1], StationID: -1, OrderNo: "",
                ProductID: -1, CustomerID: -1, TeamID: -1, PartNo: "", PreStartTime: "2000-1-1",
                PreEndTime: "2000-1-1", RelStartTime: "2000-1-1", RelEndTime: "2000-1-1",
            }, function (res) {
                ...
                $(".AllOrderNum").text(OrderCount.AllOrderNum); // 全部工单
                $(".TobeexecutedNum").text(OrderCount.TobeexecutedNum); //待执行
                $(".ExecutingNum").text(OrderCount.ExecutingNum);  //执行中
                $(".CalledoffNum").text(OrderCount.CalledoffNum);  //被叫停
                $(".FinishNum").text(OrderCount.FinishNum); //已完成
                $(".CloseNum").text(OrderCount.CloseNum); //已关闭
            });

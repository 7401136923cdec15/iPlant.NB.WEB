    ...
    function ($lin, $com, $treeview) {
        ...
        HTML = {
            TreeItemNode: [
                ...
                '<div class="lmvt-tree"  style="vertical-align:top;Color:{{ColorText}}" data-value="{{ID}}" data-type="{{Type}}" data-name="{{ProductNo}}"  data-core="{{ColorText}}">{{ProductNo}}</div> ',
                '<ul>{{Items}}',
                '</ul>',
                '</li>',
            ].join(""),
            PartStandardList: [
                ...
                '<td style="" data-title="Remark" data-value="{{Remark}}" >{{Remark}}</td>',

                '<td style="" data-title="SamplingStandard" data-value="{{SamplingStandard}}" >{{SamplingStandard}}</td>',
        ...
        model = $com.Model.create({
            ...
            events: function () {
                ...
                $("body").delegate(".partPointStandard .lmvt-do-info", "click", function () {
                    ...
                    model.com.getIPTSampleItemAll({
                        ...
                        $('.zaceText').text(SelectObj.Remark + '-发货检验项点');
                ...
                $("body").delegate("#standardList li div:not(.hitarea)", "click", function () {
                    ...
                    var $this = $(this);
                    var ProType = Number($this.attr("data-type"));
                    ...
                    if (ProType == 1) {
                        ...
                        wGroupID = Number($this.attr("data-value"));
                        var Obj = ProductList.filter((item) => { return item.ID == wGroupID })[0];
                        wProductID = Obj.ProductID;
                        wProductNo = Obj.ProductNo;
                    }

                    wName = $this.attr("data-name");
                    TreeObj = $this;
            ...
            run: function () {
                ...
                IPTMode = 21;
                wGroupID = 0;
                model.com.getFPCProductAllType({}, function (res) {
                    ...
                    $.each(res.list, function (i, item) {
                        //代表产品
                        item.Type = 0;
                        if (!cat[item.ProductTypeID]) {
                            cat[item.ProductTypeID] = item.ProductTypeID;
                            GruopAllTree.push({
                                // ID: item.ProductTypeID,
                                // ProductNo: item.ProductType,
                                // Type: 1,
                                // SonList: [],
                        ...
                        if (GruopAllTree.length > 0) {
                            $.each(GruopAllTree, function (j, jtem) {
                                if (jtem.ID == item.ProductTypeID) {
                                    jtem.nodes.push({
                                        ID: item.ID,
                                        text: item.ProductNo,
                                        Type: 2,
            ...
            com: {
                ...
                //查询所有物料组
                getFPCProductAllType: function (data, fn, context) {
                    var d = {
                        $URI: "/FPCProduct/All",
                        $TYPE: "get"
                    };

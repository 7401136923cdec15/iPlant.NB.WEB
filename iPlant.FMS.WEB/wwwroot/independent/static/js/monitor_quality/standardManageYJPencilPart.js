    ...
    function ($lin, $com, $treeview) {
        ...
        HTML = {
            PartStandardList: [
                ...
                '<td data-title="CustomName" data-value="{{CustomName}}" >{{CustomName}}</td>',
        ...
        model = $com.Model.create({
            ...
            events: function () {
                ...
                $("body").delegate("#lmvt-set-preCheckProblem", "click", function () {
                    $("#input-FileProblem").val("");
                    $("#input-FileProblem").click();
                });

                $("body").delegate("#input-FileProblem", "change", function () {
                    //alert()
                    var $this = $(this);
                    ...
                    // PartID: lSelectID
                    model.com.postImportProblem(form, function (res) {
...
lmvtType = 0;         model.com.refreshStandardInfo(StandardID);
                ...
                //监听select事件
                $("body").delegate("#lmvt-select-customer", "change", function () {
                    var $this = $(this);
                    pSelectCustomerID = $this.find("option:selected").text();
                    pSelectIDCustomerPro = $this.find("option:selected").attr("value");
                    model.com.RanderSelect(wSelectID);
                    // if (zaceType == 1) {
                    //     model.com.renderTree(zaceProduct);
                    // } else {
                        ...
                        if (list) {
                            ...
                            model.com.getAPSProductAll({}, function (res) {
                                ...
                                if (list) {
                                    ...
                                    //局段
                                    model.com.getCustomer({ active: 2 }, function (res) {
                                        if (!res)
                                            return;
                                        var list = res.list,
                                            rst = [];
                                        if (list) {
                                            zaceCustomer = [];
                                            zaceCustomerTest = $com.util.Clone(res.list);

                                            for (var index = 0; index < zaceCustomerTest.length; index++) {

                                                if (zaceCustomerTest[index].Active == 1) {
                                                    zaceCustomer.push(zaceCustomerTest[index]);
                                                };

                                            }
                                            var CustomerList = [];
                                            $.each(zaceCustomer, function (j, jtem) {
                                                CustomerList.push("<option value = '", jtem.ID, "'" + " >" + jtem.CustomerName + "</option>");
                                            });
                                            $('#lmvt-select-customer').html(CustomerList.join(""));
                                            pSelectCustomerID = $('#lmvt-select-customer option:selected').text();

                                            //alert(pSelectID);
                                            pSelectIDCustomerPro = Number($('#lmvt-select-customer option:selected').attr("value"));
                                            model.com.RanderSelect(wSelectID);
            ...
            com: {
                ...
                refreshPartAll: function () {
                    ...
                    model.com.getStandardPartList({ LineID: wSelectID, PartID: lSelectID, ProductID: pSelectIDPro, CustomID: pSelectIDCustomerPro }, function (res) {
                        if (!res) {
                            return;
                        }
                ...
                postImportProblem: function (data, fn, context) {
                    var d = {
                        $URI: "/IPTPreCheckProblem/Import",
                        $TYPE: "post",
                        $SERVER: '/MESQMS',

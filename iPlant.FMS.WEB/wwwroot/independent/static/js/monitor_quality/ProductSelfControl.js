    ...
    function ($lin, $com, $treeview) {
        ...
        HTML = {
            ...
            StandardList: [
                ...
                '<td data-title="DefaultManufactor" data-value="{{DefaultManufactor}}">{{DefaultManufactor}}</td>',
                '<td data-title="DefaultModal" data-value="{{DefaultModal}}">{{DefaultModal}}</td>',
                '<td data-title="CheckPoint" data-value="{{CheckPoint}}">{{CheckPoint}}</td>',
                '<td data-title="DefaultNumber" data-value="{{DefaultNumber}}">{{DefaultNumber}}</td>',
                '<td data-title="OtherValue" data-value="{{OtherValue}}">{{OtherValue}}</td>',
                '<td data-title="Process" data-value="{{Process}}">{{Process}}</td>',
        ...
        (function () {
            ...
            KETWROD_LIST_Item = [
                ...
                "ManufactorOption|客户选项|InputArray",
                "OrderID|顺序",
                "PartsCoding|部件编码",
                ...
                "Details|参数描述",
                "IsManufactorFill|是否联动|ArrayOne",
                "IsModalFill|联动行数",
                "IsNumberFill|是否可编辑|ArrayOne",
                "IsPeriodChange|是否主键|ArrayOne",
                "IsQuality|是否可重复|ArrayOne",
                "DefaultManufactor|默认值类型|ArrayOne",
                "DefaultModal|默认值格式",
                "CheckPoint|关联字段",
                "DefaultNumber|关联字段系数",
                "OtherValue|值长度",
                "Process|过滤",
        ...
        model = $com.Model.create({
            ...
            events: function () {
                ...
                $("body").delegate(".modal .femi-modal-body .femi-modal-item select#modal_select_PartID", "change", function (e) {
                    ...
                    // $("#modal_select_PartPointID").next().show();
                });
                //监听工步
                $("body").delegate(".modal .femi-modal-body .femi-modal-item select#modal_select_PartPointID", "change", function (e) {
                    ...
                    // $("#modal_select_StationID").next().show();
                });
                ...
                $("body").delegate("#lmvt-standard-add", "click", function () {
                    ...
                    var wStationID = SelectData[0].StationID;
                ...
                $("body").delegate("#lmvt-standarditem-add", "click", function () {
                    ...
                    $("body").append($com.modal.show(Defaul_Value_Item, KETWROD_Item, "新增版本项", function (rst) {
                        ...
                        var _data = {
                            ...
                            IsQuality: rst.IsQuality,
                            CheckPoint: rst.CheckPoint,

                            DefaultNumber: rst.DefaultNumber,

                            OtherValue: rst.OtherValue,
                            Process: rst.Process,
                ...
                $("body").delegate("#lmvt-edit", "click", function () {
                    ...
                    var dataSource = partAllList.filter(item => item.ID == StandardID)[0];

                    model.com.postCopyStandard({
                        data: dataSource,
                    }, function (res) {
                        model.com.refreshEditAll(function () {
                            $(".zace-partAll-body tr").each(function (i, item) {
                                var $item = $(item),
                                    EditStandardID = Number($item.find("td[data-title=ID]").attr("data-value"));
                                if (EditStandardID == res.info.ID) {
                                    $item.find(".lmvt-do-info").trigger("click");
                                    return false;
                                }
                            });
                        })
                ...
                $("body").delegate("#lmvt-select-line", "change", function () {
                    ...
                    $.each(partSource, function (i, item) {
                        if (item.UnitID == lSelectID) {
                            $.each(item.UnitList, function (j, jtem) {
                                wLine.push("<option value = ", jtem.UnitID, ">", jtem.Name, "</option>");
                            });
                            $("#lmvt-select-partpoint").html(wLine.join(""));
                        }
                    });
                    $("#lmvt-select-partpoint").selectpicker({});
                    $("#lmvt-select-partpoint").selectpicker('render');
                ...
                $("body").delegate(".partPointStandard .lmvt-do-info", "click", function () {
                    ...
                    $(".zace-type-body").html("<tr><td> <input type=\"checkbox\" class=\"femi-tb-checkbox\" style=\"margin: 1px 0px 1px\" value=\"{{functionID}}\" /> </td><td colspan=\"30\">请先点击需要查看的项点。。。</td></tr>");
                    ...
                    $('.zaceText').text(SelectObj.Remark + '-生产自检项点');

                    //判断相关按钮是否显示
                    if (SelectObj.IsUsed == 0) {
                        $("#lmvt-standarditem-delete").show();
                        $("#lmvt-standarditem-group").show();
                        $("#lmvt-standarditem-add").show();
                        $("#lmvt-set-currentNew").show();
                        $("#lmvt-edit").hide();
                    } else {
                        $("#lmvt-standarditem-delete").hide();
                        $("#lmvt-standarditem-group").hide();
                        $("#lmvt-standarditem-add").hide();
                        $("#lmvt-set-currentNew").hide();
                        $("#lmvt-edit").show();
                    }
                ...
                $("body").delegate(".lmvt-version-copy", "click", function () {
                    ...
                    // $("#modal_select_PartID").next().hide();
                    // $("#modal_select_PartPointID").next().hide();
                    // $("#modal_select_StationID").next().hide();

                    $("body").append($com.modal.show(Defaul_Value, KETWROD_Standard, "复制", function (rst) {
                        if (!rst || $.isEmptyObject(rst)) {
                            return false;
                        }
                        ...
                        dataSource.LineID = Number(rst.LineID);
                        dataSource.PartID = Number(rst.PartID);
                        dataSource.PartPointID = Number(rst.PartPointID);
                        dataSource.StationID = Number(rst.StationID);
                        dataSource.ProductID = Number(rst.ProductID);
                        dataSource.ProductNo = Formattrt_Standard["ProductID"](dataSource.ProductID);
                    ...
                    }, TypeSource_Standard, function (rst) {
                        var msg = "";
                        if (rst.LineID == -1) {
                            msg = "产线未选择";
                        }
                ...
                $("body").delegate(".partAllStandard .lmvt-do-info", "click", function () {
                    ...
                    var Defaul_Value = {
                        ...
                        IsManufactorFill: SelectData[0].IsManufactorFill,
                        // ManufactorOption: SelectData[0].ManufactorOption,
                        ...
                        IsModalFill: SelectData[0].IsModalFill,
                        IsNumberFill: SelectData[0].IsNumberFill,
                        IsPeriodChange: SelectData[0].IsPeriodChange,
                        IsQuality: SelectData[0].IsQuality,
                        DefaultManufactor: SelectData[0].DefaultManufactor,
                        DefaultModal: SelectData[0].DefaultModal,
                        CheckPoint: SelectData[0].CheckPoint,
                        DefaultNumber: SelectData[0].DefaultNumber,
                        OtherValue: SelectData[0].OtherValue,
                        Process: SelectData[0].Process,
                    ...
                    $("body").append($com.modal.show(Defaul_Value, KETWROD_Item, "修改版本项", function (rst) {
                        ...
                        $.each(dataSource.ItemList, function (i, item) {
                            if (item.ID == SelectData[0].ID) {
                                ...
                                item.UnitID = _data.UnitID;
                                item.ValueSource = _data.ValueSource;
                                item.IsManufactorFill = _data.IsManufactorFill;
                                // item.ManufactorOption = _data.ManufactorOption;
                                ...
                                item.IsModalFill = _data.IsModalFill;
                                item.IsNumberFill = _data.IsNumberFill;
                                item.IsPeriodChange = _data.IsPeriodChange;
                                item.IsQuality = _data.IsQuality;
                                item.DefaultManufactor = _data.DefaultManufactor;
                                item.DefaultModal = _data.DefaultModal;
                                item.CheckPoint = _data.CheckPoint;
                                item.DefaultNumber = _data.DefaultNumber;
                                item.OtherValue = _data.OtherValue;
                                item.Process = _data.Process;
                            }
                        });
                    ...
                    model.com.getAPSProductTypeAll({}, function (res) {
                        ...
                        if (list) {
                            ...
                            $.each(Data_WorkSourc, function (i, item) {
                                ...
                                TypeSource_Standard.LineID.push({
                                    name: item.Name,
                                    value: item.ID
                                })
                            });
                            ...
                            model.com.getAPSProductAll({}, function (res) {
                                ...
                                if (list) {
                                    ...
                                    for (var index = 0; index < zaceProductTest.length; index++) {
                                        if (zaceProductTest[index].Active == 1) {
                                    ...
                                    $.each(zaceProduct, function (j, jtem) {
                                        ...
                                        TypeSource_Standard.ProductID.push({
                                            name: jtem.ProductNo,
                                            value: jtem.ID
                                        })
                                    });
            ...
            com: {
                refreshEditAll: function (fn) {


                    model.com.getStandardPartList({ IPTMode: IPTMode, LineID: wSelectID, PartID: lSelectID, ProductID: pSelectIDPro }, function (res) {
                        if (!res) {
                            return;
                        }
                ...
                RanderSelect: function (wID) {
                    ...
                    model.com.getFMCLineUnit({ LineID: wID, ID: 0, ProductID: pSelectIDPro }, function (res) {
                        ...
                        // for (var m = 0; m < list.length; m++) {
                        //     if (list[m].Type == 1 && list[m].LevelID == 2) {
                        //         _zaceList.push(list[m]);
                        //     }
                        ...
                        $("#lmvt-select-line").val(lSelectID).trigger("change");
                        model.com.refreshPartAll();
                    });
                },
                //删除
                ...
                getStandardPartList: function (data, fn, context) {
                    var d = {
                        $URI: "/IPTStandard/StationVersionAll",
                        $TYPE: "get",
                        $SERVER: '/MESQMS',
                    };

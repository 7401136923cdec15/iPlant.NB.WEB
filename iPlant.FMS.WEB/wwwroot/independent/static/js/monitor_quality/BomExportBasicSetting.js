require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/base/paging'], function ($zace, $com, $page) {
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            window.setFunctionTrigger("BomExportBasicSetting", function (res) {
                ...
                $('.zace-header-title').text('(' + mPartNo + ')' + '台车数据检查');
                model.com.loadZace();
                // alert(res.ID);
                //刷新
            });
        ...
        com: {
            ...
            getAPSBOMCheck: function (data, fn, context) {
                var d = {
                    $URI: "/OMSOrder/Check",
                    $TYPE: "get",
                    $SERVER:'/MESAPS',
            ...
            refresh: function () {
                ...
                model.com.getAPSBOMCheck({ OrderID: mOrderID }, function (resP) {
                    if (!resP)
                        return;
                    if (resP && resP.list) {
                        resP.list.sort(function (a, b) { return Number(a.Type) - Number(b.Type) });

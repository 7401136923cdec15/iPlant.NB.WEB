require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base'], function ($yang, $com) {
    ...
    HTML = {
        DeviceTemplate: [
            ...
            '<div class="col-md-6 lmvt-do-info"><UL id="lmvt-nav">',
            '<LI class="lmvt-do-info">状态设置<UL>',
            '<LI data-value="{{ID}}" id="active2">闲置</LI>',
    ...
    model = $com.Model.create({
        ...
        events: function () {
            $("body").delegate("#device_apply", "click", function () {
                var vdata = { 'header': '备件申请单', 'id': 'SpareApplyList', 'href': './device_manage/spareApplyList.html', 'src': './static/images/menu/deviceManage/deviceMaintainLog.png' };
            ...
            $("body").delegate("#lmvt-left-check1", "click", function () {
                ...
                $("body").append($com.modal.show(DEFAULT_VALUE3, KEYWORD3, "查询", function (rst) {
                    if (!rst || $.isEmptyObject(rst))
                        return;
                    model.com.getRecond({
                        SpareLedgerID: SpareLedgerID, DeviceLedgerID: rst.DeviceNo, Used: -1, StartTime: $com.util.format('yyyy-MM-dd ', rst.StartTime), EndTime: $com.util.format('yyyy-MM-dd ', rst.EndTime)
                    }, function (res) {
                        if (!res)
                            return;
                        var list = res.list;
                        for (var i = 0; i < list.length; i++) {
                            list[i].StartTime = $com.util.format("yyyy-MM-dd hh:mm:ss", list[i].StartTime);
                            list[i].EndTime = $com.util.format("yyyy-MM-dd hh:mm:ss", list[i].EndTime);
                            if (list[i].Used == 0)
                                list[i].Used = "是";
                            if (list[i].Used == 1)
                                list[i].Used = "否";
            ...
            refresh: function () {
                ...
                model.com.get({
                    ...
                    for (var i = 0; i < RanderData.length; i++) {
                        RanderData[i].OperatorTime = $com.util.format("yyyy-MM-dd hh:mm:ss", RanderData[i].OperatorTime);

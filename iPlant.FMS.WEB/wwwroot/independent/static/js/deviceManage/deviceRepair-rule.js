    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            $("body").delegate("#zace-add-Device-Regular", "click", function () {
                ...
                if (StandardID == 0) {
                    alert("未选择设备或者设备型号，请先点击左侧设备树中的数据");
                    return false;
                }

                if (StandardObj.length <= 0) {
                    alert("当前型号未设置保养项点，请设置后重新再试");
                    return false;
                }
        ...
        com: {
            ...
            refresh: function () {
                model.com.getDeviceMaintainRule({
                    ...
                    BusinessUnitID: 0, BaseID: 0, FactoryID: 0, WorkShopID: 0, LineID: 0, MaintainType: 2, StandardID: StandardObj.ID
                }, function (resRule) {
                    if (!resRule)
                        return;
            ...
            refreshStandard: function () {
                model.com.getStandardCurrent({ IPTMode: IPTMode, BusinessUnitID: -1, BaseID: -1, FactoryID: -1, WorkShopID: -1, LineID: 0, PartID: -1, PartPointID: -1, StationID: wModelID, ProductID: StandardID }, function (res) {
                    StandardObj = res.info;
                });
            },
            //版本查询

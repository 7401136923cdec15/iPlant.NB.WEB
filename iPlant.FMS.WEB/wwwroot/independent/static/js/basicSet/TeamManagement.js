require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base'], function ($alfie, $com) {
    ...
    HTML = {
        TableNode_item: [
            ...
            '<td style="min-width: 50px" data-title="LeaderName" data-value="{{LeaderName}}">{{LeaderName}}</td>',
            '<td style="min-width: 50px" data-title="MateName" data-value="{{MateName}}">{{MateName}}</td>',
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            $("body").delegate("#alfie-add-level", "click", function () {
                ...
                $("body").append($com.modal.show(DEFAULT_VALUE_D, KEYWORD_department, "新增班组", function (rst) {
                    //调用插入函数然后用load刷新数据源

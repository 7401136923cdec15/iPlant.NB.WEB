require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base'], function ($zace, $com) {
    ...
    HTML = {
        ...
        TableCar: [
            ...
            '<td style="min-width: 50px;vertical-align: middle;">{{ReworkNum}}</td>',
            '<td style="min-width: 50px;vertical-align: middle;">{{NcrNum}}</td>',
            '<td style="min-width: 50px;vertical-align: middle;">{{ExcNum}}</td>',
        ...
        EXCTable: [
            ...
            '<p style="float: left;margin-left: 0.6vw;margin-right: 1vw;">异常信息</p><p style="float: left; margin-right: 1vw;color: #C51D1D;">A级:{{NumA}}</p> <p style="float: left;margin-right: 1vw;color: #EB3B3B;">B级:{{NumB}}</p> <p style="float: left;margin-right: 1vw;color: #F5B128;">C级:{{NumC}}</p > <p style="float: left;color: #1c7171;">D级:{{NumD}}</p>',
            '</div>',
        ].join(""),
    };
    ...
    model = $com.Model.create({
        ...
        run: function () {
            // var wName="sddd";
            // var wChinese="张三ss";
            // alert(/[\u4e00-\u9fa5]+/.test(wName));
            // alert(/[\u4e00-\u9fa5]+/.test(wChinese));
            ...
            refreshOrder: function () {
                model.com.getTrainOrderInfoList({}, function (res) {
                    ...
                    for (var i = 0; i < OrderDataList.length; i++) {
                        ...
                        if (OrderDataList[i].PartNo == "1C#0391") {
                            OrderDataList[i].FinishNum = 1;
                            OrderDataList[i].SurplusNum = 9;
                            OrderDataList[i].Progress = 10;
                            OrderDataList[i].Status = "工位开工";
                        }
                            ...
                            model.com.getEXCAndonAll({ShiftID: NowTime}, function (res2) {
                                ...
                                model.com.getAPSTaskPart({}, function (resPart) {
                                    ...
                                    model.com.getTimeAll({
                                        StartTime: sTimeNcr, EndTime: ETimeNcr,
                                    }, function (resNcr) {
                    ...
                    model.com.GetTask({StartTime: StartTime, EndTime: EndTime, PartNo: ""}, function (res) {
                        ...
                        if (res && res.list) {
                            ...
                            for (var i = 0; i < res.list.length; i++) {
                                if (res.list[i].Status == 1 || res.list[i].Status == 2 || res.list[i].Status == 3 || res.list[i].Status == 4 || res.list[i].Status == 5) {

require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/cross2.js', '../static/utils/js/charf.js', '../static/utils/js/base/jquery.treeview', '../static/utils/js/base/paging', '../static/utils/js/pickPeopleWeb'], function ($zace, $com, $cross, $charf, $tree, $page, $pick) {
    ...
    HTML = {
        ...
        TableModeCommitDetail: [
            ...
            '<td data-title="CurbRemark" data-value="{{CurbRemark}}" >{{CurbRemark}}</td>',
            '<td data-title="UpFlowName" data-value="{{UpFlowName}}" >{{UpFlowName}}</td>',
            '<td data-title="CreateTimeText" data-value="{{CreateTimeText}}" >{{CreateTimeText}}</td>',
            '<td data-title="SubmitTime" data-value="{{SubmitTime}}" >{{SubmitTime}}</td>',
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            //多选返回发起界面
            $("body").delegate("#zace-exportMu-commitReturn", "click", function () {
                $(".bottom").hide();
                $('.zacePencil').hide();
                $('.zacePencilDone').hide();
                $('.zacePlanTable').hide();
            ...
            $("body").delegate("#lmvt-reset", "click", function () {
                ...
                mUpFlowID = -1;
                $(".search-content-CheckerID").val("");
                $(".search-content-CheckerID").attr("data-value", 0)
                mCheckerID = -1;
                $(".search-content-PartNo").val("");
                mPartNo="";
            ...
            //监听工件码
            $("body").delegate(".search-content-PartNo", "input", function () {
                var $this = $(this);
                mPartNo = $this.val();
            });
            //查询发起人
            ...
            //查询检查人
            $("body").delegate(".search-content-CheckerID", "click", function () {

                EchoData = [];
                if ($(".search-content-CheckerID").attr("data-value").length > 1) {
                    EchoData = $(".search-content-CheckerID").attr("data-value").split(",");
                }
                ...
                $pick.show(param, function (mNameList) {
                    ...
                    $(".search-content-CheckerID").val(StrArrayName);
                    $(".search-content-CheckerID").attr("data-value", StrArray)
                    mCheckerID = StrArray[0];
                });
            });
        },
        ...
        run: function () {
            ...
            model.com.getFMCLine({ FactoryID: 0, BusinessUnitID: 0, WorkShopID: 0 }, function (resP) {
                ...
                model.com.getFMCWorkShop({ FactoryID: 0, BusinessUnitID: 0 }, function (resW) {
                    ...
                    wWorkShop = $com.util.Clone(wWorkShopList);
        ...
        com: {
            ...
            refreshSend: function (StartTime, EndTime) {
                ...
                model.com.getCurbProduct({
                    FlowType: 2020,
                    UpFlowID: mUpFlowID,
                    CheckerID: mCheckerID,
            ...
            //生产遏制查询
            getCurbProduct: function (data, fn, context) {
                var d = {
                    $URI: "/QMSTask/CurbProduct",
                    $TYPE: "get",
                    $SERVER: "/MESQMS"
                };
                function err() {

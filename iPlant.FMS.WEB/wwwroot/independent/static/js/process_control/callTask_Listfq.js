require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/cross2.js', '../static/utils/js/charf.js', '../static/utils/js/base/jquery.treeview', '../static/utils/js/pickPeopleWeb'
    ...
    wExceptionType = ["", "设备", "物料", "人员", "工艺","环境","检测"];
    ProcessInstanceID = 0;
    ...
    OnSiteList = [
        ...
        HTML = {
            ...
            TableModeCommitDetail: [
                ...
                '<td style="min-width: 50px;display:none" data-title="StepID" data-value="{{StepID}}">{{StepID}}</td> ',
                '<td data-title="Code" data-value="{{Code}}" >{{Code}}</td>',
                '<td data-title="RespondLevelName" data-value="{{RespondLevelName}}" >{{RespondLevelName}}</td>',
                '<td data-title="ExceptionTypeName" data-value="{{ExceptionTypeName}}" >{{ExceptionTypeName}}</td>',
                '<td data-title="UpFlowName" data-value="{{UpFlowName}}" >{{UpFlowName}}</td>',
                '<td data-title="CreateTimeText" data-value="{{CreateTimeText}}" >{{CreateTimeText}}</td>',
                '<td data-title="SubmitTime" data-value="{{SubmitTime}}" >{{SubmitTime}}</td>',
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            $("body").delegate("#lmvt-search", "click", function () {
                ...
                mRespondLevel = $(".search-content-RespondLevel").val() == 0 ? -1 : $(".search-content-RespondLevel").val();
                mExceptionType = $(".search-content-ExceptionType").val() == 0 ? -1 : $(".search-content-ExceptionType").val();
                model.com.refreshSend(mZCommitStartTime, mZCommitEndTime);
            });
            //重置
        ...
        run: function () {
            ...
            model.com.getDevice({
                ...
                model.com.getEXCExceptionType({
                    ...
                    var selectExceptionType_search = $("#slpkExceptionType_search");
                    model.com.ProcessingData(wEXCExceptionType, selectExceptionType_search);
                    ...
                    model.com.getEXCStationAll({ RelevancyType: -1, RelevancyID: -1, StationType: -1, StationName: "", Active: -1 }, function (res) {
                        ...
                        var selectRespondLevel_search = $("#slpkRespondLevel_search");
                        model.com.ProcessingData(RespondLevelList, selectRespondLevel_search);
        ...
        com: {
            ...
            refreshSend: function (StartTime, EndTime) {
                ...
                model.com.getHistory({
                    UpFlowID: mUpFlowID,
                    ResponseLevel: mRespondLevel,
                    ...
                    for (var k = 0; k < wData.length; k++) {
                        ...
                        wData[k].RespondLevelName=wRespondLevel[wData[k].RespondLevel];
                        wData[k].ExceptionTypeName=wExceptionType[wData[k].ExceptionTypeID];
                    }
            ...
            //用人拿任务
            getHistory: function (data, fn, context) {
                var d = {
                    $URI: "/EXCCallTaskBPM/History",
                    $TYPE: "get",
                    $SERVER: "/MESQMS"
                };
                function err() {

require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/cross2.js', '../static/utils/js/charf.js', '../static/utils/js/base/jquery.treeview', '../static/utils/js/pickPeopleWeb', '../static/utils/js/pickDepartment', '../static/utils/js/base/paging'
], function ($zace, $com, $cross, $charf, $tree, $pick, $pickDepartment, $page) {
    ...
    mTypeDone = 3;
    StatusList = [{ ID: 5, Name: "复线申请" }, { ID: 20, Name: "已完成" }];
    CheckResultList = ["合格", "合格", "不合格"];
    MeasuresSourceList = ["", "NCR", "培训记录", "分析报告", "其他"];
    ProcessInstanceID = 0;
    HaltProperty = {
        "ReceivingDeptID": "ReceivingDept",
        "ResponsorIDList": "ResponsorName",
        ...
        "MeasuresSourceList": "MeasuresSourceName",
        "AuditApproval": "AuditApprovalName",
        "TransferPerson": "TransferPersonName",
    };
    ...
    var PropertyBusiness = [{
        ...
        Name: '纠正措施',
        TiTle: 'CorrectiveMeasures',
        Value: '',
    }, {
        Name: '预防措施',
        TiTle: 'PreventiveMeasures',
        Value: '',
    }, {
        Name: '措施来源',
        TiTle: 'MeasuresSourceName',
        Value: '',
    }];
    wMeasuresSourceList = [
        {
        ...
        HTML = {
            ...
            TableModeCommitDetail: [
                ...
                '<td data-title="PartNames" data-value="{{PartNames}}" >{{PartNames}}</td>',
                '<td data-title="StopReason" data-value="{{StopReason}}" >{{StopReason}}</td>',
                '<td data-title="UpFlowName" data-value="{{UpFlowName}}" >{{UpFlowName}}</td>',
                '<td data-title="CreateTimeText" data-value="{{CreateTimeText}}" >{{CreateTimeText}}</td>',
                '<td data-title="SubmitTime" data-value="{{SubmitTime}}" >{{SubmitTime}}</td>',
    ...
    (function () {
        KEYWORD_Level_LIST = [
            "StartTime|生产停线(序)时间|DateTime",
            "EndTime|生产复线(序)时间|DateTime",
        ];
        KEYWORD_Level = {};
        FORMATTRT_Level = {};
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            //时间选择
            $("body").delegate(".Write-Control-StartTime", "click", function () {

                var DEFAULT_VALUE = {
                    StartTime: "",
                }

                $("body").append($com.modal.show(DEFAULT_VALUE, KEYWORD_Level, "时间", function (rst) {
                    //调用插入函数 

                    if (!rst || $.isEmptyObject(rst))
                        return;

                    $('.Write-content-StartTime').text($com.util.format("yyyy-MM-dd hh:mm", rst.StartTime));
                }, TypeSource_Level));

            });
            //时间选择
            $("body").delegate(".Write-Control-EndTime", "click", function () {
                ...
                $("body").append($com.modal.show(DEFAULT_VALUE, KEYWORD_Level, "时间", function (rst) {
                    ...
                    $('.Write-content-EndTime').text($com.util.format("yyyy-MM-dd hh:mm", rst.EndTime));
                }, TypeSource_Level));
            ...
            //查询发出部门
            $("body").delegate(".search-content-IssuingDeptID", "click", function () {
                EchoData = [];
                if ($(".search-content-IssuingDeptID").attr("data-value").length > 1) {
                    EchoData = $(".search-content-IssuingDeptID").attr("data-value").split(",");
                }
                ...
                $pickDepartment.show(param, function (mNameList) {
                    ...
                    $(".search-content-IssuingDeptID").val(StrArrayName);
                    $(".search-content-IssuingDeptID").attr("data-value", StrArray)
                    mIssuingDeptID = StrArray[0];
                });
            });
            //查询受单部门
            $("body").delegate(".search-content-ReceivingDeptID", "click", function () {
                EchoData = [];
                if ($(".search-content-ReceivingDeptID").attr("data-value").length > 1) {
                    EchoData = $(".search-content-ReceivingDeptID").attr("data-value").split(",");
                }
                ...
                $pickDepartment.show(param, function (mNameList) {
                    ...
                    $(".search-content-ReceivingDeptID").val(StrArrayName);
                    $(".search-content-ReceivingDeptID").attr("data-value", StrArray)
                    mReceivingDeptID = StrArray[0];
                });
            });
            //选择受单部门
            $("body").delegate(".Write-Control-ReceivingDeptID", "click", function () {
                EchoData = [];
                if ($(".Write-content-ReceivingDeptID").attr("data-value").length > 1) {
                    EchoData = $(".Write-content-ReceivingDeptID").attr("data-value").split(",");
                }
                ...
                $pickDepartment.show(param, function (mNameList) {
                    ...
                    $(".Write-content-ReceivingDeptID").text(StrArrayName);
                    $(".Write-content-ReceivingDeptID").attr("data-value", StrArray)
                });
            });
            //查询发起人
            ...
            $("body").delegate("#lmvt-reset", "click", function () {
                ...
                mUpFlowID = -1;
                $(".search-content-IssuingDeptID").val("");
                $(".search-content-IssuingDeptID").attr("data-value", 0)
                mIssuingDeptID = -1;
                $(".search-content-ReceivingDeptID").val("");
                $(".search-content-ReceivingDeptID").attr("data-value", 0)
                mReceivingDeptID = -1;
            ...
            //监听车间
            $("body").delegate(".search-content-WorkShopID", "change", function () {
                var $this = $(this);
                mWorkShopID = $this.val() == 0 ? -1 : $this.val();
            });
            //监听产线
            $("body").delegate(".search-content-LineID", "change", function () {
                var $this = $(this);
                mLineID = $this.val() == 0 ? -1 : $this.val();
            });
            //监听状态
            $("body").delegate(".search-content-Status", "change", function () {
                var $this = $(this);
                mStateID = $this.val() == 0 ? -1 : $this.val();
            });
        },
        ...
        run: function () {
            ...
            var selectMeasuresSourceList = $("#slpkMeasuresSourceList");
            model.com.ProcessingData(wMeasuresSourceList, selectMeasuresSourceList);
            model.com.getFMCWorkShop({ FactoryID: 0, BusinessUnitID: 0 }, function (resF) {
                wWorkShop = resF.list;
                var selectWorkShopID = $("#slpkWorkShopID_search");
                model.com.ProcessingData(wWorkShop, selectWorkShopID);
                model.com.getFMCLine({ FactoryID: 0, BusinessUnitID: 0, WorkShopID: 0 }, function (resP) {
                    ...
                    wLineSearch = $com.util.Clone(wLineList);
                    var selectLineID = $("#slpkLineID");
                    model.com.ProcessingData(wLineList, selectLineID);
                    var selectLineIDSearch = $("#slpkLineID_search");
                    model.com.ProcessingData(wLineSearch, selectLineIDSearch);
                    //获取所有部门
                    model.com.getFPCPart({
                        FactoryID: 0, BusinessUnitID: 0
                    }, function (data) {
                        wPartList = data.list;
                        var selectPartList = $("#slpkPartList");
                        model.com.ProcessingData(wPartList, selectPartList);

                        model.com.getDepartment({
                        }, function (data) {
                            wDepartment = $com.util.Clone(data.list);
                            // var wDepartmentReceivingDept = $com.util.Clone(data.list);
                            // var selectReceivingDeptID = $("#slpkReceivingDeptID");
                            // model.com.ProcessingData(wDepartmentReceivingDept, selectReceivingDeptID);
        ...
        com: {
            ...
            addCheck: function (Text) {
                ...
                if (Todotasks.ActivitiID == "Stop1") {
                    _listCommitObj.PartList = _listCommitObj.PartList.split(",");
                }
                if (Todotasks.ActivitiID == "Stop6") {
                    _listCommitObj.MeasuresSourceList = _listCommitObj.MeasuresSourceList.split(",");
                }
                if ($com.util.format('yyyy-MM-dd hh:mm', _listCommitObj.StartTime) < $com.util.format('yyyy-MM-dd hh:mm', new Date()) && Todotasks.ActivitiID == "Stop1") {
                    alert("生产停线(序)时间小于当前时间");
                    return false;
                }
                if ($com.util.format('yyyy-MM-dd hh:mm', _listCommitObj.EndTime) < $com.util.format('yyyy-MM-dd hh:mm', new Date()) && Todotasks.ActivitiID == "Stop6") {
                    alert("生产复线(序)时间小于当前时间");
                    return false;
                }

                $com.util.deleteLowerProperty(_listCommitObj);
                $com.app.loaded();
                if (!confirm("是否" + Text + "生产停线(序)？")) {
                    return;
                }
                $com.app.loading("提交中!");
            ...
            refreshSend: function (StartTime, EndTime) {
                ...
                model.com.getStopProduct({
                    FlowType: 2007,
                    UpFlowID: mUpFlowID,
                    WorkShopID: mWorkShopID,
                    LineID: mLineID,
                    AuditResult: mAuditResult,
                    IssuingDeptID: mIssuingDeptID,
                    ReceivingDeptID: mReceivingDeptID,
                    StateID: mStateID,
                    StartTime: StartTime,
                    EndTime: EndTime,
                }, function (data) {
                    ...
                    $page.init($("#femi-zacePlanSend-tbody").closest("table"), wData, "", function (res) {
                        $("#femi-zacePlanSend-tbody").html($com.util.template(res, HTML.TableModeCommitDetail));
                    });
                    // model.com.searchTopHeight($(".ds-search-top-contain")); 
                    $(window).resize();
            ...
            //停线序查询
            getStopProduct: function (data, fn, context) {
                var d = {
                    $URI: "/QMSTask/StopProduct",
                    $TYPE: "get",
                    $SERVER: "/MESQMS"
                };
                function err() {
            ...
            BuinessDetail: function (wInfo, type) {
                if (type == 1) {
                    $(".headerAll .BasicTable .FieldContentOne .Contentshow").text("待发起生产停线(序)");
                } else {
            ...
            Handle: function (wInfo) {
                var SourceList = [];
                if (wInfo.MeasuresSourceList.length > 0) {
                    for (var k = 0; k < wInfo.MeasuresSourceList.length; k++) {
                        SourceList.push(MeasuresSourceList[wInfo.MeasuresSourceList[k]]);
                    }
                }
                wInfo.MeasuresSourceName = SourceList.join(",");
                if ($com.util.format('yyyy-MM-dd hh:mm', wInfo.EndTime) < $com.util.format('yyyy-MM-dd hh:mm', "2010-1-1 01:01")) {
                    wInfo.EndTime = "";
                }
            },
            ...
            TranslationField: function (wFormPropertyList) {
                for (var i = 0; i < wFormPropertyList.length; i++) {
                    //翻译配置的受单部门以及默认通知部门负责人
                    if (wFormPropertyList[i].Key == "ReceivingDeptID") {
                        mCloneData.ReceivingDeptID = wFormPropertyList[i].Variable;
                    }
                    if (wFormPropertyList[i].Key == "TransferPerson") {
                        mCloneData.ResponsorIDListString = wFormPropertyList[i].Variable
                        mCloneData.ResponsorIDList = wFormPropertyList[i].Variable.split(",");
                        mCloneData.TransferPerson = wFormPropertyList[i].Variable;
                    }
                }
                for (var k = 0; k < wDepartment.length; k++) {
                    if (wDepartment[k].ID == mCloneData.ReceivingDeptID) {
                        mCloneData.ReceivingDept = wDepartment[k].Name;
                    }
                }

                if (!mCloneData.ResponsorIDListString) {
                    mCloneData.ResponsorName = "";
                    mCloneData.TransferPersonName = "";
                } else {
                    mCloneData.ResponsorName = model.com.getDepartmentName(mCloneData.ResponsorIDListString, wUser);
                    mCloneData.TransferPersonName = model.com.getDepartmentName(mCloneData.ResponsorIDListString, wUser);
                }
                $(".Write-content-TransferPerson").attr("data-value", mCloneData.ResponsorIDList);
                $(".Write-content-TransferPerson").text(mCloneData.ResponsorName);
            },

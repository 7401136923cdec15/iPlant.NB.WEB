require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/cross2.js', '../static/utils/js/charf.js', '../static/utils/js/base/jquery.treeview', '../static/utils/js/pickPeopleWeb', '../static/utils/js/pickDepartment'
    ...
    wLevelList = ["无", "I级", "II级"];
    wCheckResult = ["", "合格", "不合格"];
    wRepairTypeList = ["", "外购件", "内部", "顾客反馈"];
    wHandleTypeList = ["", "返工", "返修"];
    ProcessInstanceID = 0;
    ...
    var PropertyBusiness = [{
        ...
        Name: '返工(修)复检要求',
        TiTle: 'RecheckStandard',
        Value: '',
    }, {
        Name: '质量问题等级',
        TiTle: 'RepairLevelName',
        Value: '',
    }, {
        Name: '返工(修)结果',
    ...
    CheckResult = [
        ...
        HTML = {
            ...
            TableModeCommitDetail: [
                ...
                '<td style="min-width: 50px;display:none" data-title="StepID" data-value="{{StepID}}">{{StepID}}</td> ',
                '<td data-title="Code" data-value="{{Code}}" >{{Code}}</td>',
                '<td data-title="RepairTypeName" data-value="{{RepairTypeName}}" >{{RepairTypeName}}</td>',
                '<td data-title="ProductName" data-value="{{ProductName}}" >{{ProductName}}</td>',
                '<td data-title="ProductNo" data-value="{{ProductNo}}" >{{ProductNo}}</td>',
                '<td data-title="HandleTypeName" data-value="{{HandleTypeName}}" >{{HandleTypeName}}</td>',
                '<td data-title="ExecutiveDept" data-value="{{ExecutiveDept}}" >{{ExecutiveDept}}</td>',
                '<td data-title="RepairLevelName" data-value="{{RepairLevelName}}" >{{RepairLevelName}}</td>',
                '<td data-title="UpFlowName" data-value="{{UpFlowName}}" >{{UpFlowName}}</td>',
                '<td data-title="CreateTimeText" data-value="{{CreateTimeText}}" >{{CreateTimeText}}</td>',
                '<td data-title="SubmitTime" data-value="{{SubmitTime}}" >{{SubmitTime}}</td>',
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            $("body").delegate(".cby-charf-detail-item", "click", function () {
                ...
                for (var m = 0; m < DetailList.length; m++) {
                    if (DetailList[m].Name.indexOf("附件") != -1 || DetailList[m].Name == "返修流程图或作业指导书" || DetailList[m].Name.indexOf("图片") != -1) {
                        var Temp = "";
            ...
            $("body").delegate("#lmvt-search", "click", function () {
                ...
                mLineID = $(".search-content-LineID").val() == 0 ? -1 : $(".search-content-LineID").val();
                mProductID = $(".search-content-ProductNo").val() == 0 ? -1 : $(".search-content-ProductNo").val();
                mRepairType = $(".search-content-RepairType").val() == 0 ? -1 : $(".search-content-RepairType").val();
                mHandleType = $(".search-content-HandleType").val() == 0 ? -1 : $(".search-content-HandleType").val();
                mRepairLevel = $(".search-content-RepairLevel").val() == 0 ? -1 : $(".search-content-RepairLevel").val();
                mStateID = $(".search-content-Status").val()== 0 ? -1 : $(".search-content-Status").val();
                mExecutiveDeptID = $(".search-content-ExecutiveDeptID").attr("data-value") == 0 ? -1 : $(".search-content-ExecutiveDeptID").attr("data-value");
                model.com.refreshSend(mZCommitStartTime, mZCommitEndTime);
            });
            //重置
            $("body").delegate("#lmvt-reset", "click", function () {
                ...
                mUpFlowID = -1;
                $(".search-content-ExecutiveDeptID").val("");
                $(".search-content-ExecutiveDeptID").attr("data-value", 0)
                mExecutiveDeptID = -1;
            ...
            //查询执行部门
            $("body").delegate(".search-content-ExecutiveDeptID", "click", function () {
                EchoData = [];
                if ($(".search-content-ExecutiveDeptID").attr("data-value").length > 1) {
                    EchoData = $(".search-content-ExecutiveDeptID").attr("data-value").split(",");
                }
                ...
                $pickDepartment.show(param, function (mNameList) {
                    ...
                    $(".search-content-ExecutiveDeptID").val(StrArrayName);
                    $(".search-content-ExecutiveDeptID").attr("data-value", StrArray)
                    mExecutiveDeptID = StrArray[0];
                });
            });
        },
        ...
        run: function () {
            ...
            var selectCheckResult = $("#slpkCheckResult");
            model.com.ProcessingData(CheckResult, selectCheckResult);

            var selectRepairLevel = $("#slpkRepairLevel");
            model.com.ProcessingData(LevelList, selectRepairLevel);

            var selectRepairLevel_search = $("#slpRepairLevel_search");
            model.com.ProcessingData(LevelList, selectRepairLevel_search);
            var selectRepairType = $("#slpkRepairType");
            model.com.ProcessingData(RepairTypeList, selectRepairType);

            var selectRepairType_search = $("#slpRepairType_search");
            model.com.ProcessingData(RepairTypeList, selectRepairType_search);

            var selectHandleType = $("#slpkHandleType");
            model.com.ProcessingData(HandleTypeList, selectHandleType);

            var selectHandleType_search = $("#slpHandleType_search");
            model.com.ProcessingData(HandleTypeList, selectHandleType_search);
            ...
            model.com.getFPCProduct({ ProductTypeID: 0, BusinessUnitID: 0 }, function (resP) {
                ...
                for (var i = 0; i < wProductName.length; i++) {
                    wProductName[i].Name = wProductName[i].ProductName + "(" + wProductName[i].ProductNo + ")";
                }
                ...
                model.com.ProcessingData(wProductName, selectProductNo_search);
                model.com.getDepartment({
                    ...
                    model.com.getUser({}, function (res) {
                        ...
                        model.com.getFMCWorkShop({ FactoryID: 0, BusinessUnitID: 0 }, function (resW) {
                            ...
                            var selectWorkShopID_search = $("#slpkWorkShopID_search");
                            model.com.ProcessingData(wWorkShopList, selectWorkShopID_search);
                            model.com.getFMCLine({ FactoryID: 0, BusinessUnitID: 0, WorkShopID: 0 }, function (resP) {
                                ...
                                var selectLineID_search = $("#slpkLineID_search");
                                model.com.ProcessingData(wLineList, selectLineID_search);
                                model.com.refreshSend(mZCommitStartTime, mZCommitEndTime);
                            });
                        });
                    });
        ...
        com: {
            ...
            addCheck: function (Text) {
                if (mQueryTaskID <= 0) {
                    mCloneData.ApplyID = mCloneData.UpFlowID + ""
                }
                    ...
                    if (!$WriteContent[0]) {
                        ...
                        if ($(".Write-Control-" + wFormPropertyList[i].Key).hasClass("Select")) {
                            ...
                            if ($(".Write-Control-ImageList").is(":visible")) {
                                ...
                                if (Todotasks.ActivitiID == "RRO2") {
                                    _listCommitObj.RepairProcessFileList = FilePath;
                                }

                                if (Todotasks.ActivitiID == "RRO14") {
            ...
            refreshSend: function (StartTime, EndTime) {
                ...
                model.com.getRepairTask({
                    ...
                    RepairType: mRepairType,
                    HandleType: mHandleType,
                    ExecutiveDeptID: mExecutiveDeptID,
                    RepairLevel: mRepairLevel,
                    StateID: mStateID,
                }, function (data) {
                    wData = [];
                    ...
                    for (var k = 0; k < wData.length; k++) {
                        ...
                        wData[k].RepairTypeName=wRepairTypeList[wData[k].RepairType];
                        wData[k].HandleTypeName=wHandleTypeList[wData[k].HandleType];
                        wData[k].RepairLevelName=wLevelList[wData[k].RepairLevel];
                    }
            ...
            //用人拿任务
            getRepairTask: function (data, fn, context) {
                var d = {
                    $URI: "/QMSTask/RepairTask",
                    $TYPE: "get",
                    $SERVER: "/MESQMS"
                };
                function err() {
            ...
            BuinessDetail: function (wInfo, type) {
                if (type == 1) {
                    $(".headerAll .BasicTable .FieldContentOne .Contentshow").text("待发起 返工(修)");
                } else {
            ...
            Handle: function (wInfo) {
                wInfo.RepairLevelName = wLevelList[wInfo.RepairLevel];
                wInfo.RepairTypeName = wRepairTypeList[wInfo.RepairType];
                wInfo.HandleTypeName = wHandleTypeList[wInfo.HandleType];
                wInfo.CheckResultName = wCheckResult[wInfo.CheckResult];
            },
            ...
            TranslationField: function (wFormPropertyList) {
                for (var i = 0; i < wFormPropertyList.length; i++) {
                    ...
                    if (wFormPropertyList[i].Key == "ExecutiveDeptID") {
                        mCloneData.ExecutiveDeptID = wFormPropertyList[i].Variable;
                    }
                }
                for (var k = 0; k < wDepartment.length; k++) {
                    if (wDepartment[k].ID == mCloneData.ExecutiveDeptID) {
                        mCloneData.ExecutiveDept = wDepartment[k].Name;
                    }
                }
                $(".Write-content-ExecutiveDeptID").val(mCloneData.ExecutiveDept);
                $(".Write-content-ExecutiveDeptID").attr("data-value", mCloneData.ExecutiveDeptID);
            },
            unique: function (arr) {

require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/cross2.js', '../static/utils/js/charf.js', '../static/utils/js/base/jquery.treeview', '../static/utils/js/pickPeopleWeb'
    ...
    HTML = {
        ...
        TableModeCommitDetail: [
            ...
            '<td data-title="LedgerName" data-value="{{LedgerName}}" >{{LedgerName}}</td>',
            '<td data-title="ModelNo" data-value="{{ModelNo}}" >{{ModelNo}}</td>',
            '<td data-title="CalibrationResultName" data-value="{{CalibrationResultName}}" >{{CalibrationResultName}}</td>',
            '<td data-title="UpFlowName" data-value="{{UpFlowName}}" >{{UpFlowName}}</td>',
            '<td data-title="CreateTimeText" data-value="{{CreateTimeText}}" >{{CreateTimeText}}</td>',
            '<td data-title="SubmitTime" data-value="{{SubmitTime}}" >{{SubmitTime}}</td>',
        ...
        InspectionItems: [
            '<tr data-value={{ID}} data-type={{ItemType}} data-standardtype={{StandardType}}>',
            '<td>{{WID}}</td>',
            '<td>{{Text}}</td>',
            '<td>{{Standard}}</td>',
            '<td><input type="checkbox" class="switch Write-content-CheckResult" checked style="" data-value="1"></td>',
            '{{ITEMControl}}',
            '<td style="padding: 0px;"><div style="width: 100%;height: 45px;"><input class="Write-content-Remark" style="width: 100%;height: 100%;border: 0;" data-value="" placeholder="请填备注" value=""></div></td>',
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            $("body").delegate("#femi-tree-update", "click", function () {
                ...
                for (var m = 0; m < list.length; m++) {
                    TextList.push(list[m].Text);
                    CheckIDList.push(list[m].ID)
                }
                $(".Write-content-MaintainItem").text(TextList.join(","));
                $(".Write-content-MaintainItem").attr("data-value", CheckIDList.join(","));
                $("#myModalItem").hide();
                $('.modal-backdrop').remove();
            });
            //关闭选择项的tree
            $("body").delegate("#femi-tree-remove", "click", function () {
                $("#myModalItem").hide();
                $('.modal-backdrop').remove();
            });
            ...
            $("body").delegate("#lmvt-search", "click", function () {
                ...
                mLedgerID = $(".search-content-LedgerID").val() == 0 ? -1 : $(".search-content-LedgerID").val();
                mCalibrationResult = $(".search-content-CalibrationResult").val() == 0 ? -1 : $(".search-content-CalibrationResult").val();
                mModelIDParm = $(".search-content-ModelID").val() == 0 ? -1 : $(".search-content-ModelID").val();
                mWorkShopIDParm = $(".search-content-WorkShopID").val() == 0 ? -1 : $(".search-content-WorkShopID").val();
                mLineIDParm = $(".search-content-LineID").val() == 0 ? -1 : $(".search-content-LineID").val();
                mStatus = $(".search-content-Status").val() == 0 ? -1 : $(".search-content-Status").val();
                model.com.refreshSend(mZCommitStartTime, mZCommitEndTime);
            });
            //重置
        ...
        run: function () {
            ...
            var selectCalibrationResult_search = $("#slpCalibrationResult_search");
            model.com.ProcessingData(wMaintainResultList, selectCalibrationResult_search);

            model.com.getDeviceModel({ Active: -1, Type: 1 }, function (resD) {
                mDeviceModel = [];
                for (var i = 0; i < resD.list.length; i++) {
                    if (resD.list[i].Active == 1) {
                        resD.list[i].Name = resD.list[i].ModelName + "(" + resD.list[i].ModelNo + ")";
                        mDeviceModel.push(resD.list[i]);
                    }
                }
                var selectModelID_search = $("#slpModelID_search");
                model.com.ProcessingData(mDeviceModel, selectModelID_search);
                model.com.getDeviceLedger({
                    ModelID: 0, WorkShopID: 0, LineID: 0, BusinessUnitID: 0, BaseID: 0, FactoryID: 0, ApplyID: 0, Type: 3
                }, function (resL) {
                    ...
                    mDeviceLedger = $com.util.Clone(resL.list);
                    var selectLedgerID_search = $("#slpLedgerID_search");
                    model.com.ProcessingData(mDeviceLedger, selectLedgerID_search);
                    model.com.getFMCLine({ FactoryID: 0, BusinessUnitID: 0, WorkShopID: 0 }, function (resP) {
                        ...
                        var selectslpkLineID_search = $("#slpkLineID_search");
                        model.com.ProcessingData(wLineList, selectslpkLineID_search);
        ...
        com: {
            ...
            renderTree: function (list) {
                ...
                function FindData(wGroupID) {
                    var _rst_Array = [];
                    $.each(list, function (i, item) {
                        if (wGroupID == item.GroupID) {
            ...
            renderTreeCheck: function (list) {
                ...
                $("#roleTree li span input[type=checkbox].femi-tree-checkbox").each(function (i, item) {
                    ...
                    var _index = $com.util.findIndex(list, function (p) { return p.ID == functionID });
                    if (_index < 0) {
                ...
                if (Todotasks.ActivitiID == "Maintain4") {
                    ...
                    model.com.save({
                        ...
                        model.com.getEmployeeInfo({
                            ...
                            $com.app.loaded();
                            if (Todotasks.ActivitiID == "Maintain4") {
                                for (var k = 0; k < mCloneData.MaintainValueList.length; k++) {
                                    if (mCloneData.MaintainValueList[k].ID == 0) {
                                        alert("请填写检验项点！");
                                        return false;
                                    }
                                    if (mCloneData.MaintainValueList[k].Value.length == 0 && mCloneData.MaintainValueList[k].ItemType == 2 && mCloneData.MaintainValueList[k].StandardID == mCloneData.StandardID && mCloneData.MaintainValueList[k].IPTItemID != 0) {
                                        alert("实际值必填！");
                                        return false;
                                    }
                                }
                            }
                            if (!confirm("是否" + Text + "量具校正流程？")) {
                                return;
                            }
                            _listCommitObj.MaintainValueList = mCloneData.MaintainValueList;
                            _listCommitObj.MaintainItemList = [];
            ...
            getItemValue: function () {
                ...
                $("#femi-Import-tbody tr").each(function (i, item) {
                    ...
                    var StandardType = $this.attr("data-standardtype");
                    ...
                    if (ItemType == 2) {
                        var Obj = {
                            ID: 0,
                            StandardID: mCloneData.StandardID,
                            IPTItemID: $this.attr("data-value"),
                            IPTMode: 20,
                            VideoPath: [],
                            SolveID: 0,
                            Manufactor: "",
                            ...
                            TaskID: mCloneData.ID,
                            Value: $this.find("td div .Write-content-Value").val(),
                        }
                        if (StandardType == 1 || StandardType == 11 || StandardType == 12) {
                            var SelectedList = [];
                            $this.find("td div .selectpicker option:selected").each(function (i, item) {
                                SelectedList.push($(item).text());
                            });
                            Obj.Value = SelectedList.join(",");
                        }
            ...
            refreshSend: function (StartTime, EndTime) {
                ...
                model.com.getTaskAll({
                    FlowType: 2022,
                    StartTime: StartTime,
                    EndTime: EndTime,
                    LedgerID: mLedgerID,
                    MaintainResult: mCalibrationResult,
                    CalibrationType: 1,
                    ModelID: mModelIDParm,
                    WorkShopID: mWorkShopIDParm,
                    LineID: mLineIDParm,
                    Status: mStatus,
                    BaseID: -1,
                    StandardID: -1,
                    ...
                    for (var k = 0; k < wData.length; k++) {
                        ...
                        if (wData[k].CalibrationResult == 1) {
                            wData[k].CalibrationResultName="合格";
                        }else if(wData[k].CalibrationResult == 2){
                            wData[k].CalibrationResultName="不合格";
                        }else{
                            wData[k].CalibrationResultName="无";
                        }
                ...
                if (mQueryTaskID <= 0) {
                    ...
                    model.com.getDeviceInfo({ ID: mDevice }, function (res) {
                        ...
                        model.com.getMaintainItem({
                            IPTMode: 20, BusinessUnitID: -1, BaseID: -1, FactoryID: -1, WorkShopID: -1, LineID: 0, PartID: -1, PartPointID: -1, StationID: res.info.ModelID, ProductID: res.info.WorkType
                        }, function (res) {
                            StandardInfo = res.info;
                            MaintainItem = [];
                            for (var i = 0; i < res.info.ItemList.length; i++) {
                                // if (res.info.ItemList[i].ItemType == 2) {
                                res.info.ItemList[i].Name = res.info.ItemList[i].Text;
                                MaintainItem.push(res.info.ItemList[i]);
                                // }
                            }
                            model.com.renderTree(MaintainItem);

                            // var selectMaintainItem = $("#slpkMaintainItem");
                            // model.com.ProcessingData(MaintainItem, selectMaintainItem);
                            model.com.onTask({
            ...
            CountStandard: function (StandardObject, number) {
                var standardString = '';
                switch (number) {
                    case 0: standardString = ''; break;
                    case 1: standardString = ''; break;
            ...
            CheckItem: function (mCloneDataInfo, Type) {
                ...
                slpkDateGroup = [];
                _zaceTreeLsit = model.com.buildTreePro(CheckItem, Type);
                var m = slpkDateGroup.length;
                slpkDate = [];
                for (var i = 0; i < _zaceTreeLsit._item.length; i++) {
                    _zaceTreeLsit._item[i].NumberText = model.com.CountStandard(_zaceTreeLsit._item[i], Number(_zaceTreeLsit._item[i].StandardType));
                    switch (_zaceTreeLsit._item[i].StandardType) {
                        case 0:
                        ...
                        case 1:
                            ...
                            var slpkList = [];
                            slpkList.push({
                                slpkWorkValue: "slpkWorkValue" + m
                            });
                            slpkDate.push({
                                IDName: slpkList[0],
                                ValueSource: _zaceTreeLsit._item[i].ValueSource
                            })
                            htmlTemp = $com.util.template(slpkList, HTML.ITEM.Combo);
                            break;
                        case 2:
                        case 3:
                        case 4:
                        ...
                        case 11:
                            ...
                            htmlTemp = $com.util.template(slpkList, HTML.ITEM.Check);
                            break;
                        case 12:
                            m = m + 1;
                            var slpkList = [];
                    ...
                    _zaceTreeLsit._item[i].ITEMControl = $com.util.template(_zaceTreeLsit._item[i], htmlTemp);

                    // _zaceTreeLsit._item[i].WID = i + 1;
                    for (var k = 0; k < wResult.length; k++) {
                        if (_zaceTreeLsit._item[i].ID == wResult[k].IPTItemID) {
                            _zaceTreeLsit._item[i].Value = wResult[k].Value;
                            _zaceTreeLsit._item[i].Remark = wResult[k].Remark;
                            _zaceTreeLsit._item[i].ResultText = CheckResultList[wResult[k].Result];
                            var ImgList = [];
                            var Temp = '<img class="ShowImg" src="{{Value}}" style="width: 30px;height: 30px;margin: 2px;"></img>';
                            if (wResult[k].ImagePath.length > 0) {
                                for (var m = 0; m < wResult[k].ImagePath.length; m++) {
                                    ImgList.push({
                                        Value: wResult[k].ImagePath[m]
                                    })
                                }
                                _zaceTreeLsit._item[i].ImageSrc = $com.util.template(ImgList, Temp);
                            } else {
                                _zaceTreeLsit._item[i].ImageSrc = "";
                            }
                        }
                    }
                }
                if (Type == 1) {
                    $("#femi-Import-tbodyDetail").html($com.util.template(_zaceTreeLsit._item, HTML.InspectionItemsReadOnly) + $com.util.template(_zaceTreeLsit._group, HTML.InspectionGroup));
                } else {
                    if (Todotasks.ActivitiID == "Maintain4") {
                        $("#femi-Import-tbody").html($com.util.template(_zaceTreeLsit._item, HTML.InspectionItems) + $com.util.template(_zaceTreeLsit._group, HTML.InspectionGroup));
                    } else {
                        $("#femi-Import-tbody").html($com.util.template(_zaceTreeLsit._item, HTML.InspectionItemsReadOnly) + $com.util.template(_zaceTreeLsit._group, HTML.InspectionGroup));
                    }
                }
                if (slpkDateGroup.length > 0) {
                    for (var k = 0; k < slpkDateGroup.length; k++) {
                        var ItemList = [];
                        for (var m = 0; m < slpkDateGroup[k].ValueSource.length; m++) {
                            ItemList.push({
                                ID: m + 1,
                                Name: slpkDateGroup[k].ValueSource[m]
                            });
                        }
                        var selectName = $("#" + slpkDateGroup[k].IDName.slpkWorkValue);
                        model.com.ProcessingData(ItemList, selectName);

                    }
                }
                if (slpkDateGroup.length > 0) {
                    for (var k = 0; k < slpkDate.length; k++) {
                        var ItemList = [];
                        for (var m = 0; m < slpkDate[k].ValueSource.length; m++) {
                            ItemList.push({
                                ID: m + 1,
                                Name: slpkDate[k].ValueSource[m]
                            });
                        }
                        var selectName = $("#" + slpkDateGroup[k].IDName.slpkWorkValue);
                        model.com.ProcessingData(ItemList, selectName);
            ...
            buildTreePro: function (list, Type) {
                n = 0;
                var _Result = {
                    _group: [],
                    _item: [],
                ...
                $.each(list, function (i, item) {
                    if (item.GroupID == 0) {
                        if (item.ItemType == 4 && item.GroupID == 0) {
                            Num = Num + 1;
                            item.WID = Num;
                            _Result._group.push(item);
                            model.com.buildTreeProItem(item, list, item.WID, Type);
                        }
                        else {
                            Num = Num + 1;
                            item.WID = Num
                            _Result._item.push(item);
                        }
                    }
                });
            ...
            buildTreeProItem: function (_item, list, Num, Type) {
                ...
                for (var k = 0; k < list.length; k++) {
                    if (list[k].GroupID == _item.ID) {
                        if (list[k].ItemType == 4) {
                            NumItem = NumItem + 1;
                            list[k].WID = Num + "." + NumItem
                            _item.ItemIPTList_G.push(list[k]);
                            model.com.buildTreeProItem(list[k], list, list[k].WID, Type);
                        }
                        else {
                            NumItem = NumItem + 1;
                            list[k].WID = Num + "." + NumItem
                            _item.ItemIPTList.push(list[k]);
                        }
                    }
                }
                _item.ItemModuleList = '';
                for (var i = 0; i < _item.ItemIPTList.length; i++) {


                    _item.ItemIPTList[i].NumberText = model.com.CountStandard(_item.ItemIPTList[i], Number(_item.ItemIPTList[i].StandardType));
                    switch (_item.ItemIPTList[i].StandardType) {
                        case 0:
                            htmlTemp = HTML.ITEM.Text;
                            break;
                        case 1:
                            ...
                            var slpkList = [];
                            slpkList.push({
                                slpkWorkValue: "slpkWorkValue" + n
                            });
                            slpkDateGroup.push({
                                IDName: slpkList[0],
                                ValueSource: _item.ItemIPTList[i].ValueSource
                            })
                            htmlTemp = $com.util.template(slpkList, HTML.ITEM.Combo);
                            break;
                        case 2:
                    ...
                    _item.ItemIPTList[i].ITEMControl = $com.util.template(_item.ItemIPTList[i], htmlTemp);

                    for (var k = 0; k < wResult.length; k++) {
                        if (_item.ItemIPTList[i].ID == wResult[k].IPTItemID) {
                            _item.ItemIPTList[i].Value = wResult[k].Value;
                            _item.ItemIPTList[i].Remark = wResult[k].Remark;
                            _item.ItemIPTList[i].ResultText = CheckResultList[wResult[k].Result];
                            var ImgList = [];
                            var Temp = '<img class="ShowImg" src="{{Value}}" style="width: 30px;height: 30px;margin: 2px;"></img>';
                            if (wResult[k].ImagePath.length > 0) {
                                for (var m = 0; m < wResult[k].ImagePath.length; m++) {
                                ...
                                _item.ItemIPTList[i].ImageSrc = $com.util.template(ImgList, Temp);
                            } else {
                                _item.ItemIPTList[i].ImageSrc = "";
                            }
                        }
                    }
                    ...
                    if (Type == 1) {
                        _item.ItemModuleList += $com.util.template(_item.ItemIPTList[i], HTML.InspectionItemsReadOnly);
                    } else {
                        if (Todotasks.ActivitiID == "Maintain4") {
                            _item.ItemModuleList += $com.util.template(_item.ItemIPTList[i], HTML.InspectionItems);
                        } else {
                            _item.ItemModuleList += $com.util.template(_item.ItemIPTList[i], HTML.InspectionItemsReadOnly);
                        }
                ...
                _item.ItemModuleList += $com.util.template(_item.ItemIPTList_G, HTML.InspectionGroup);

            },
            BuinessDetail: function (wInfo, type) {
                if (type == 1) {
                    $(".headerAll .BasicTable .FieldContentOne .Contentshow").text("待发起量具校正流程");
                } else {

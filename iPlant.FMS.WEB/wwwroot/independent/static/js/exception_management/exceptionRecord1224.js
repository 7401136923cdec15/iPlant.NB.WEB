require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/base/timeLine'], function ($jq, $com, $timeLine) {
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            //异常任务模糊查询
            $("body").delegate("#cby-search-text-ledger", "input", function () {
                var $this = $(this),
                    value = $(this).val();
            ...
            $("body").delegate("#lmvt-apply-deal", "click", function () {
                ...
                model.com.getEXCCallApplyAll({ ID: SelectData[0].ApplyID }, function (res) {
                    if (!res)
                        return;
                    var list = res.info,
                        rst = [];
                    if (list) {
                        ...
                        cate.push({
                            Position: "申请",
                            People: model.com.GetUserName(res.info.ApplicantID),
                            Time: $com.util.format('yyyy-MM-dd hh:mm:ss', new Date(res.info.ApplicantTime))
                        });
                        cate.push({
                            Position: "审批",
                            People: model.com.GetUserName(res.info.ApproverID),
                            Time: $com.util.format('yyyy-MM-dd hh:mm:ss', new Date(res.info.ApproverTime))
                        });
                        cate.push({
                            Position: "确认",
                            People: model.com.GetUserName(res.info.ConfirmID),
                            Time: $com.util.format('yyyy-MM-dd hh:mm:ss', new Date(res.info.ConfirmTime))
                        });

                        $(".call-right-info").hide();
                        $(".lmvt-apply").show();
                        $("#lmvt-apply-tbody").html($com.util.template(cate, HTML.ApplyCost));
            ...
            $("body").delegate("#cby-task-tbody tr", "dblclick", function () {
                ...
                var WID = Number($this.find('td[data-title=OrderID]').attr('data-value'));

                $(".call-right-info-zace").show();
                $(".call-right-info").hide();
                $(".lmvt-apply").hide();
                var taskID = WID;
                ...
                model.com.getTree({
                    ...
                    //发起人                        
                    $("#personnel_faqi").html(model.com.getDepAndName(data.info.CallTask.ApplicantID));
                    $("#time_faqi").html($com.util.format("yyyy-MM-dd hh:mm", data.info.CallTask.CreateTime));

                    //确认人
                    $("#personnel_confirm").html(model.com.getDepAndName(data.info.CallTask.ConfirmID));



                    //处理人
                    $("#personnel_deal").html(model.com.getDepAndName(data.info.CallTask.OperatorID[0]));


                    if (data.info.CallTask.Status == 6) {
                        $("#status_confirm").html("已确认");
                        //处理
                        $("#mProcessTracking").hide();  //处理
                        // $("#status_deal").html("已处理");
                        // $("#mLeft_jgzj div").css("background-color","green");
                        // $("#mLeft_jgzj img").attr("src",'../static/images/checkbox/selected.png');

                        $("#time_deal").show();
                        var _list = data.info.CallDispatchList[data.info.CallDispatchList.length - 1].ActionList;
                        $("#time_deal").html($com.util.format("yyyy-MM-dd hh:mm", _list[_list.length - 2].CreateTime));

                        //确认
                        $("#mProcess_apply").hide();
                        // $("#mLeft_apply div").css("background-color","green");
                        // $("#mLeft_apply img").attr("src",'../static/images/checkbox/selected.png');

                        // $("#time_confirm").show();
                        // $("#time_confirm").html($com.util.format("yyyy-MM-dd hh:mm", data.info.CallTask.EditTime));

                    } else if (data.info.CallTask.Status == 4) {
                        $("#status_confirm").html("待确认");
                        //处理
                        $("#mProcessTracking").hide();  //处理
                        // $("#status_deal").html("已处理");
                        // $("#mLeft_jgzj div").css("background-color","green");
                        ...
                        // $("#time_deal").show();
                        // var _list=data.info.CallDispatchList[data.info.CallDispatchList.length-1].ActionList;
                        // $("#time_deal").html($com.util.format("yyyy-MM-dd hh:mm", _list[_list.length-1].CreateTime));

                        //确认
                        $("#mLeft_apply div").css("background-color", "gray");
                        $("#mLeft_apply img").attr("src", '../static/images/checkbox/unselected.png');
                        $("#time_confirm").hide();

                    } else if (data.info.CallTask.Status == 9) {
                        $("#mProcess_apply").hide();  //确认
                        $("#mProcessTracking").hide();  //处理
                        $("#mProcess_moudle").hide();   //详情
                        $("#status_faqi").html("已撤销");


                        $("#time_faqi").html($com.util.format("yyyy-MM-dd hh:mm", data.info.CallCancel.CreateTime));
                    }
                    else {
                        $("#mProcessTracking").show();  //处理
                        $("#mProcess_apply").show();


                        $("#status_confirm").html("待确认");
                        //处理
                        $("#status_deal").html("待处理");
                        $("#mLeft_jgzj div").css("background-color", "gray");
                        $("#mLeft_jgzj img").attr("src", '../static/images/checkbox/unselected.png');
                        $("#time_deal").hide();

                        //$("#time_deal").html($com.util.format("yyyy-MM-dd hh:mm", data.info.CallDispatchList[data.info.CallDispatchList.length-1].EditTime));
                    ...
                    //处理详情
                    model.com.render(data.info.CallDispatchList);
            ...
            $("body").delegate("#call-info-right-zace", "click", function () {
                ...
                var  _list=data.list;             
                    ...
                    //发起人                        
                    $("#personnel_faqi-second").html(model.com.getDepAndName(data.info.CallTask.ApplicantID));
                    $("#time_faqi-second").html($com.util.format("yyyy-MM-dd hh:mm", data.info.CallTask.CreateTime));

                    //确认人
                    $("#personnel_confirm-second").html(model.com.getDepAndName(data.info.CallTask.ConfirmID));



                    //处理人
                    $("#personnel_deal-second").html(model.com.getDepAndName(data.info.CallTask.OperatorID[0]));


                    if (data.info.CallTask.Status == 6) {
                        $("#status_confirm-second").html("已确认");
                        //处理
                        $("#mProcessTracking-second").hide();  //处理
                        // $("#status_deal").html("已处理");
                        // $("#mLeft_jgzj div").css("background-color","green");
                        // $("#mLeft_jgzj img").attr("src",'../static/images/checkbox/selected.png');

                        $("#time_deal-second").show();
                        var _list = data.info.CallDispatchList[data.info.CallDispatchList.length - 1].ActionList;
                        $("#time_deal-second").html($com.util.format("yyyy-MM-dd hh:mm", _list[_list.length - 2].CreateTime));

                        //确认
                        $("#mProcess_apply-second").hide();
                        // $("#mLeft_apply div").css("background-color","green");
                        // $("#mLeft_apply img").attr("src",'../static/images/checkbox/selected.png');
                        ...
                        $("#status_confirm-second").html("待确认");
                        //处理
                        $("#mProcessTracking-second").hide();  //处理
                        // $("#status_deal").html("已处理");
                        // $("#mLeft_jgzj div").css("background-color","green");
                        ...
                        //确认
                        $("#mLeft_apply-second div").css("background-color", "gray");
                        $("#mLeft_apply-second img").attr("src", '../static/images/checkbox/unselected.png');
                        $("#time_confirm-second").hide();

                    } else if (data.info.CallTask.Status == 9) {
                        $("#mProcess_apply-second").hide();  //确认
                        $("#mProcessTracking-second").hide();  //处理
                        $("#mProcess_moudle-second").hide();   //详情
                        $("#status_faqi-second").html("已撤销");


                        $("#time_faqi-second").html($com.util.format("yyyy-MM-dd hh:mm", data.info.CallCancel.CreateTime));
                    }
                    else {
                        $("#mProcessTracking-second").show();  //处理
                        $("#mProcess_apply-second").show();


                        $("#status_confirm-second").html("待确认");
                        //处理
                        $("#status_deal-second").html("待处理");
                        $("#mLeft_jgzj-second div").css("background-color", "gray");
                        $("#mLeft_jgzj-second img").attr("src", '../static/images/checkbox/unselected.png');
                        $("#time_deal-second").hide();
                    ...
                    //处理详情
                    model.com.renderSecond(data.info.CallDispatchList);
            ...
            //呼叫详情隐藏
            $("body").delegate("#cby-hidden-list-zace", "click", function () {
                $(".call-right-info-zace").hide();
            ...
            $("body").delegate(".image-show", "click", function () {
                ...
                $(".lmvt-change-photo").css("background", "url(" + imgSrc + ") ");
                $(".lmvt-change-photo").css("background-size", "100%  100%");
            ...
            $("body").delegate(".femi-tb-scroll table.table tbody tr", "click", function (e) {
                ...
                $table.find("tbody tr").each(function (i, item) {
                    var $tr = $(this);
                    if (checkboxID == $tr.find("td[data-title=OrderID]").attr("data-value"))
                        return true;
                    else {
                ...
                if (list.length >= 1) {
                    ...
                    if (list.length > 1) {
                            ...
                            if (!(list[index].Status == 5 && list[index].ActionList.length == 0)) {
                                ...
                                if (_obj.ActionList.length > 0) {
                                    if (_obj.ActionList.length >= 1) {
                                        for (var j = 0; j < _obj.ActionList.length; j++) {
                                            var _tempListdetail = [];
                                            ...
                                            $("#mProcess_moudle-second").append($com.util.template(_tempListdetail, HTML.moduleSecond));
                                        }
                                    }
                                }
                                ...
                                if (_obj.ActionList.length < 2) {
                                    //主流程
                                    $("#mProcess_moudle-second").append($com.util.template(_tempList, HTML.moduleSecond));
                                }
                        ...
                        if (list[list.length - 1].ActionList.length > 0) {
                            ...
                            for (var j = 0; j < _listLast.length; j++) {
                                ...
                                $("#mProcess_moudle-second").append($com.util.template(_tempListdetaillast, HTML.moduleSecond));
                            }
                        }
                    } else {
                        //处理详情
                        if (list[0].ActionList.length > 0) {
                            if (list[0].ActionList.length >= 1) {
                                for (var j = 0; j < list[0].ActionList.length; j++) {
                                    var _tempListdetail = [];

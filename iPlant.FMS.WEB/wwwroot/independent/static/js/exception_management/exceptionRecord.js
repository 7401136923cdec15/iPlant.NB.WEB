require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/base/timeLine'], function ($jq, $com, $timeLine) {
    ...
    // mStartTimeZace = $com.util.format('yyyy-MM-dd hh:mm:ss', new Date().getTime() - 1 * 30 * 24 * 3600 * 1000);
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            $("body").delegate("#call-zace-refresh", "click", function () {
                //重新查找接口数据
                model.com.getTask({ ExceptionType: mExceptionType, ApplyID: -1, StationType: -1, StationName: "", StationID: -1, RespondLevel: mRespondLevel, DisplayBoard: -1, OnSite: -1, Status: 0, ApplicantID: -1, OperatorID: -1, ConfirmID: -1, ShiftID: -1, StartTime: $com.util.format('yyyy-MM-dd', mStartimeZace) + ' 00:00:00', EndTime: $com.util.format('yyyy-MM-dd', new Date(mEndTimeZace)) + ' 23:59:59', OAGetType: -1 }, function (data) {
                    DataAll = data;
            ...
            $("body").delegate("#cby-search-ledger", "click", function () {
                var default_value = {
                    StartTime: $com.util.format('yyyy-MM-dd', new Date(mStartimeZace)),
                    EndTime: $com.util.format('yyyy-MM-dd', new Date(mEndTimeZace)),
                    RespondLevel: mRespondLevel,
                ...
                $com.modal.show(default_value, KEYWORD, "查询", function (rst) {
                    ...
                    mRespondLevel = Number(rst.RespondLevel);
                    mExceptionType = Number(rst.ExceptionType);

                    //重新查找接口数据
                    model.com.getTask({ ExceptionType: mExceptionType, ApplyID: -1, StationType: -1, StationName: "", StationID: -1, RespondLevel: mRespondLevel, DisplayBoard: -1, OnSite: -1, Status: 0, ApplicantID: -1, OperatorID: -1, ConfirmID: -1, ShiftID: -1, StartTime: $com.util.format('yyyy-MM-dd', rst.StartTime) + ' 00:00:00', EndTime: $com.util.format('yyyy-MM-dd', new Date(rst.EndTime)) + ' 23:59:59', OAGetType: -1 }, function (data) {
                        DataAll = data;
                        chooseTimeArr = data.list;
                        model.com.refreshData(DataAll);
            ...
            window.setFunctionTrigger("exceptionRecord", function (res) {
                mStartimeZace = $com.util.format('yyyy-MM-dd hh:mm:ss', new Date().getTime() - 1 * 30 * 24 * 3600 * 1000);
                mEndTimeZace = $com.util.format('yyyy-MM-dd hh:mm:ss', new Date());
                mRespondLevel = -1;
                mExceptionType = -1;
                ...
                if (res.StartT) {
                    mStartimeZace = $com.util.format('yyyy-MM-dd', new Date(res.StartT));
                }
    
                if (res.EndT) {
                    mEndTimeZace = $com.util.format('yyyy-MM-dd', new Date(res.EndT));
                }
    
                if (res.Respond) {
                    mRespondLevel = Number(res.Respond);
                }
                if (res.Exception) {
                    mExceptionType = Number(res.Exception);
                }
                model.com.Zaceloaded();
                // alert(res.ID);
                //刷新
            });
        ...
        run: function () {
            ...
            if (model.query.startT) {
                mStartimeZace = $com.util.format('yyyy-MM-dd', new Date(model.query.startT));
            }

            if (model.query.endT) {
                mEndTimeZace = $com.util.format('yyyy-MM-dd', new Date(model.query.endT));
            }

            if (model.query.respond) {
                mRespondLevel = Number(model.query.respond);
            }
            if (model.query.exception) {
                mExceptionType = Number(model.query.exception);
            }
            ...
            ZaceuserData = window.parent._UserAll;
        ...
        com: {
            ...
            refreshTable: function () {
                model.com.getTask({ ExceptionType: mExceptionType, ApplyID: -1, StationType: -1, StationName: "", StationID: -1, RespondLevel: mRespondLevel, DisplayBoard: -1, OnSite: -1, Status: 0, ApplicantID: -1, OperatorID: -1, ConfirmID: -1, ShiftID: -1, StartTime: $com.util.format('yyyy-MM-dd', new Date(mStartimeZace)) + ' 00:00:00', EndTime: $com.util.format('yyyy-MM-dd', new Date(mEndTimeZace)) + ' 23:59:59', OAGetType: -1 }, function (data) {
                    DataAll = data;

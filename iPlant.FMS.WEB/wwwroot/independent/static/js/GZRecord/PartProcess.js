require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/base/paging'], function ($zace, $com, $page) {
    ...
    HTML = {
        TableModeSegment: [
            ...
            '<td data-title="RecordType" data-value="{{RecordType}}" >{{RecordType}}</td>',
            '<td data-title="Operator" data-value="{{Operator}}" >{{Operator}}</td>',
            '<td data-title="Mutualer" data-value="{{Mutualer}}" >{{Mutualer}}</td>',
            '<td data-title="Speciler" data-value="{{Speciler}}" >{{Speciler}}</td>',
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            $("body").delegate("#zace-active-ISQualityFalse", "click", function () {
            ...
            $("body").delegate("#zace-active-ISQuality", "click", function () {
                mIsQuality = true;
                model.com.refresh();
        ...
        com: {
            ...
            refresh: function () {
                ...
                if (mIsQuality) {
                    $('.zace-titleText').text(''+'质量过程控制记录');
                } else {
                    $('.zace-titleText').text(''+'生产过程控制记录');
                }
                // 申请
                model.com.getProcessRecord({ OrderID: mOrderID, StartTime: mStartTime+' 00:00:00', EndTime: mEndTime+' 23:59:59', StepID: mStepID, RecordType: mRecordType, SubmitID: mSubmitID, StationID: mStationID,IsQuality: mIsQuality }, function (resP) {
                    if (!resP)
                        return;
                    if (resP && resP.list) {
            ...
            getProcessRecord: function (data, fn, context) {
                var d = {
                    $URI: "/IPTOrderReport/ProcessRecord",
                    $TYPE: "get",
                    $SERVER: '/MESQMS'
                };

require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/base/paging'], function ($zace, $com, $page) {
    ...
    model = $com.Model.create({
        ...
        events: function () {
            window.setFunctionTrigger("PartProcessO", function (res) {
        ...
        com: {
            ...
            refresh: function () {
                ...
                if (mIsQuality) {
                    $('.zace-titleText').text('【' + mPartNo + '】' + '质量过程控制记录');
                } else {
                    $('.zace-titleText').text('【' + mPartNo + '】' + '生产过程控制记录');
                }

                // 申请
                model.com.getProcessRecord({ OrderID: mOrderID, IsQuality: mIsQuality }, function (resP) {
                    if (!resP)
                        return;
                    if (resP && resP.list) {

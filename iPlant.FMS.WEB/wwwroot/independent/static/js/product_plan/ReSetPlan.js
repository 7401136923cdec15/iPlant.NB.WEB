require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/ganttGZWUtil'], function ($zace, $com, $ganttWeek) {
    ...
    HTML = {
        ...
        TimeTableBody: [
            '<td style=\"width:125px\" data-title="PartName" data-value="{{PartName}}">{{PartName}}</td>',
            '<td style=\"width:125px\" data-title="OrderNo" data-value="{{OrderNo}}">{{OrderNo}}</td>',
        ].join(""),
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            // });
            //  跳转计划提交界面
            $("body").delegate("#zace-export-submit", "click", function () {

                var Selection_Data = VersionSource.filter((item) => { return item.VersionNo == wVersionNo }),
                    WorkShopID = Selection_Data[0].WorkShopID;

                var ISMove = true;
                if ((Selection_Data[0].Status != 1 || Selection_Data[0].Status != 0) || new Date().getTime() >= new Date(Selection_Data[0].EndTime).getTime()) {
                    ISMove = false;
                    // alert("该版本禁止提交计划！");
                    // return false;
                }

                var vdata = { 'header': '周计划详情', 'href': './product_plan/ReSetPlanBPM.html?ID=' + Selection_Data[0].ID + "&VersionNo=" + Selection_Data[0].VersionNo + "&WorkShopID=" + Selection_Data[0].WorkShopID + "&ISMove=" + ISMove, 'id': 'ReSetPlanBPM', 'src': './static/images/menu/newfactoryModel/techniquePart.png' };
        ...
        run: function () {
            ...
            ISMove = model.query.ISMove == "false" ? false : true;
        ...
        com: {
                ...
                if (data.length > 0) {
                    ...
                    for (const key in data[0]) {
                        ...
                        if (key.startsWith("DT_")) {
                            ...
                            BodyStr += "<td style=\"min-width:50px\" data-title=" + titleDate + " data-value=" + "{{UniqueID}}>" + "<input style=\"border:none\" class=\"lmvt-input\" value=\"{{" + key + "}}\">" + "</td>";
                        }
                    }
            ...
            ChangeShiftDetailItemCol: function (Data, DTEnd, ChangeDate, CZFQTY, UniqueID, Source, model_type) {
                ...
                if (item[$com.util.format("DT_yyyy_MM_dd", ChangeDate)] + _CZFQTY < 0) {
                    ...
                    var _ChangeDate = ChangeDate;
                    if (item.APSNext > 0) {
                        _ChangeDate = new Date(_ChangeDate.getTime() + (60 * 60 * 24 * 1000 * item.APSNext));
                    }
                    $.each(_List, function (j, j_item) {

require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/base/jquery.treeview'], function ($zace, $com, $tree) {
    ...
    SelectedLine = 0;
    mShiftID = 20200101;
    var Status = ['black', 'black', '#fa1ff4c9', 'blue', '#a94442', 'green', 'red', '#f8391bc9', 'blue', '#e6c685', 'orange'];//'#00CCFF' 完工
    ...
    var DataAllRecord = [];
    var mZaceDateTime=$com.util.format('yyyy-MM-dd', new Date());
    var mPartTableAll = [];
    ...
    HTML = {
        ...
        TablePartModePl: [
            '<tr data-id="{{ID}}">',
            // '<td style="display:none"><input type="checkbox"  data-id="{{ID}}"   class="femi-tb-checkbox" style="margin: 1px 0px 1px" /></td>',  //禁止单选
            //'<td data-title="WID" data-value="{{WID}}" >{{WID}}</td>',
            // '<td  data-title="WID" data-value="{{WID}}" >{{WID}}</td>',
            '<td style="min-width: 50px;display:none" data-title="ID" data-value="{{ID}}" >{{ID}}</td> ',
        ...
        TablePartModePAudit: [
            ...
            '<td style="background:{{ColorText}}"  data-title="UnPlanSize" data-value="{{UnPlanSize}}" >{{UnPlanSize}}</td>',
            '<td data-title="StepFinish" data-value="{{StepFinish}}" >{{StepFinish}}</td>',
            '<td style="" data-title="FinishProgress" data-value="{{FinishProgress}}"><progress max="100" value={{FinishProgress}} style="width: 100px ;height: 15px;"><progress></td>',
            '<td data-title="StepSchedule" data-value="{{StepSchedule}}" >{{StepSchedule}}</td>',
            '<td style="" data-title="ScheduleProgress" data-value="{{ScheduleProgress}}"><progress max="100" value={{ScheduleProgress}} style="width: 100px ;height: 15px;"><progress></td>',
        ...
        TablePartMode: [
            ...
            '<td data-title="PartID" data-value="{{PartID}}" >{{PartID}}</td>',
            '<td data-title="StepSize" data-value="{{StepSize}}" >{{StepSize}}</td>',
            '<td style="background:{{ColorText}}" data-title="PlanSize" data-value="{{PlanSize}}" >{{PlanSize}}</td>',
            '<td data-title="StepMaking" data-value="{{StepMaking}}" >{{StepMaking}}</td>',
            '<td data-title="StepFinish" data-value="{{StepFinish}}" >{{StepFinish}}</td>',
            '<td style="" data-title="FinishProgress" data-value="{{FinishProgress}}"><progress max="100" value={{FinishProgress}} style="width: 100px ;height: 15px;"><progress></td>',
            '<td data-title="StepSchedule" data-value="{{StepSchedule}}" >{{StepSchedule}}</td>',
    ...
    model = $com.Model.create({
        ...
        events: function () {
            $("body").delegate("#zace-searchZApproval-send", "click", function () {
                var vdata = { 'header': '日计划记录', 'href': './product_plan/LOCOHistoryDayPlan.html?id=' + 1 + '&name=' + '' + '&PartNo=' + '', 'id': 'LOCOHistoryDayPlan', 'src': './static/images/menu/newfactoryModel/techniquePartpoint.png' };
                window.parent.iframeHeaderSet(vdata);
                window.callFunctionTrigger("LOCOHistoryDayPlan", { ID: 2, Name: '', PartNo: '' });
            ...
            $("body").delegate("#zace-searchZApproval-refreshCommiitDone", "click", function () {
                $('.zzzPlanSearch').hide();
                $('.zzzbMakePlan').hide();
                $('.zzzb').hide();
                $('.zzza').hide();
            ...
            $("body").delegate("#zace-searchZApproval-level-SearchAuditDone", "click", function () {

                var value = $("#zace-search-levelPartAuditDone").val();
                if (value == undefined || value == "" || value.trim().length < 1)
                    $("#femi-riskLevelPartAuditDone-tbody").children("tr").show();
                else
                    $com.table.filterByLikeString($("#femi-riskLevelPartAuditDone-tbody"), DataAllSearchAudit, value, "ID");
                    ...
                    if (item.Checked && item.Checked == "checked" && item.Status == 1) {
                        ...
                        if (item.Status == 1) {
                            ...
                            if (item.Remark.Remark.match(/^[ ]*$/)) {
                                ...
                                _text = item.PartNo + '--' + FORMATTRT_Level['PartID'](item.PartID) + '--' + FORMATTRT_Level['StepID'](item.StepID);
                                boolSubmit = true;
                                return false;
            ...
            $("body").delegate("#femi-riskLevelPartAuditDone-tbody tr", "dblclick", function () {
            ...
            $("body").delegate("#zace-ShowText", "click", function () {
                $('.zzzPlanSearch').hide();
                $('.zzzbMakePlan').hide();
                $('.zzzb').show();
                $('.zzza').hide();
                ...
                $(".zzzb").css("width", "600px");
                $(".zzzaAudit").css("margin-right", "600px");
                model.com.refreshPartZace(_partList);//渲染工序
            });
            ...
            $("body").delegate("#zace-makePlan-levelZace", "click", function () {
                ...
                $(".zzzbMakePlan").css("width", "0px");
                $(".zzzaMake").css("margin-right", "0px");
                mZaceDateTime=$com.util.format('yyyy-MM-dd', new Date());
                model.com.refreshPo();
            ...
            $("body").delegate("#zace-searchAudit-Today", "click", function () {
                ModeType = 2           
                mZaceDateTime=$com.util.format('yyyy-MM-dd', new Date());
                $('.zzzPlanSearch').hide();
                $('.zzzbMakePlan').hide();
            ...
            $("body").delegate("#zace-searchAudit-Tommorrow", "click", function () {
                ModeType = 2           
                mZaceDateTime=$com.util.format('yyyy-MM-dd', new Date().getTime()+24*3600*1000);
                $('.zzzPlanSearch').hide();
                $('.zzzbMakePlan').hide();
                $('.zzzb').hide();
                $('.zzza').hide();
            ...
            $("body").delegate("#zace-edit-levelDone", "click", function () {
            ...
            //审批
            $("body").delegate("#zace-searchZApproval-levelZaceClose", "click", function () {
            ...
            //制定计划
            $("body").delegate("#zace-searchZApproval-MakePlanClose", "click", function () {
                $('.zzzPlanSearch').hide();
                $('.zzzbMakePlan').hide();
                $('.zzzb').hide();
                $('.zzza').hide();
            ...
            //查询
            $("body").delegate("#zace-searchZApproval-PlanClose", "click", function () {
        ...
        com: {
            ...
            refreshPartPointAudit: function (id, orderID, list) {
                ...
                for (var m = 0; m < list.length; m++) {
                    ...
                    if (id == list[m].TaskPartID) {
                        if (mTypeDone==1) {
                            if (list[m].Status!=2&&list[m].Status!=10&&list[m].Status!=4&&list[m].Status!=5) {
                                mPartPointTableAll.push(list[m]);
                            }
                    ...
                    if (resItem && resItem.list) {
                        ...
                        if (resItem.list.length > 0) {
                            ...
                            $.each(mPartPointTableAll, function (i, item) {
                                ...
                                item.StartTime = item.APSTaskPart.StartTime;
            ...
            refreshPoAudit: function () {
                ...
                var _dateText = $com.util.format('yyyy-MM-dd', new Date());
                ...
                model.com.getAPSTaskStepQuery({ IsAudit: true }, function (resItem) {//ShfitID: _checkShiftID,
                    ...
                    if (resItem && resItem.list) {
                        ...
                        if (resItem.list.length > 0) {
                            ...
                            OrderList = $com.util.Clone(resItem.OrderList);
                            if (mTypeDone == 1) {//待审
                                TaskPartList = $com.util.Clone(resItem.ToDoList);
                                _TaskPartList = $com.util.Clone(resItem.ToDoList);
                            } else if (mTypeDone == 2) {
                                TaskPartList = $com.util.Clone(resItem.DoneList);
                                _TaskPartList = $com.util.Clone(resItem.DoneList);
                            }
                            ...
                            var _UnDoneNum = 0;
                            $.each(_partList, function (j, item_j) {

                                item_j.Checked = 'checked';
                                if (item_j.Status == 1) {
                                    _UnDoneNum += 1;
                                }
                            ...
                            $.each(mPartPointTableAllAudit, function (i, item) {
                                ...
                                item.UnPlanSize = _num;//未排数
                                if (item.UnPlanSize > 0) {
                                    item.ColorText = '#e48e8e';
                                }
                            ...
                            if (mTypeDone == 1) {
                                ...
                                $("#femi-riskLevelPartAuditDone-tbody").html($com.util.template(Grade, HTML.TablePartModePAudit));
                            }
                            ...
                            $("#femi-riskLevelPartAuditDone-tbody").html($com.util.template([], HTML.TablePartModePAudit));
                            alert('暂无审批任务！');
                            $com.app.loaded();
                        }
            ...
            refreshPo: function () {
                ...
                $com.app.loading('数据加载中...');
                var _dateText = $com.util.format('yyyy-MM-dd', new Date(mZaceDateTime));
                $('.partText').text('(' + _dateText + ')' + '工位制定日计划');
                var _checkShift = 0;

                _checkShift = model.com.getShiftID(mZaceDateTime);

                mShiftID = _checkShift;
                model.com.postAPSTaskStepCreate({ data: mZaceDateTime }, function (resItem) {
            ...
            refreshPoCheck: function () {
                ...
                var _dateText = $com.util.format('yyyy-MM-dd', new Date(ShiftStartDate));
                if (!SelfShiftID)
                    SelfShiftID = model.com.getShiftID(new Date());

                var _checkShiftID = 0;
                _checkShiftID = model.com.getShiftID(ShiftStartDate);

                mShiftID = _checkShiftID;
                if ((mShiftID - SelfShiftID) > 1) {
                    alert("明日无周计划");
                    return false;
                    $com.app.loaded();
                }
            ...
            getNewPartList: function (ListPart, LsitPoint) {
                ...
                $.each(ListPart, function (i, item) {
                    ...
                    $.each(LsitPoint, function (k, item_k) {
                        ...
                        item_k.UnitItemList = [];
                        if (item.APSTaskPart.ID == item_k.TaskPartID) {
                            if (item_k.Status == 1) {
                                item.PlanSize += 1;
                            }

require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base'], function ($alfie, $com) {
    ...
    mHTML = {
        TableNode_item: [
            ...
            '<td style="min-width: 50px" data-title="StepName" data-value="{{StepName}}">{{StepName}}</td>',
            '<td style="min-width: 50px" data-title="Operators" data-value="{{Operators}}">{{Operators}}</td>',
            '<td style="min-width: 50px" data-title="StartTimeText" data-value="{{StartTimeText}}">{{StartTimeText}}</td>',
            '<td style="min-width: 50px" data-title="EndTimeText" data-value="{{EndTimeText}}">{{EndTimeText}}</td>',
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            window.setFunctionTrigger("LOCOOrderTimeRoute", function (res) {
        ...
        com: {
            ...
            refresh: function () {
                ...
                $('.zacce-partTime').text('【'+mPartNo+'】生产概况');
                model.com.getPartNoList({ OrderID:mOrderID}, function (res) {
                    if (res && res.list) {
                        ...
                        $.each(wItem, function (i, item) {
                            ...
                            if (item.StartTime<'2010-01-01') {
                                item.StartTimeText='-';
                            }
                            if (item.EndTime<'2010-01-01') {
                                item.EndTimeText='-';
                            }
            ...
            //获取列表
            getPartNoList: function (data, fn, context) {
                var d = {
                    $URI: "/SFCOrderForm/SequentialInfo",
                    $TYPE: "Get",
                    $SERVER: "/MESAPS"
                };

require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base'], function ($zace, $com) {
    ...
    mZaWBSNo = '';
    mZaLineID = 0;
    mZaStartTime = $com.util.format('yyyy-MM-dd hh:mm:ss', new Date().getTime() - 15 * 24 * 3600 * 1000);
    mZaEndTime = $com.util.format('yyyy-MM-dd hh:mm:ss', new Date().getTime() + 15 * 24 * 3600 * 1000);
    ...
    (function () {
        KEYWORD_Level_LIST = [
            ...
            "zBureauSectionID|配属局段|ArrayOne",
            "zLineID|修程|ArrayOne",
            "zProductID|车型|ArrayOne",
            "Status|状态|ArrayOne",
            "mZaStartTime|开始时间|Date",
            "mZaEditTime|结束时间|Date",
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            $("body").delegate("#zace-edit-levelSearch", "click", function () {

                var default_value = {
                    Status:mStatus,
                    mZaWBSNo: mZaWBSNo,
                    zBureauSectionID: mZaCustomerID,
                    zLineID: mZaLineID,
                    zProductID: mZaProductID,
                    mZaStartTime: $com.util.format('yyyy-MM-dd', new Date(mZaStartTime)),
                    mZaEditTime: $com.util.format('yyyy-MM-dd', new Date(mZaEndTime)),
                };
                $("body").append($com.modal.show(default_value, KEYWORD_Level, "查询", function (rst) {
                    ...
                    mZaProductID = Number(rst.zProductID);
                    mZaCustomerID = Number(rst.zBureauSectionID);
                    mZaWBSNo = rst.mZaWBSNo;
                    mZaLineID = Number(rst.zLineID);
                    mZaStartTime = $com.util.format('yyyy-MM-dd ', new Date(rst.mZaStartTime));
                    mZaEndTime = $com.util.format('yyyy-MM-dd ', new Date(rst.mZaEditTime));
                    mStatus=Number(rst.Status);
                    model.com.refreshItem();
            ...
            $("body").delegate("#femi-riskLevelOrder-tbody tr", "dblclick", function () {
                ...
                for (var i = 0; i < DataAllOrder.length; i++) {
                    ...
                    if (DataAllOrder[i].ID == CommandID) {
                        ...
                        mCustomerID = DataAllOrder[i].CustomerID;  //配属局段
            ...
            $("body").delegate("#zace-active-levelRoute", "click", function () {
                ...
                model.com.getFPCRoute({ LineID: SelectData[0].LineID, ProductID: SelectData[0].ProductID, CustomerID: SelectData[0].CustomerID }, function (resP) {
                    if (!resP)
                        return;
                    if (TypeSource_Level.RouteID.length > 1)
                        TypeSource_Level.RouteID = [];
            ...
            $("body").delegate("#input-file", "input", function () {
                ...
                model.com.postImportExcel(form, function (res) {
                    ...
                    if (list) {
                        ...
                        var DataParams = $com.table.postExportParams(postData, $(".table-import>table"));
                        var arr1 = [];
                        var arr1List = [];
                        ...
                        var ProductBool = false;
                        var LineBool = false;
                        var CusBool = false;

                        var _zaceList = [];
                        $.each(arr1List, function (i, item) {

                            var _mode = $com.util.Clone(PositionTemp);
                            _mode.PartNo = item.ProductID+'#'+item.PartNoText;
                            for (var i = 0; i < ZProductList.length; i++) {

                                if (item.ProductID == ZProductList[i].ProductName) {
                                    item.ProductID = ZProductList[i].ID;
                                    ProductBool = true;
                            ...
                            for (var i = 0; i < ZLinelist.length; i++) {
                                if (item.LineID == ZLinelist[i].Name) {
                                    item.LineID = ZLinelist[i].ID;
                                    LineBool = true;
                                }
                            }
                            ...
                            for (var i = 0; i < ZTypeIDlist.length; i++) {
                                if (item.BureauSectionID == ZTypeIDlist[i].CustomerName) {
                                    item.BureauSectionID = ZTypeIDlist[i].ID;
                                    CusBool = true;
                                }
                            }

                            _mode.ProductID = item.ProductID;
                            _mode.BureauSectionID = item.BureauSectionID;
                            _mode.LineID = item.LineID;
                            _mode.Active = 0;
                            _mode.Priority = item.Priority;
                            _mode.Remark = item.Remark;
                            _mode.WBSNo = item.WBSNo;
                          
                            _mode.PlanFinishDate = $com.util.format('yyyy-MM-dd hh:mm:ss', new Date(item.PlanFinishDate));
                            _mode.PlanReceiveDate = $com.util.format('yyyy-MM-dd hh:mm:ss', new Date(item.PlanReceiveDate));


                            _zaceList.push(_mode);
                        ...
                        if (_zaceList.length<1) {
                            alert('数据为空！');
                            return false;
                        }
                        model.com.importZaOMSOrder({
                            data: _zaceList,
            ...
            $("body").delegate("#zace-active-level", "click", function () {
                ...
                // model.com.getOrderInfo({
                //     ID: SelectData[0].CommandID,
                // }, function (resCommand) {
        ...
        com: {
            ...
            loadZace: function () {
                ...
                model.com.getFMCLine({ FactoryID: 0, BusinessUnitID: 0, WorkShopID: 0 }, function (resP) {
                    ...
                    ZLinelist = $com.util.Clone(resP.list);
                    ...
                    model.com.getCustomer({ active: 2 }, function (resP) {
                        if (!resP)
                            return;
                        ZTypeIDlist = $com.util.Clone(resP.list);
                        $.each(resP.list, function (i, item) {
                            TypeSource_Level.BureauSectionID.push({
                                value: item.ID,
                                name: item.CustomerName
                            });
                            TypeSource_Level.zBureauSectionID.push({
                                value: item.ID,
                                name: item.CustomerName
                            });
                        });
                        ...
                        model.com.getFPCProduct({ ProductTypeID: 0, BusinessUnitID: 0 }, function (resP) {
                            if (!resP)
                                return;
                            ZProductList = $com.util.Clone(resP.list);
                            $.each(resP.list, function (i, item) {
                                ...
                                if (item.Active == 1&&item.TransportType==1) {
                                    ...
                                    TypeSource_Level.zProductID.push({
                                        value: item.ID,
                                        name: item.ProductName
                                    });
                                }
                            ...
                            model.com.get({ active: 1 }, function (resP) {
                                ...
                                model.com.getFPCRoute({ }, function (resP) {
                                    if (!resP)
                                        return;
                                    if (TypeSource_Level.RouteID.length > 1)
                                        TypeSource_Level.RouteID = [];
            ...
            getOrderInfo: function (data, fn, context) {
                var d = {
                    $URI: "/OMSOrder/Info",
                    $TYPE: "get",
                    $SERVER: '/MESAPS'
            ...
            refreshItem: function () {
                $com.app.loading('数据加载中！！');
                model.com.getOrderAll({ ProductID: mZaProductID, Line: mZaLineID, CustomerID: mZaCustomerID, WBSNo: mZaWBSNo, StartTime: mZaStartTime, EndTime: mZaEndTime,Status:mStatus }, function (resP) {
                    if (!resP)
                        return;
                    if (resP && resP.list) {
                        var Grade = $com.util.Clone(resP.list);
                        ...
                        $.each(Grade, function (i, item) {
                            ...
                            item.PartNoText=item.PartNo;
                            // item.PartNoText=item.PartNo.split("#")[1].length>0?item.PartNo.split("#")[1]:'';
            ...
            importZaOMSOrder:function (data, fn, context) {
                var d = {
                    $URI: "/OMSOrder/SaveList",
                    $TYPE: "post",
                    $SERVER: "/MESAPS"
                };

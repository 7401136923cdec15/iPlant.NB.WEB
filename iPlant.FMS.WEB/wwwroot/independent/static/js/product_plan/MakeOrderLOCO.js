require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/ganttGZWUtil'], function ($zace, $com, $ganttWeek) {
    ...
    HTML = {
        ...
        Progress: [
            '<div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="{{FQTYActual}}" aria-valuemin="0" aria-valuemax="{{FQTYPlan}}" style = "width: {{ProgressNow}}%;color: black;">{{FQTYActual}}/{{FQTYPlan}}',
            '</div>'
        ].join(""),
        ...
        TimeTableBody: [
            '<td style=\"min-width:125px\" data-title="OrderNo" data-value="{{OrderNo}}">{{OrderNo}}</td>',
            '<td style=\"min-width:125px\" data-title="PartName" data-value="{{PartName}}">{{PartName}}</td>',
        ].join(""),
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            $("body").delegate("#zace-export-saved", "click", function () {
                ...
                $com.util.deleteLowerProperty(TableGanteSource);
                ...
                model.com.postOMSOrderSaved({
                    ...
                    $("#zace-export-gantt").trigger("click");
                    var ISMove = true;
                    var vdata = { 'header': '周计划详情', 'href': './product_plan/ReSetPlan.html?ID=' + res.info.ID + "&VersionNo=" + res.info.VersionNo + "&WorkShopID=" + res.info.WorkShopID + "&ISMove=" + ISMove, 'id': 'ReSetPlan', 'src': './static/images/menu/newfactoryModel/techniquePart.png' };
                    window.parent.iframeHeaderSet(vdata);
                    window.callFunctionTrigger("RanderGantt", { ID: res.info.ID, VersionNo: res.info.VersionNo, WorkShopID: res.info.WorkShopID, ISMove: ISMove });
            ...
            $("body").delegate("#zace-aotu-orderPriority", "click", function () {
                ...
                model.com.postOMSOrderPriority({
                    ...
                    $("#femi-orderPriority-tbody tr").each(function (i, item) {
                        ...
                        var $item = $(item),
                            tID = Number($item.find("td[data-title=ID]").attr("data-value"));

                        var SelectData = mSelDataOrderList.filter((item) => { return item.ID == tID })[0];
            ...
            $("body").delegate(".lmvt-input", "change", function () {
                ...
                $.each(TableGanteSource, function (i, item) {
                    if (item.UniqueID == UniqueID) {
                        var Counts = 0;
                        var _self = 0;
                        $.each(item.ShiftDetail, function (j, jtem) {
                            if ($com.util.toDate(jtem.WorkDate).getTime() == $com.util.toDate(ChangeDate).getTime())
                                _self = jtem.PlanNum;
                ...
                var data = model.com.ChangeShiftDetail(TableGanteSource, mStartTime, wMaxTime, UniqueID, ChangeDate, fqty, ModelType);

                model.com.ChangeGantteSource(TableGanteSource, data);
                            ...
                            $.each(TableGanteSource, function (n, ntem) {
                                if (ntem.UniqueID == mtem.UniqueID) {
                                    if (ntem.ShiftDetail && ntem.ShiftDetail.length > 0) {
                                        ...
                                        mtem.ShiftDetail = [];
                                        $.each(ntem.ShiftDetail, function (s, stem) {
                                            if (stem.PlanNum > 0) {
                                                stem.WorkDate = $com.util.format("yyyy-MM-dd hh:mm:ss", stem.WorkDate);
                                                mtem.ShiftDetail.push(stem);
                                            }
                                        });

                                        mtem.ShiftDetail = ntem.ShiftDetail;
            ...
            $("body").delegate("#zace-aotu-orderPlan", "click", function () {
                ...
                dayZace = Number($('.zace-selectDay input').val());
            ...
            window.setFunctionTrigger("MakeOrderLOCO", function (res) {
            ...
            //模组排程
            $("body").delegate("#lmvt-planHistory", "click", function () {

                var vdata = { 'header': '排程记录', 'href': './product_plan/LOCOHistoryPlanWeek.html?WorkShopID=' + wWorkShopID, 'id': 'LOCOHistoryPlanWeek', 'src': '/MESCore/upload/web/月计划.svg' };
                window.parent.iframeHeaderSet(vdata);
                window.callFunctionTrigger("LOCOHistoryPlanWeek", { WorkShopID: wWorkShopID });
            ...
            $("body").delegate("#lmvt-changeType-order", "click", function () {
                ...
                wOrderFirst = 1;
                $("#zace-aotu-orderPlan").trigger("click");
            ...
            model.com.getFPCProduct({ ProductTypeID: 0, BusinessUnitID: 0 }, function (resW) {
                if (resW && resW.list) {
                    ...
                    for (var i = 0; i < DataProductItem.length; i++) {
                        ...
                        DataProductItem[i].Name = DataProductItem[i].ProductNo;
                    ...
                    model.com.getCustomer({ active: 1 }, function (resS) {
                        ...
                        model.com.getFMCLine({ FactoryID: 0, BusinessUnitID: 0, WorkShopID: 0 }, function (resW) {
                            ...
                            model.com.getFMCWorkDay({}, function (resW) {
                                ...
                                mEndTimeNew = model.com.addDays($com.util.format('yyyy-MM-dd', new Date()), 30);
        ...
        com: {
            ...
            //表格改变甘特
            BackGantt: function (source, data) {
                ...
                $.each(source, function (j, jtem) {
                    $.each(jtem.TaskPartList, function (k, ktem) {
                        ...
                        $.each(ktem.TaskPartList, function (m, mtem) {
                            ...
                            $.each(source, function (n, ntem) {
                                if (mtem.UniqueID == ntem.UniqueID) {

                                    if (mtem.ShiftDetail.length > 0) {

                                        var arr = model.com.ChangeTableSource(mtem, ntem);
            ...
            //表格
            ChangeGantteSource: function (ganttSource, tableSource) {

                $.each(ganttSource, function (i, item) {
                    var array = [];

                    $.each(tableSource, function (j, jtem) {
                        if (item.UniqueID == jtem.UniqueID) {
                            for (const key in jtem) {
                                if (key.startsWith("DT_")) {
                                    var _date = key.substr(3);
                                    _date = _date.replace(/_/g, "-");

                                    var tep = $com.util.Clone(item.ShiftDetail[0]);

                                    tep.WorkDate = $com.util.format("yyyy-MM-dd hh:mm:ss", _date)
                                    tep.PlanNum = jtem[key];
                ...
                if (data.length > 0) {
                    ...
                    for (const key in data[0]) {
                        ...
                        if (key.startsWith("DT_")) {
                            ...
                            BodyStr += "<td style=\"min-width:50px\" data-title=" + titleDate + " data-value=" + "{{UniqueID}}>" + "<input style=\"border:none\" class=\"lmvt-input\" type=\"number\" value=\"{{" + key + "}}\">" + "</td>";
            ...
            getShiftDetailTable: function (Data_L, DTStart, DTEnd) {

                var Data = $com.util.Clone(Data_L);
                ...
                for (var index = 0; index < Data.length; index++) {
                    ...
                    wTaskPart.ShiftDetail.sort(function (o1, o2) {
                        return new Date(o1.WorkDate).getTime() - new Date(o2.WorkDate).getTime();
                    })

                    wTaskPart.StartTime = wTaskPart.ShiftDetail
                    ...
                    for (var j = 0; j < wTaskPart.ShiftDetail.length; j++) {
                        ...
                        if (j == 0) {
                            _templateData.StartTime = wShift.WorkDate;
                        }
                        if (j == wTaskPart.ShiftDetail.length - 1) {
                            _templateData.EndTime = wShift.WorkDate;
                        }
            ...
            ChangeShiftDetail: function (Data, StartTime, EndTime, UniqueID, ChangeDate, fqty, model_type) {
                ...
                var item = undefined;
                $.each(wResult, function (i, i_item) {
                    if (i_item.UniqueID != UniqueID) {
                        return true;
                    }
                    item = i_item;
                    return false;
                ...
                if (item.APSNext > 0) {
                    _ChangeDate = new Date(new Date(_ChangeDate).getTime() + (60 * 60 * 24 * 1000 * item.APSNext));
                }

                $.each(_List, function (j, j_item) {
                    model.com.ChangeShiftDetailItemCol(wResult, DTEnd, _ChangeDate, _CZFQTY, j_item, Data, model_type);
                });

                switch (model_type) {
                    case 1:
                        //1 顺延
                        model.com.changeSoruceDetailDate(item);
                        var _YXDate = new Date(ChangeDate);
                        if (_YXDate.getTime() < DTEnd.getTime()) {
                            _YXDate = new Date(_YXDate.getTime() + (60 * 60 * 24 * 1000));
                        }
                        model.com.ChangeShiftDetailItemRow(wResult, DTEnd, _YXDate, -_CZFQTY, UniqueID, Data, model_type);
                        model.com.changeSoruceDetailDate(item);
                        break;
                    case 2:
                        //2 推迟时间
                        ...
                        if (_CZFQTY > 0) {
                            var _YXDate = new Date($com.util.format("yyyy/MM/dd", item.EndTime)).getTime();
                        }
                        if (_YXDate > DTEnd) {
                            _YXDate = DTEnd;
                        }
            ...
            changeSoruceDetailDate: function (data) {

                var wStartTime = undefined;
                var wEndTime = undefined;
                for (var key in data) {
                    if (!key.startsWith("DT_")) {
                        continue;
                    }
                    ...
                    if (wStartTime == undefined)
                        wStartTime = _date;
                    if (wEndTime == undefined)
                        wEndTime = _date;

                    if (_date.getTime() <= wStartTime.getTime())
                        wStartTime = _date;
                    if (_date.getTime() >= wEndTime.getTime())
                        wEndTime = _date;

                }
                if (wStartTime != undefined)
                    data.StartTime = wStartTime;
                if (wEndTime != undefined)
                    data.EndTime = wEndTime;
            },
            ...
            // 修改的当前日期
            // 
            ChangeShiftDetailItemCol: function (Data, DTEnd, ChangeDate, CZFQTY, UniqueID, Source, model_type) {
                ...
                if (item[$com.util.format("DT_yyyy_MM_dd", ChangeDate)] + _CZFQTY < 0) {
                    ...
                    while (_CZFQTY < 0) {
                        ...
                        if (_CZFQTY_1 < 0) {
                            ...
                            // item[$com.util.format("DT_yyyy_MM_dd", _StartT)] = 0;
                        } else {
                            //变化值直接为其变化值
                            _CZFQTY_1 = _CZFQTY;
                        }
                        ...
                        if (_CZFQTY_1 != 0) {
                            model.com.ChangeShiftDetailItemCol(Data, DTEnd, _StartT, _CZFQTY_1, UniqueID, Source, model_type);
                        }

                        _StartT = new Date(new Date(_StartT).getTime() + (60 * 60 * 24 * 1000));
                        _CZFQTY -= _CZFQTY_1;
                        if (_StartT > DTEnd)
                            break;
                    ...
                    $.each(_List, function (j, j_item) {
                        model.com.ChangeShiftDetailItemCol(Data, DTEnd, _ChangeDate, _CZFQTY, j_item, Source, model_type);
                    });
                    ...
                    switch (model_type) {
                        case 1:
                            ...
                            model.com.ChangeShiftDetailItemRow(Data, DTEnd, _YXDate, -_CZFQTY, UniqueID, Source, model_type, model_type);
                            model.com.changeSoruceDetailDate(item);
                            break;
                        case 2:
                            //2 推迟时间
                            ...
                            model.com.ChangeShiftDetailItemRow(Data, DTEnd, _YXDate, -_CZFQTY, UniqueID, Source, model_type);
            ...
            ChangeShiftDetailItemRow: function (Data, DTEnd, ChangeDate, CZFQTY, UniqueID, Source, model_type) {
                //迭代改影响工序  值为0且不是尾天的得跳日期哦 
                // 超出排程结束日期不顺延 直接加在最后一天
                ...
                if (item[$com.util.format("DT_yyyy_MM_dd", ChangeDate)] + _CZFQTY < 0) {
                    ...
                    while (_CZFQTY < 0) {
                        ...
                        if (_CZFQTY_1 != 0) { model.com.ChangeShiftDetailItemRow(Data, DTEnd, _StartT, _CZFQTY_1, UniqueID, Source, model_type); }
            ...
            refresh: function () {
                ...
                model.com.getOMSOrderList({ StartTime: mStartTimeNew, EndTime: mEndTimeNew, APSShiftPeriod: 6, WorkShopID: wWorkShopID }, function (resP) {
                    if (!resP)
                        return;
                    if (resP && resP.list) {
                        resP.list.sort(function (o1, o2) {
                            return new Date(o2.CreateTime).getTime() - new Date(o1.CreateTime).getTime();
                        }).sort(function (o1, o2) {
                        ...
                        $.each(_list, function (i, item) {
                            ...
                            item.Progress = $com.util.template(item, HTML.Progress);
                        ...
                        $("#femi-orderMakeAll-tbody tr").each(function (i, item) {
                            ...
                            var SelectData = DataOrderList.filter((item) => { return item.ID == tID })[0];

                            if (SelectData.Status == 2 || SelectData.Status == 3 || SelectData.Status == 4) {
                                $item.trigger("click");
                            }
                        });
                    }
            ...
            GetDate: function (source) {
                var arr = [],
                    maxDate = new Date(source[0].EndTime).getTime() / 1000 / 60 / 60 / 24,
                    minDate = new Date(source[0].StartTime).getTime() / 1000 / 60 / 60 / 24;
                if (source.length == 1) {
                    arr.push($com.util.format('yyyy-MM-dd', new Date(source[0].StartTime).getTime() - 7 * 1000 * 60 * 60 * 24));
                    arr.push($com.util.format('yyyy-MM-dd', new Date(source[0].EndTime).getTime() + 7 * 1000 * 60 * 60 * 24));
                }
                else {
                    $.each(source, function (i, item) {
                        if (new Date(item.StartTime).getTime() / 1000 / 60 / 60 / 24 <= minDate) {
                            minDate = new Date(item.StartTime).getTime() / 1000 / 60 / 60 / 24;
                        }
                        if (new Date(item.EndTime).getTime() / 1000 / 60 / 60 / 24 >= maxDate) {
                            maxDate = new Date(item.EndTime).getTime() / 1000 / 60 / 60 / 24;
                        }
                    });
                    arr.push($com.util.format('yyyy-MM-dd', new Date(minDate).getTime() * 1000 * 60 * 60 * 24));
                    arr.push($com.util.format('yyyy-MM-dd', new Date(maxDate).getTime() * 1000 * 60 * 60 * 24));
                }
                return arr;
            },
            //得到工段

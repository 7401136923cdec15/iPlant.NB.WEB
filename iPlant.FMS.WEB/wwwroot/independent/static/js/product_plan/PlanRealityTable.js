require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/base/echarts.min'], function ($zace, $com, $echarts) {
    ...
    HTML = {
        ...
        CustomTP: [
            '<tr>',
                '<td style="">{{CustomerName}}</td>',
                '<td style="" >{{YearFinsh}}</td>',
            '</tr>',
        ].join(""),
        //工位头部模板
        PartTP: [
        ...
        MonthFristTemplate: [
            '<tr>',
                '<td rowspan="{{LineLength}}" style="font-size: 35px;min-width: 75px;">{{Month}}</td>',
                '<td rowspan="3" style="font-size: 22px;" >{{LineName}}</td>',
                '<td class="lmvt-showNeed" style="font-size: 18px;color:#3E90C6">计划完工</td>',
                '{{Secendplan}}',
            '</tr>',
            '<tr>',
            '    <td class="lmvt-showNeed" style="font-size: 18px;color:green">实际完工</td>',
                '{{Secendrea}}',
            '</tr>',
            '<tr style="">',
        ...
        //修程对应第一块模板
        PlanFristTemplate: [
            '<tr style="color:#3E90C6" data-month={{MonthID}}>',
                '<td rowspan="{{PartCounts}}" style="font-size: 28px;min-width: 75px;">{{Month}}</td>',
                '<td rowspan="3" style="font-size: 22px;" >{{LineName}}</td>',
                '<td class="lmvt-showNeed" style="font-size: 18px;color:#3E90C6">计划完工</td>',
                '{{Secendplan}}',
            '</tr>',
            '<tr data-month={{MonthID}} style="color:green">',
            '    <td class="lmvt-showNeed" style="font-size: 18px;color:green">实际完工</td>',
                '{{Secendrea}}',
            '</tr>',
            '<tr data-month={{MonthID}} style="color:red">',
                '<td class="lmvt-showNeed" style="font-size: 18px;color:red">落后计划</td>',
                '{{Secenddelay}}',
            '</tr>',
                        '{{nextTr}}',
        ...
        //修程对应第二块模板
        PlanSecendTemplate: [
            '<tr data-month={{MonthID}} style="color:#3E90C6"> ',
                '<td rowspan="3" style="font-size: 22px;">{{LineName1}}</td>',
                '<td class="lmvt-showNeed" class="lmvt-plan-{{LineID}}" style="font-size: 18px;color:#3E90C6">计划完工</td>',
                '{{Secendplan1}}',
            '</tr>',
            '<tr data-month={{MonthID}} style="color:green">',
                '<td class="lmvt-showNeed" class="lmvt-rea-{{LineID}}" style="font-size: 18px;color:green">实际完工</td>',
                '{{Secendrea1}}',
            '</tr>',
            '<tr data-month={{MonthID}} style="olor:red">',
                '<td class="lmvt-showNeed" class="lmvt-delay-{{LineID}}" style="font-size: 18px;color:red">落后计划</td>',
                '{{Secenddelay1}}',
            '</tr>',
        ...
        PlanTR: [
            '<tr data-color=""><td>{{TypeName}}</td>{{CountList}}</tr>',
        ].join(""),
    };
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            $("body").delegate("#lmvt-search", "click", function () {
                ...
                //    var startTime = $com.util.format("yyyy-MM-01", new Date().getFullYear() + "-" + num);

                //    var endTime = new Date().getFullYear() + '-' + num + '-' + new Date(new Date().getFullYear(), num, 0).getDate();

                //    var vdata = { 'header': '月统计报表', 'href': './product_plan/PlanTableInfo.html?startTime=' + startTime + "&endTime=" + endTime, 'id': 'PlanTableInfo', 'src': './static/images/menu/newfactoryModel/techniquePart.png' };
                //    window.parent.iframeHeaderSet(vdata);
                //    //window.parent.randerMonthReport(startTime, endTime);
            ...
            $("body").delegate(".lmvt-order-body tr", "dblclick", function () {
                ...
                var startTime = $com.util.format("yyyy-MM-01", new Date().getFullYear() + "-" + Month);
                var endTime = new Date().getFullYear() + '-' + Month + '-' + new Date(new Date().getFullYear(), Month, 0).getDate();
                var vdata = { 'header': '生产日报明细', 'href': './product_plan/PlanTableInfo.html?startTime=' + startTime + "&endTime=" + endTime, 'id': 'PlanTableInfo', 'src': './static/images/menu/newfactoryModel/techniquePart.png' };
                window.parent.iframeHeaderSet(vdata);
                window.callFunctionTrigger("randerMonthReport", { startTime: startTime, endTime: endTime });
            });
            //视图切换
            $("body").delegate("#lmvt-changeView", "click", function () {
                if (NumberView == 0) {
                    $(".lmvt-tableView").hide();
                    $(".lmvt-chartsView").show();
                    NumberView = 1;
                    return false;
                }
                if (NumberView == 1) {
                    $(".lmvt-tableView").show();
                    $(".lmvt-chartsView").hide();
                    NumberView = 0;
                    return false;
                }
            });
        ...
        run: function () {
            ...
            var myChart1 = $echarts.init(document.getElementById('lmvt-left-contain'));

            var Str = new Date().getFullYear() + "年" + (new Date().getMonth() + 1) + "月" + new Date().getDate() + "日" + ")";
            ...
            var adarr = {},
                CustomerList = [];
            model.com.getCustomer({ active: 1 }, function (res1) {
                model.com.getRPTCustomerShiftAll({ StartTime: StartTime, EndTime: EndTime }, function (res) {
                    ...
                    $.each(res.list, function (j, jtem) {
                        ...
                        $.each(res1.list, function (i, item) {
                            ...
                            if (item.ID == jtem.CustomerID && !adarr[jtem.CustomerID]) {
                                adarr[jtem.CustomerID] = {};
                                option.yAxis.data.push(item.CustomerName);
                                option.series[0].data.push(jtem.YearFinsh);
                                CustomerList.push({
                                    CustomerName: item.CustomerName,
                                    YearFinsh: jtem.YearFinsh
                                })
                            }
                        });
                    });
                    myChart1.setOption(option);
                    $(".lmvt-left-body").append($com.util.template(CustomerList, HTML.CustomTP));


                    //$(".AvgRepairPeriod").text(res.info.AvgRepairPeriod);
                });
            });


            model.com.getRPTProductShiftInfo({ ShiftDate: EndTime, ShiftPeriod: 4 }, function (res1) {
                var YearFinsh = 0, YearEnter = 0, MonthFinsh = 0, MonthEnter = 0, RealPlant = 0,
                RealRepair = 0;

                $.each(res1.list, function (i, item) {
                    YearFinsh += item.YearFinsh;
                    YearEnter += item.YearEnter;
                    MonthFinsh += item.MonthFinsh;
                    MonthEnter += item.MonthEnter;
                    RealPlant += item.RealPlant;
                    RealRepair += item.RealRepair;

                    if (item.LineID == 1) {
                        $(".C5Finsh").text(item.YearFinsh);
                        var avg = item.AvgRepairPeriod.toFixed(2)
                        $(".C5AvgRepairPeriod").text(avg);
                    }
                    if (item.LineID == 2) {
                        $(".C6Finsh").text(item.YearFinsh);
                        var avg = item.AvgRepairPeriod.toFixed(2)
                        $(".C6AvgRepairPeriod").text(avg);
                    }
                });
                $(".YearFinsh").text(YearFinsh); $(".YearEnter").text(YearEnter); $(".MonthFinsh").text(MonthFinsh);
                $(".MonthEnter").text(MonthEnter);
                $(".RealPlant").text(RealPlant); $(".RealRepair").text(RealRepair);
            });

            model.com.getRPTOrderPartAllTJ({ Year: StartTime }, function (res) {
                TreeNumber = res.Tree;
                model.com.getWorkAreaAll({ ID: -1, Active: -1 }, function (res1) {
                    ...
                    $.each(TreeNumber, function (m, mtem) {

                        mtem.PartCounts = mtem.TreeList.length * 3;
                        if (mtem.TreeList.length <= 1) {
                            mtem.nextTr = "";
                        }
                        $.each(mtem.TreeList, function (k, ktem) {
                            var RanderTP = {};
                            RanderTP.Plan = [];
                            RanderTP.Rea = [];
                            RanderTP.Delay = [];
                            ...
                            $.each(PartList, function (i, item) {
                                $.each(ktem.TreeList, function (j, jtem) {
                                    if (item.ID == jtem.PartID) {
                                        RanderTP.Plan.push({ Number: jtem.PlantCount });
                                        RanderTP.Rea.push({ Number: jtem.FinshCount });
                                        RanderTP.Delay.push({ Number: jtem.LaterCount });
                                        index.push(item.ID);
                                        return true;
                                    }
                                });
                            ...
                            $.each(arr, function (j, jtem) {
                                $.each(PartList, function (i, item) {
                                    if (item.ID == jtem.ID) {
                                        RanderTP.Plan.splice(i, 0, { Number: 0 });
                                        RanderTP.Rea.splice(i, 0, { Number: 0 });
                                        RanderTP.Delay.splice(i, 0, { Number: 0 });
                                        return true;
                                    }
                                });
                            });
                            if (k == 0) {
                                mtem.LineLength = ktem.TreeList.length * 3;
                                ...
                                mtem.LineName = ktem.LineName;
                            }
                            else {
                                mtem.Secendplan1 = $com.util.template(RanderTP.Plan, HTML.NumberTemplate);
                                mtem.Secendrea1 = $com.util.template(RanderTP.Rea, HTML.NumberTemplate);
                                mtem.Secenddelay1 = $com.util.template(RanderTP.Delay, HTML.NumberTemplate);
                                mtem.LineName1 = ktem.LineName;
                                mtem.nextTr = $com.util.template(mtem, HTML.PlanSecendTemplate);
                            }
                        });
                    ...
                    $(".lmvt-order-body").html($com.util.template(TreeNumber, HTML.PlanFristTemplate));
        ...
        com: {
            ...
            OnceAgain: function (type) {
                ...
                if (type == 0) {
                    $.each(TreeNumber, function (i, item) {
                        if (item.MonthID != NowMonth)
                            return true;
                        arr.push(item);
                    });
                }
                $.each(arr, function (m, mtem) {
                ...
                $(".lmvt-order-body").html($com.util.template(arr, HTML.PlanFristTemplate));
            },
            //查询信息
            ...
            //C5C6统计信息
            getRPTProductShiftInfo: function (data, fn, context) {
                var d = {
            ...
            randerGante: function (arr) {
                model.com.getCompareList({ dara: arr }, function (res) {
            ...
            //年报表
            getRPTOrderPartAllTJ: function (data, fn, context) {
                var d = {
                    $URI: "/RPTOrderPart/AllTJ",
                    $TYPE: "get",
                    $SERVER: '/MESAPS'
                };

require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base'], function ($zace, $com) {
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            $("body").delegate("#zace-edit-level", "click", function () {
                ...
                $("body").append($com.modal.show(default_value, KEYWORD_Level, "修改", function (rst) {
                    ...
                    if (Number(rst.PrevPartID) == SelectData[0].ID) {
                        alert("不能选自己！")
                        return false;
                    }
            ...
            $("body").delegate("#zace-add-levelNext", "click", function () {
                ...
                $("body").append($com.modal.show(default_value, KEYWORD_Level, "修改", function (rst) {
                    ...
                    SelectData[0].NextID = rst.NextID;
        ...
        com: {
            ...
            refresh: function () {
                $com.app.loading('数据加载中...');
                $('.zace-header-title').text('(' + FORMATTRT_Level["RouteID"](mRouteIDZace) + ')' + '二级流程');
                model.com.getFPCRoutePart({ RouteID: mRouteIDZace }, function (resP) {
                    if (!resP)
                        return;
                    if (resP && resP.list) {
                        // var Grade = $com.util.Clone(resP.list);
                        TypeSource_Level.PrevPartID.splice(1, TypeSource_Level.PrevPartID.length - 1);

                        $.each(resP.list, function (i, item) {
                            TypeSource_Level.PrevPartID.push({
                                name: item.Name,
                                value: item.ID,
                            });
                        });

                        TypeSource_Level.NextID = TypeSource_Level.PrevPartID;

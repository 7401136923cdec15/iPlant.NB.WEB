require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/base/jquery.treeview', '../static/utils/js/base/paging'], function ($zace, $com, $tree, $page) {
    ...
    model = $com.Model.create({
        ...
        com: {
            ...
            refresh: function () {
                if (mLineID <= 0) {
                    alert("无产线数据，请先确定产线存在并且已激活。");
                    return;
                }
                if (mProductID <= 0) {
                    alert("无型号数据，请先确定型号存在并且已激活。");
                    return;
                }
            ...
            refreshc: function () {
                ...
                if (mID > 0) {
                    model.com.getItemTree({ LineID: mLineID, ProductID: mProductID, ID: mID }, function (resBomItem1) {
                        ...
                        var name = FORMATTRT_BOMItem["LineID"](mLineID) + "线—" + FORMATTRT_BOMItem["ProductID"](mProductID) + "—型号清单";
                        $("#zace-spanTextChange").html(name);

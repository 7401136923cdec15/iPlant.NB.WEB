define(['./jquery-3.1.1', './base/base'], function ($jQ, $com) {
    ...
    var GetNextPartIDList = function (wPartList, wPartID, _Result) {
        if (!wPartList || wPartList.length <= 0)
            return _Result;
        if (!wPartID || isNaN(wPartID))
            return _Result;
        wPartID = Number(wPartID);
        if (wPartID <= 0)
            return _Result;

        $.each(wPartList, function (i, item) {

            if (item.PrevPartID != wPartID) {
                return true;
            }

            if ($.inArray(item.PartID, _Result) >= 0)
                return true;
            _Result.push(item.PartID);

            if (item.NextPartIDMap) {
                for (var key in item.NextPartIDMap) {
                    if (isNaN(key))
                        continue;
                    if ($.inArray(Number(key), _Result) >= 0)
                        continue;
                    _Result.push(Number(key));
                    GetNextPartIDList(wPartList, Number(key), _Result);
                }
            }
            GetNextPartIDList(wPartList, item.PartID, _Result);
        });

    }

    var GetPartListByCtrl = function (_RouteDic, wOrderID, wPartID) {
        var _Result = [];
        if (!_RouteDic || !_RouteDic["O" + wOrderID])
            return _RouteDic;

        var _Route = _RouteDic["O" + wOrderID];
        if (!_Route.PartList || _Route.PartList.length <= 0)
            return _RouteDic;
        ...
        GetNextPartIDList(_Route.PartList, wPartID, _Result);

        $.each(_Route.PartList, function (i, item) {
            if (item.PartID != wPartID)
                return true;
            if (item.NextPartIDMap) {
                for (var key in item.NextPartIDMap) {
                    if (isNaN(key))
                    ...
                    if ($.inArray(Number(key), _Result) >= 0)
                        continue;
                    _Result.push(Number(key));
                    GetNextPartIDList(_Route.PartList, Number(key), _Result);
                }
            }
        });
        return _Result;
    ...
    var ColorClass = ['#FF7F50', '#6495ED', '#008B8B', '#91AB65', '#8795cc', '#8f9fe0', '#E84D3B', '#7E8C8D', '#8548A7', '#B0C97D', '#429FC0', '#6D7181', '#b2ce83', '#7ca932', '#628628', '#c3a1d2', '#c78ee0', '#9c64b5', '#8795cc', '#5670d6', '#8f9fe0', '#344aa0', '#E84D3B', '#7E8C8D', '#C59BD7', '#8548A7', '#697787', '#91AB65', '#429FC0', '#6D7181', '#b2ce83', '#a4b981', '#7ca932', '#628628', '#c3a1d2', '#c78ee0', '#9c64b5', '#8795cc', '#5670d6', '#8f9fe0', '#344aa0', '#E84D3B', '#7E8C8D', '#C59BD7', '#8548A7', '#697787', '#91AB65', '#B0C97D', '#429FC0', '#6D7181', '#b2ce83', '#a4b981', '#7ca932', '#628628', '#c3a1d2', '#c78ee0', '#9c64b5', '#8795cc', '#5670d6', '#8f9fe0', '#344aa0', '#E84D3B', '#7E8C8D', '#C59BD7', '#8548A7', '#697787', '#91AB65', '#B0C97D', '#429FC0', '#6D7181', '#b2ce83', '#a4b981', '#7ca932', '#628628', '#c3a1d2', '#c78ee0', '#9c64b5', '#C59BD7'];
    ...
    var install = function (ContainDiv, ScrollContainDiv, positionIn) {
        ...
        var $scroll = $(' <div style=""></div>'),
            $canvas = $('<canvas style="border: 1px solid #DCE1E2;float: left;">'),
            $canvasLeft = $('<div class="canvasLeft1" style="position:absolute;overflow: auto;" ><canvas></div>'),
            $canvasTop = $('<div class="canvasTop1" style="position:absolute;overflow: auto;" ><canvas></div>'),
            $canvaslefttop1 = $('<div class="canvaslefttop1" style="position:absolute;" ><canvas></div>');
        $scroll.html($canvas);
        ...
        // positionIn.series.data[0] = $com.util.format("yyyy-MM-dd hh:mm:ss", positionIn.series.data[0]);
        // positionIn.series.data[1] = $com.util.format("yyyy-MM-dd hh:mm:ss", positionIn.series.data[1]);
        ...
        canvasTop.height = 52;
        canvaslefttop.height = 52;
        canvaslefttop.width = freedom;
        ...
        canvas.onmousedown = function (evt) {
            ...
            Direction = wDownRectResult.wIsLeft;

            //console.log(wDownRectResult);
            ...
            canvas.onmousemove = function (evt) {
                ...
                var fristCircle = false,
                    secendCircle = false,
                    //第二层循环
                    deepFristCircle = false,
                    deepSecendCircle = false;
                ...
                var stratdistance;
                if (ISPush) {
                    stratdistance = eventDown_x;
                    movedistance = eventMove_x - stratdistance;
                    ISPush = false;
                } else {
                    movedistance = eventMove_x - stratdistance;
                }

                if (movedistance < 35 && movedistance > -35) {
                    return false;
                } else {
                    stratdistance = eventDown_x;
                }
                ...
                //MoveDays = day * 24 * 3600000;

                if (position.ISMouseMove && position.ISMouseMove == 1)
                    return;
                    ...
                    $.each(position.Task.data[i].TaskPartList, function (j, jtem) {
                        $.each(jtem.TaskPartList, function (m, mtem) {
                            ...
                            if (mtem.UniqueID == wDownRectResult.wIndex) {
                                ...
                                wDownRectResult.wIsLeft = 2;
                                position.effect.Part = 1;
                                // if (true) {
                                //拖动矩形
                                ...
                                if (wDownRectResult.wIsLeft == -1 &&
                                    new Date(new Date(mtem.startDate).getTime() + (day * 24 * 3600000)) < new Date(position.series.data[0])) {
                                    return;
                                }
                                ...
                                if (position.effect.Time != 0 || position.effect.Part != 0) {

                                    var nextMapList = [];
                                    $.each(position.Task.routeList, function (r, rtem) {
                                        if (mtem.RouteID == rtem.ID) {
                                            var _data = {};
                                            _data["O" + mtem.OrderID] = rtem;
                                            nextMapList = GetPartListByCtrl(_data, mtem.OrderID, mtem.PartID);
                                        }
                                    });
                                    ...
                                    for (var i = 0; i < position.Task.data.length; i++) {
                                        $.each(position.Task.data[i].TaskPartList, function (t, ttem) {
                                            $.each(ttem.TaskPartList, function (p, ptem) {
                                                ...
                                                if (ptem.PartNo == mtem.PartNo && ptem.PartID != mtem.PartID && nextMapList.indexOf(pPartID) >= 0) {
                                                    if (position.effect.Time == 1 && position.effect.Part == 0) {
                                                        ...
                                                        if (pLineID == mLineID && nextMapList.indexOf(pPartID) >= 0) {
                                                            // if (wDownRectResult.wIsLeft == 1) {
                                                            //     if (new Date(new Date(ptem.startDate).getTime()
                                                            //         + ((ptem.time + day) * 24 * 3600000)) <= new Date(new Date(position.series.data[1]).getTime() + 24 * 3600000))
                                                            //         ptem.time += day;
                                                            ...
                                                            if (wDownRectResult.wIsLeft == 2) {
                                                                if (new Date(new Date(ptem.startDate).getTime()
                                                                    ...
                                                                    if (day >= 0) {
                                                                        ...
                                                                        if (new Date(new Date(ptem.startDate).getTime() + (day * 24 * 3600000 - 8 * 3600000)) <= new Date(position.series.data[0])) {
                                                                            //console.log(new Date(new Date(ptem.startDate).getTime() + (day * 24 * 3600000)).getTime() + "=====" + new Date(position.series.data[0]).getTime());
                                                                            ptem.startDate = position.series.data[0];
                                                                        }
                                                                }
                                                            }
                                                        ...
                                                        if (nextMapList.indexOf(pPartID) >= 0 && pTime >= mTime && pLineID == mLineID) {
                                                            // if (wDownRectResult.wIsLeft == 1) {
                                                            //     if (new Date(new Date(ptem.startDate).getTime()
                                                            //         + ((ptem.time + day) * 24 * 3600000)) <= new Date(new Date(position.series.data[1]).getTime() + 24 * 3600000))
                                                            //         ptem.time += day;
            ...
            for (var i = 0; i < position.Task.data.length; i++) {
                $.each(position.Task.data[i].TaskPartList, function (j, jtem) {
                    $.each(jtem.TaskPartList, function (m, mtem) {
                        ...
                        //     mtem.startDate = new Date(mtem.startDate);
                        // else {
                        //     mtem.startDate = new Date($com.util.format("yyyy-MM-dd", new Date(mtem.startDate).getTime() - 8 * 1200000));
                        // }

                        // //拖到的时候  半天四舍五入
                        // var time = $com.util.Clone(mtem.startDate.getTime());
                        // var ab = mtem.startDate.getTime() / (24 * 3600000);
                        // var ab_str = ab.toString();
                        // var ab_num = parseInt(ab_str.substring(ab_str.indexOf('.') + 1, ab_str.indexOf('.') + 2));
                        // var FloatDatW = 0;

                        // var time_length = mtem.time.toString(),
                        //     time_num = parseInt(time_length.substring(time_length.indexOf('.') + 1, time_length.indexOf('.') + 2));

                        // if (Direction == -1) {
                        //     if (ab_num >= 0 && ab_num < 5) {
                        //         FloatDatW = 0
                        //         mtem.startDate = new Date(Math.round(mtem.startDate.getTime() / (24 * 3600000)) * (24 * 3600000) + FloatDatW);
                        //         if (time != mtem.startDate.getTime()) {
                        //             mtem.time += ((time - mtem.startDate.getTime()) / (24 * 3600000));
                        //             //mtem.time = Math.round(mtem.time)-0.5;
                        //         }

                        //     }
                        //     if (ab_num >= 5) {
                        //         FloatDatW = 12 * 3600000
                        //         mtem.startDate = new Date(Math.round(mtem.startDate.getTime() / (24 * 3600000)) * (24 * 3600000) - FloatDatW);
                        //         if (time != mtem.startDate.getTime()) {
                        //             mtem.time += ((time - mtem.startDate.getTime()) / (24 * 3600000));
                        //             //mtem.time = Math.round(mtem.time)+0.5;
                        ...
                        //         mtem.time = Math.round(mtem.time) - 0.5;
                        //     }
                        //     console.info(mtem.startDate.getTime());
                        // }
    ...
    window.resizeObjectArray.splice(0, 0, function gantt_resize() {
        $(".canvasLeft1").css("top", $(".lmvt-container-gantt").offset().top);
        $(".canvasLeft1").css("left", $(".lmvt-container-gantt").offset().left);
        $(".canvasLeft1").css("width", position.freedomPx);
        $(".canvasLeft1").css("height", $(".lmvt-container-gantt").height() - 10);

        $(".canvaslefttop1").css("top", $(".lmvt-container-gantt").offset().top);
        $(".canvaslefttop1").css("left", $(".lmvt-container-gantt").offset().left);

        $(".canvasTop1").css("top", $(".lmvt-container-gantt").offset().top);
        $(".canvasTop1").css("left", $(".lmvt-container-gantt").offset().left);
        $(".canvasTop1").css("width", $(".lmvt-container-gantt").width());
        $(".canvasTop1").css("height", 52);
    });
    //监听容器变化
    $(".lmvt-container-gantt").resize(function () {

    });

    //画布上绘制任务时间矩形
    var ResfushCanvasOne = function (obj, num) {
        ...
        DrawRoundRect(rectStart_X, rectStart_Y, rectWidth, 25, radius, obj.color, obj.PartNo, obj.APSMessage, num);
            ...
            $.each(data, function (j, jtem) {
                $.each(jtem.TaskPartList, function (k, ktem) {
                    $.each(ktem.TaskPartList, function (m, mtem) {
                        ...
                        ResfushCanvasOne(mtem, j);
                    });
                    DrawPartPoint(TOPHEIGHT, ktem, j);
                    PREHEIGHT = TOPHEIGHT;
                });
                DrawPart(TOPHEIGHT, jtem, j);
    ...
    var CanvasBasic = function () {
        ...
        for (var j = 0; j <= daycat; j++) {
            ...
            if (true) {
                ...
                context.beginPath();
                context.strokeStyle = '#E9EEF6';
                context.moveTo(j * space + freedom + 0.5 * space, 18);
                context.lineTo(j * space + freedom + 0.5 * space, canvas.height);
                if (j == days) {
                    context.strokeStyle = '#E9EEF6';
                    context.moveTo((j + 1) * space + freedom + 0.5 * space, 18);
                    context.lineTo((j + 1) * space + freedom + 0.5 * space, canvas.height);
                }
                context.stroke();
                context.closePath();
            }
        ...
        if (!position.SortByOrder || position.SortByOrder != 1) {
            contextlefttop.fillText("工序", 40, 36);
            contextlefttop.fillText("产线", 188, 36);
        }else{
            contextlefttop.fillText("订单", 40, 36);
            contextlefttop.fillText("工序", 188, 36);
        }
    ...
    var MianCanvas = function () {
        function GetDays(startDate, endDate) {
            var days;
            days = (new Date(endDate) - new Date(startDate)) / 1000 / 60 / 60 / 24;
            return days;
        }
        var daycount = GetDays(position.series.data[0], position.series.data[1]);
    ...
    var ISodd = false,
        ISfirst = false;
    //画左侧修
    var DrawPartPoint = function (TOPHEIGHT, jtem, num) {
        if (num % 2 == 1) {
            contextleft.beginPath();
            contextleft.fillStyle = '#e9f4ff';
            contextleft.fillRect(100, PREHEIGHT, 150, TOPHEIGHT - PREHEIGHT);
        ...
        if (!position.SortByOrder || position.SortByOrder != 1) {
            contextleft.fillText(jtem.LineName, 120, (TOPHEIGHT - PREHEIGHT) / 2 + PREHEIGHT);
        } else {
            contextleft.fillText(jtem.PartName, 120, (TOPHEIGHT - PREHEIGHT) / 2 + PREHEIGHT);
        }
    ...
    var DrawPart = function (TOPHEIGHT, jtem, num) {
        ISodd = false;
        if (num % 2 == 1) {
            contextleft.beginPath();
            contextleft.fillStyle = '#e9f4ff';
            contextleft.fillRect(0, PARTHEIGHT, 120, TOPHEIGHT - PARTHEIGHT);
            // contextleft.moveTo(150, 35);
            // contextleft.lineTo(150, TOPHEIGHT);
            // contextleft.moveTo(LineX, TOPHEIGHT);
            // contextleft.lineTo(freedom, TOPHEIGHT);
        ...
        if (!position.SortByOrder || position.SortByOrder != 1) {
            ...
            contextleft.fillText(jtem.OrderNo, 10, (TOPHEIGHT - PARTHEIGHT) / 2 + PARTHEIGHT);
        }
        ...
        contextleft.beginPath();
        contextleft.strokeStyle = '#E9EEF6';
        contextleft.moveTo(120, 35);
        contextleft.lineTo(120, TOPHEIGHT);
        contextleft.moveTo(LineX, TOPHEIGHT);
        contextleft.lineTo(freedom, TOPHEIGHT);
        contextleft.stroke();
        contextleft.closePath();
    ...
    //圆角矩形
    var flag = false;
    var DrawRoundRect = function (x, y, width, height, radius, color, Name, str, num) {

        //context.canvas.
        if (num % 2 == 1) {
            context.beginPath();
            context.fillStyle = '#e9f4ff';
            context.fillRect(freedom, y, canvas.width, 23);
            context.fill();
            context.closePath();
        }
    ...
    var SearchMouseDownRect = function (m_x, m_y, data) {
        ...
        for (var i = 0; i < AllRectPosition.length; i++) {
            ...
            //判断某一行
            if (m_y <= rectStart_Y1 || m_y >= rectStart_Y1 + 25 || m_x <= rectStart_X1 || m_x >= rectStart_X1 + rectWidth1) {
                continue;
            }

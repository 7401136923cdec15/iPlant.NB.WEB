define(['../jquery-3.1.1', './base'], function ($zace, $com) {
    ...
    var route = (function () {
        ...
        var route_show = function ($contain, parms) {
            ...
            options_p.dataSet = parms.dataSet ? parms.dataSet : _parms.dataSet;
            options_p.dataSet.TypeIndex = options_p.dataSet.TypeIndex ? options_p.dataSet.TypeIndex : "Type";
            ...
            for (var i = 0; i < data.length; i++) {
                if (data[i][options_p.dataSet.TypeIndex] == 1) {
                    //画方格
                    coordinate = drawRect({
                        ctx: ctx,
                ...
                if (data[i][options_p.dataSet.TypeIndex] == 2) {
                    //画菱形
                    drawEllipse(
                        ctx,
                        data[i].X,
                ...
                if (data[i][options_p.dataSet.TypeIndex] == 3) {
                    //画椭圆
                    drawDiamond(
                        ctx,
                        data[i].X,
            ...
            var drag = function (x, y) {
                ...
                if (drag_obj.bool == true) {
                    ...
                    canvas2.onmousemove = function (ev) {
                        ...
                        //自身加金色边框区分
                        if (_dataInfo[options_p.dataSet.TypeIndex] == 1) {
                            drawBorder(ctx, _dataInfo.X - options_p.constant.rect_width / 2, _dataInfo.Y - options_p.constant.rect_height / 2, options_p.constant.dragColor, options_p.constant, _dataInfo[options_p.dataSet.TypeIndex]);
                        }
                        if (_dataInfo[options_p.dataSet.TypeIndex] == 2 || _dataInfo[options_p.dataSet.TypeIndex] == 3) {
                            drawBorder(ctx, _dataInfo.X, _dataInfo.Y, options_p.constant.dragColor, options_p.constant, _dataInfo[options_p.dataSet.TypeIndex]);
                        }
                        ...
                        //添加分支停放
                        for (var i = 0; i < data.length; i++) {
                            var _son = manySon(data, data[i][options_p.dataSet.Index]);
        ...
        var getFrame = function (targetData, canvasLeft, canvasTop, options_p, canvas2, $contain, lineList) {
            tepCanvas = canvas2;
            var frameHeight = 25 + 20 * targetData[options_p.dataSet.NextIndex].length + 10;
            ...
            '<span style="font-weight:bold;font-size:15px;color:white">' + targetData[options_p.dataSet.Text] + '</span>',
                '</div>',
            ...
            for (var i = 0; i < options_p.data.length; i++) {
                for (var j = 0; j < targetData[options_p.dataSet.NextIndex].length; j++) {
 
                    if (options_p.data[i][options_p.dataSet.Index] == targetData[options_p.dataSet.NextIndex][j]) {
            ...
            $(".route-bottom-left").click(function () {
                ...
                var _thisLine = [];
                for (var i = 0; i < options_p.data.length; i++) {
                    if (options_p.data[i][options_p.dataSet.Index] == parseInt(_sonID)) {
                        _sonData = options_p.data[i];
                    }
                }
                //第二步：判断子集在父级的位置,并找出线条
                var _retrunData = getPosition(options_p.data, targetData.X, targetData.Y, _sonData[options_p.dataSet.Index]);
            ...
            $(".route-top-add").click(function () {
                alert("请点双击被添加的流程(金黄色边框)", function () {
                    ...
                    _canvas2.addEventListener("dblclick", function (event) {
                        ...
                        if (target[options_p.dataSet.Index] == targetData[options_p.dataSet.Index]) {
                            alert("支流程不能选择本流程");
                            return false;
                        }
                        if (!target[options_p.dataSet.Index])
                            return;

                        targetData[options_p.dataSet.NextIndex].push(target[options_p.dataSet.Index]);
                        //加入已选择的支流程
                        for (var i = 0; i < options_p.data.length; i++) {
                            if (targetData[options_p.dataSet.Index] == options_p.data[i][options_p.dataSet.Index]) {
            ...
            $(".route-bottom-deit").click(function () {
                ...
                alert("请点双击将要修改的流程(金黄色边框区域)", function () {
                    ...
                    canvas2.addEventListener("dblclick", function (event) {
                        ...
                        for (var i = 0; i < targetData[options_p.dataSet.NextIndex].length; i++) {
                            if (targetData[options_p.dataSet.NextIndex][i] == parseInt(_sonID)) {
                                targetData[options_p.dataSet.NextIndex][i] = target[options_p.dataSet.Index];
                            }
                        }
                        //targetData[options_p.dataSet.NextIndex].push(target[options_p.dataSet.Index]);
                        //加入已选择的支流程
            ...
            $(".route-bottom-remove").click(function () {
                ...
                if (_sonID) {
                    for (var i = 0; i < targetData[options_p.dataSet.NextIndex].length; i++) {
                        if (targetData[options_p.dataSet.NextIndex][i] == parseInt(_sonID)) {
                            targetData[options_p.dataSet.NextIndex].splice(i, 1);
                        }
                    }
                    for (var i = 0; i < options_p.data.length; i++) {
                        if (targetData[options_p.dataSet.Index] == options_p.data[i][options_p.dataSet.Index]) {
        ...
        var addBoder = function (ctx2, targetData, options_p) {
            ...
            for (var i = 0; i < data.length; i++) {
                if (data[i][options_p.dataSet.Index] == targetData[options_p.dataSet.Index]) {
                    continue;
                } else {
                    if (data[i][options_p.dataSet.TypeIndex] == 1) {
                        drawBorder(ctx2, data[i].X - options_p.constant.rect_width / 2, data[i].Y - options_p.constant.rect_height / 2, options_p.constant.dragColor, options_p.constant, data[i][options_p.dataSet.TypeIndex]);
                    } else {
                        drawBorder(ctx2, data[i].X, data[i].Y, options_p.constant.dragColor, options_p.constant, data[i][options_p.dataSet.TypeIndex]);
                    }
                }
            }
        }
        ...
        var manySon = function (data, ID) {
            var _son = {};
            for (var i = 0; i < data.length; i++) {
                if (data[i][options_p.dataSet.PrevIndex] == ID) {
                    _son = data[i];
                }
            }
            return _son;
        ...
        var addSpareData = function (x, y, data, dataInfo, constant) {
            ...
            for (var i = 0; i < _data.length; i++) {
                if (_data[i][options_p.dataSet.Index] == 1000) {
                    _data.splice(i, 1);
                }
            }
            for (var i = 0; i < data.length; i++) {
            ...
            if (leftData) {
                ...
                spareData[options_p.dataSet.Index] = 1000;
                spareData[options_p.dataSet.Text] = _dataInfo[options_p.dataSet.Text];
                spareData.isSpare = true;
                for (var i = 0; i < data.length; i++) {
                    if (leftData[options_p.dataSet.Index] == data[i][options_p.dataSet.PrevIndex]) {
                        rightData = data[i];
                        rightData[options_p.dataSet.PrevIndex] = spareData[options_p.dataSet.Index];
                        rightData.OrderID += 1;
                        replaceArray(_data, [rightData]);
                        var _list = iteration(data, rightData[options_p.dataSet.Index], []);
                ...
                spareData[options_p.dataSet.PrevIndex] = leftData[options_p.dataSet.Index];
                ...
                if (_rdata) {
                    ...
                    spareData[options_p.dataSet.Text] = _dataInfo[options_p.dataSet.Text];
                    spareData.isSpare = true;
                    spareData[options_p.dataSet.PrevIndex] = _rdata[options_p.dataSet.PrevIndex];
                    if (_rdata[options_p.dataSet.PrevIndex] || _rdata.OrderID == 1) {
                        _rdata.OrderID += 1;
                        _rdata[options_p.dataSet.PrevIndex] = spareData[options_p.dataSet.Index];
                    }
                    if (!_rdata[options_p.dataSet.PrevIndex] && _rdata.OrderID > 1) {
                        _rdata[options_p.dataSet.PrevIndex] = spareData[options_p.dataSet.Index];
                    }
                    ...
                    var _list = iteration(data, _rdata[options_p.dataSet.Index], []);
                    for (var j = 0; j < _list.length; j++) {
                        _list[j].OrderID += 1;
                    }
                    replaceArray(_data, _list);
                    ...
                    spareData[options_p.dataSet.Text] = _dataInfo[options_p.dataSet.Text];
                    spareData.isSpare = true;
                    spareData[options_p.dataSet.PrevIndex] = _tdata[options_p.dataSet.PrevIndex];
                    for (var i = 0; i < data.length; i++) {
                        if (data[i][options_p.dataSet.PrevIndex] == _dataInfo[options_p.dataSet.Index]) {
                            _sonList.push(data[i]);
                        }
                        if (data[i][options_p.dataSet.Index] == _dataInfo[options_p.dataSet.PrevIndex]) {
                            _farList = data[i];
                        }
                    }
                    for (var i = 0; i < _sonList.length; i++) {
                        if (_farList[options_p.dataSet.Index]) {
                            _sonList[i][options_p.dataSet.PrevIndex] = _farList[options_p.dataSet.Index];

                        }
                        else {
                            _sonList[i][options_p.dataSet.PrevIndex] = 0;
                        }
                        _sonList[i].OrderID -= 1;
                        replaceArray(_data, _sonList);
                        var _list = iteration(data, _sonList[i][options_p.dataSet.Index], []);
                        for (var j = 0; j < _list.length; j++) {
                            _list[j].OrderID -= 1;
                        }
                        replaceArray(_data, _list);
        ...
        var clearCloth = function (ctx, data, options_p, _dataInfo) {
            ...
            //重新绘制流程
            for (var i = 0; i < _data.length; i++) {
                if (_data[i][options_p.dataSet.Index] == _dataInfo[options_p.dataSet.Index]) {
                    if (_data[i][options_p.dataSet.TypeIndex] == 1) {
                        //画方格
                        coordinate = drawRect({
                            ctx: ctx,
                    ...
                    if (_data[i][options_p.dataSet.TypeIndex] == 2) {
                        //画菱形
                        drawEllipse(
                            ctx,
                            _data[i].X,
                    ...
                    if (_data[i][options_p.dataSet.TypeIndex] == 3) {
                        //画椭圆
                        drawDiamond(
                            ctx,
                            _data[i].X,
                ...
                if (!_data[i].isSpare) {
                    ...
                    if (_data[i][options_p.dataSet.TypeIndex] == 1) {
                        drawBorder(ctx, _data[i].X - options_p.constant.rect_width / 2, _data[i].Y - options_p.constant.rect_height / 2, options_p.constant.dragColor, options_p.constant, _data[i][options_p.dataSet.TypeIndex]);
                        coordinate.X = _data[i].X - options_p.constant.rect_width / 2;
                        coordinate.Y = _data[i].Y - options_p.constant.rect_height / 2;
                        _returnData.X = coordinate.X;
                        _returnData.Y = coordinate.X;
                        _returnData[options_p.dataSet.TypeIndex] = _data[i][options_p.dataSet.TypeIndex];
                    }
                    if (_data[i][options_p.dataSet.TypeIndex] == 2 || _data[i][options_p.dataSet.TypeIndex] == 3) {
                        drawBorder(ctx, _data[i].X, _data[i].Y, options_p.constant.dragColor, options_p.constant, _data[i][options_p.dataSet.TypeIndex]);
                        coordinate.X = _data[i].X - options_p.constant.rect_width / 2;
                        coordinate.Y = _data[i].Y - options_p.constant.rect_height / 2;
        ...
        var ReleaseEvnt = function (x, y, data, dataInfo, constant) {
            ...
            if (leftData) {
                ...
                //需要移动的格子相关的数据做处理
                for (var i = 0; i < data.length; i++) {
                    if (data[i][options_p.dataSet.PrevIndex] == dataInfo[options_p.dataSet.Index]) {
                        _sonList.push(data[i]);
                    }
                    if (data[i][options_p.dataSet.Index] == dataInfo[options_p.dataSet.PrevIndex]) {
                        _farList = data[i];
                    }
                }
                for (var i = 0; i < _sonList.length; i++) {
                ...
                rightData[options_p.dataSet.PrevIndex] = dataInfo[options_p.dataSet.Index];
                dataInfo[options_p.dataSet.PrevIndex] = leftData[options_p.dataSet.Index];
                ...
                if (_rdata) {
                    ...
                    dataInfo[options_p.dataSet.PrevIndex] = _rdata[options_p.dataSet.PrevIndex];
                    if (_rdata[options_p.dataSet.PrevIndex] || _rdata.OrderID == 1) {
                        _rdata.OrderID += 1;
                        _rdata[options_p.dataSet.PrevIndex] = dataInfo[options_p.dataSet.Index];
                    }
                    if (!_rdata[options_p.dataSet.PrevIndex] && _rdata.OrderID > 1) {
                        _rdata[options_p.dataSet.PrevIndex] = dataInfo[options_p.dataSet.Index];
                    }
                    ...
                    dataInfo[options_p.dataSet.PrevIndex] = _tdata[options_p.dataSet.PrevIndex];
        ...
        var replaceArray = function (farlist, sonlist) {
            for (var i = 0; i < farlist.length; i++) {
                for (var j = 0; j < sonlist.length; j++) {
                    if (farlist[i][options_p.dataSet.Index] == sonlist[j][options_p.dataSet.Index]) {
                        farlist[i] = sonlist[j];
                    }
                }
            }
        ...
        var againDrawRect = function (x, y, target, ctx, options_p, e) {
            ...
            //}
            var coordinate = {};
            if (target[options_p.dataSet.TypeIndex] == 1) {
                //画方格
                coordinate = drawRect({
                    ctx: ctx,
            ...
            if (target[options_p.dataSet.TypeIndex] == 2) {
                //画椭圆
                drawEllipse(
                    ctx,
                    x,
            ...
            if (target[options_p.dataSet.TypeIndex] == 3) {
                //画菱形
                drawDiamond(
                    ctx,
                    x,
        ...
        var checkLineOwn = function (data, selfID, nextID, options_p) {
            var _Result = false;
            for (var i = 0; i < data.length; i++) {
                if (data[i][options_p.dataSet.Index] != nextID)
                    continue;
                if (data[i][options_p.dataSet.PrevIndex] != selfID)
                    continue;
                _Result = true;
                break; 
            }
        ...
        var checkData = function (data, obj, options_p, offset) {
            ...
            for (var i = 0; i < data.length; i++) {
                for (var j = 0; j < data[i][options_p.dataSet.NextIndex].length; j++) {

                    if (checkLineOwn(data, data[i][options_p.dataSet.Index], data[i][options_p.dataSet.NextIndex][j], options_p)) {
                        continue;
                    }
                    var obj1 = getPosition(data, data[i].X, data[i].Y, data[i][options_p.dataSet.NextIndex][j]);
        ...
        var getInfo = function (id, data) {
            for (var i = 0; i < data.length; i++) {
                if (data[i][options_p.dataSet.Index] == id) {
                    return data[i];
                }
            }
        }
        ...
        var getXY = function (data, constant) {
            ...
            //判断数据合法性
            for (var i = 0; i < data.length; i++) {
                if (data[i][options_p.dataSet.PrevIndex] != 0) {
                    // var obj = getInfo(data[i][options_p.dataSet.PrevIndex], data);
                    // if (obj.OrderID != data[i].OrderID - 1) {
                    //     alert("id为（ " + data[i][options_p.dataSet.Index] + " ）数据格式错误！");
                    //     return;
                    // }
                }
                if (data[i][options_p.dataSet.TypeIndex] < 1 || data[i][options_p.dataSet.TypeIndex] > 3) {
                    alert("type格式不合法");
                    return;
                }
            }
            ...
            for (var i = 0; i < _orderData.length; i++) {
                ...
                for (var j = 0; j < Listed.length; j++) {
                    if (Listed[j].X == _orderData[i].X && Listed[j].Y >= _orderData[i].Y && Listed[j][options_p.dataSet.Index] != _orderData[i][options_p.dataSet.PrevIndex])
                        _orderData[i].Y = Listed[j].Y + YJ;
                    //判断x不存在，并且存在上一层的子集
                    if (Listed[j][options_p.dataSet.Index] == _orderData[i][options_p.dataSet.PrevIndex]) {
                ...
                var _far = {};
                for (var j = 0; j < Listed.length; j++) {
                    if (Listed[j][options_p.dataSet.PrevIndex] == _orderData[i][options_p.dataSet.PrevIndex] && Listed[j][options_p.dataSet.PrevIndex] != 0 && _orderData[i][options_p.dataSet.PrevIndex] != 0)
                        IsOwn = true;
                    if (Listed[j][options_p.dataSet.Index] == _orderData[i][options_p.dataSet.PrevIndex])
                        _far = Listed[j];
                }
                ...
                if (IsOwn) {
                    ...
                    if (_orderData[i][options_p.dataSet.PrevIndex] != 0)
        ...
        var getOrder = function (data) {
            ...
            for (var i = 0; i < _firstOrderList.length; i++) {
                var _data = iteration(data, _firstOrderList[i][options_p.dataSet.Index], []);
            ...
            for (var i = 0; i < _list.length; i++) {
                for (var j = 0; j < _relevantList.length; j++) {
                    if (_list[i][options_p.dataSet.Index] == _relevantList[j][options_p.dataSet.Index]) {
                        _list[i].bool = true;
                    }
                }
            }
        ...
        var iteration = function (data, targetID, list) {
            if (targetID) {
                for (var i = 0; i < data.length; i++) {
                    if (data[i][options_p.dataSet.PrevIndex] == targetID) {
                        list.push(data[i]);
                        iteration(data, list[list.length - 1][options_p.dataSet.Index], list);
                    }
                }
            }
            return list;
        ...
        var return_click = function (canvas1, event, list, data1, canvasLeft, canvasTop) {
            ...
            for (var i = 0; i < list.length; i++) {
                if (x > list[i].X - data1.constant.rect_width / 2 && x < list[i].X - data1.constant.rect_width / 2 + data1.constant.rect_width
                    ...
                    var targetData = list[i];
                    target = targetData;
                    alert("已选中" + target[options_p.dataSet.Text]);
                    $("#_canvas2").remove();
                } else {
                    $("#_canvas2").remove();
                }
        ...
        var getOneData = function (id, data) {
            for (var i = 0; i < data.length; i++) {
                if (id == data[i][options_p.dataSet.Index]) {
                    return data[i];
                }
            }
        }

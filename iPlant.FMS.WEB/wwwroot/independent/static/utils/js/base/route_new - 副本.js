define(['../jquery-3.1.1', './base'], function ($zace, $com) {
    ...
    var route = (function () {
        ...
        var route_show = function ($contain, parms) {
            ...
            //添加鼠标点击事件
            if (!options_p.constant.lineOperation&&!options_p.fn_drag) {
            ...
            var drag = function (x, y) {
                ...
                if (drag_obj.bool == true) {
                    ...
                    canvas2.onmousemove = function (ev) {
                        ...
                        ctx2.clearRect(0, 0, canvas2.width, canvas2.height);
                        //重新画格子
                        againDrawRect(ax - canvasLeft, ay - canvasTop, drag_obj.dataInfo, ctx2, options_p);
                    ...
                    canvas2.onmouseup = function (ev) {
                        ...
                        //第一步：判断位置，以及改变数据
                        var _data = ReleaseEvnt(ax - canvasLeft, ay - canvasTop, data, drag_obj.dataInfo, options_p.constant);
     ...
     '</div>'].join("");
        ...
        '</div>'].join("");
            ...
            $(".route-bottom-deit").click(function () {
                ...
                alert("请点双击将要修改的流程(金黄色边框区域)", function () {
                    var $Canvas_drag = $('<canvas id="canvas2" width=' + canvas1.width + ' height=' + canvas1.height + ' style=" opacity:1;position:absolute;top:' + canvasTop + 'px;left:' + canvasLeft + 'px"></canvas>');
                    $contain.append($Canvas_drag);
                    var canvas2 = $Canvas_drag[0];
                    var ctx2 = canvas2.getContext("2d");
                    //给需要双击的部位加边框
        ...
        var againDrawRect = function (x, y, target, ctx, options_p) {
            var coordinate = {};
            if (target.Type == 1) {
                //画方格
                coordinate = drawRect({
            ...
            for (var i = 0; i < list.rect.length; i++) {
                if (x > list.rect[i].X - data1.constant.rect_width / 2 && x < list.rect[i].X - data1.constant.rect_width / 2 + data1.constant.rect_width
                   ...
                   data1.constant.rect_height) {
                    ...
                    getFrame(targetData, canvasLeft, canvasLeft, data1, canvas2, $contain, list.line);
                }
            }
        }

define(['../jquery-3.1.1', './base'], function ($jq, $com) {
    ...
    var pageTemplate = [
        ...
        '<option data-value=15 value ="15" class="optionFirst">15</option>',
    ...
    //  回调函数
    var init = function ($Table, Data, parms, fn, PageSizeNum) {
        ...
        if ($(".cby-paging-body").length > 0) {
            // PageCount = $table_foot[0].mPageCount;
            // PageIndex = $table_foot[0].mPageIndex;
            // mPageSize = $table_foot[0].mPageSize;
        }
        mPageSize = PageSizeNum;
        ...
        // $table_foot.find(".cby-paging li select .optionFirst").before('<option data-value=' + mPageSize + ' value =' + mPageSize + '>' + mPageSize + '</option>');
        $table_foot.find(".cby-paging li select .optionFirst").attr("data-value", mPageSize);
        $table_foot.find(".cby-paging li select .optionFirst").val(mPageSize);
        $table_foot.find(".cby-paging li select .optionFirst").text(mPageSize);
        var mPageSizeFirst = mPageSize + 10;
        var mPageSizeSecond = mPageSize + 20;
        $table_foot.find(".cby-paging li select").append('<option data-value=' + mPageSizeFirst + ' value =' + mPageSizeFirst + '>' + mPageSizeFirst + '</option>');
        $table_foot.find(".cby-paging li select").append('<option data-value=' + mPageSizeSecond + ' value =' + mPageSizeSecond + '>' + mPageSizeSecond + '</option>');

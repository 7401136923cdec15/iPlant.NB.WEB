define([ '../jquery-3.1.1', './bootstrap', './vue-element' ], function($yang, $Bs, Vue){

    window.resizeObjectArray = [];
    ...
    ( function(){
        ...
        //表头的全选全不选事件
        $('body')
            .delegate('.femi-tb-scroll table.table thead tr th input[type=checkbox].femi-tb-checkbox', 'change', function(e){
                ...
                if( $tableDiv[0] ){
                    $table = $tableDiv.siblings('table.table');
                }
        ...
        $('.femi-tb-scroll>table.table').find('th[data-order].lmvt-tb-th-sort').each(function(i, item){
            if( $(item).find('.lmvt-sort').length > 0 ){
                return true;
            }
            var $item = $(item);
            $item.append('<div class="lmvt-sort"><div class="lmvt-sortUp"></div><div class="lmvt-sortDown"></div></div>');
        });


        $('.femi-tb-scroll').scroll(function(){

            var $scroll = $(this),
                $tbDiv = $scroll.children('div.lmvt-tb-show');
        ...
        // $(function () {
        // 	$(".femi-tb-scroll>table.table").each(function (i, table) {
        // 		var $table = $(table),
        // 			$scroll = $table.closest(".femi-tb-scroll");
        // 		var IsOwn = ($scroll.children("div").children("table.table").length > 0);
        // 		if (IsOwn)
        // 			return true;
        // 		var tb2 = tb1.cloneNode(true);
        // 		var bak = document.createElement("div");
        // 		$(bak).addClass("lmvt-tb-show");
        // 		$scroll[0].appendChild(bak);
        ...
        //表头固定
        function tableColfix($table){
            var $scroll = $table.closest('.femi-tb-scroll'),
                size = 1;
            ...
            $scroll.scroll(function(){
                ...
                var $table = $(this).children('table.table'),
                    $Div = $(this).children('div.lmvt-tb-show');
            ...
            var IsOwn = ( $scroll.children('div.lmvt-tb-show').children('table.table').length > 0 );

            var bak = undefined;

            if( IsOwn ){
                tb2 = $scroll.children('div.lmvt-tb-show').children('table.table')[0];
                // 创建一个div  
                bak = $scroll.children('div.lmvt-tb-show')[0];

                //判断表头是否发生了变化

                var tb2Len = $(tb2).children('thead')[0].rows[0].cells.length,
                    tableLen = $table.find('thead')[0].rows[0].cells.length;
                if( tb2Len != tableLen ){
                    $(tb2).remove();
                ...
                $(tb2).find('th[data-order].lmvt-tb-th-sort').each(function(i, item){
                    if( $(item).find('.lmvt-sort').length > 0 ){
                        return true;
                    }
                    var $item = $(item);
            ...
            if( !tb1.rows[0] || tb1.rows[0].length <= 0 )
                return false;
            ...
            $(tb2).find('thead').css('visibility', 'visible');
            ...
            bak.style.top = ( $scroll.offset().top - $(window).scrollTop() ) + 'px';
            bak.style.left = ( $table.offset().left - $(window).scrollLeft() ) + 'px';


        }

        window.resizeObjectArray.splice(0, window.resizeObjectArray.length);
        window.resizeObjectArray.push(function(){
            $('.femi-tb-scroll>table.table').each(function(i, table){
                tableColfix($(table));
            });
        ...
        $('.femi-tb-scroll>table.table').bind('DOMNodeInserted', function(){
            var $table = $(this);
            tableColfix($table);
        });
        ...
        $(window).resize(function(){
            ...
            $.each(window.resizeObjectArray, function(b, bfn){
                if( bfn )
                    bfn();
        ...
        //表格内容升序
        $('body').delegate('.femi-tb-scroll table.table .lmvt-sortUp', 'click', function(e){
            var $this = $(this),
                //找到对应的点击下标
                _title = $this.closest('th').attr('data-order'),
                //找到最近的表格身体
                $tbody = $(this).closest('.femi-tb-scroll').children('table.table').children('tbody'),
                $down = $this.parents('th').find('.lmvt-sortDown'),
                tbody = $tbody[0];
            ...
            //去色
            $down.removeClass('lmvt-sortDownActive');
            //上色
            $this.addClass('lmvt-sortUpActive');

            //非负浮点数字正则
            var patternMinus = /^\d+(\.\d+)?$/;
            //负浮点数字正则
            var patternPositive = /^(-(([0-9]+\.[0-9]*[1-9][0-9]*)|([0-9]*[1-9][0-9]*\.[0-9]+)|([0-9]*[1-9][0-9]*)))$/;

            TrValue.sort(function(tr1, tr2){
                var value1 = $(tr1).children('td[data-title=' + _title + ']').text();
                var value2 = $(tr2).children('td[data-title=' + _title + ']').text();
                //数字单独处理
                if( ( patternMinus.test(value1) && patternMinus.test(value2) ) || ( patternPositive.test(value1) && patternPositive.test(value2) ) ){
            ...
            //wu bei shang tian => bei shang tian wu
        ...
        //表格内容降序
        $('body').delegate('.femi-tb-scroll table.table .lmvt-sortDown', 'click', function(e){
            var $this = $(this),
                //找到对应的点击下标
                _title = $this.closest('th').attr('data-order'),
                //找到最近的表格身体
                ...
                $up = $this.parents('th').find('.lmvt-sortUp'),
            ...
            //去色
            $up.removeClass('lmvt-sortUpActive');
            //上色
            $this.addClass('lmvt-sortDownActive');
        ...
        $('body').delegate('.femi-tb-scroll table.table>tbody>tr>td[data-title]', 'click', function(e){
            var $this = $(this),
                $tr = $(this).closest('tr'),
                $tbody = $tr.closest('tbody');

            var $check = $tr.children('td').children('input[type=checkbox].femi-tb-checkbox');
            var _count = 0;
            ...
            $tbody.children('tr').each(function(i, item){
                ...
                $(item).removeCss('background-color');
                var $checkother = $(item).children('td').children('input[type=checkbox].femi-tb-checkbox');
                if( $checkother[0] && $checkother[0].checked )
                    _count++;
                $checkother.prop('checked', false);

                $this.closest('.femi-tb-scroll').find('thead th input[type=checkbox].femi-tb-checkbox')
                    .prop('checked', false);
            ...
            if( _count > 1 ){
                ...
                $check.css('background-color', '#f0cf89');
                return;
            }
            $check.click();
        });
        $('body')
            .delegate('.femi-tb-scroll table.table>tbody>tr>td>input[type=checkbox].femi-tb-checkbox', 'change', function(e){
                var $this = $(this),
                    $tr = $(this).closest('tr');
        ...
        // 通知  options: https://element.eleme.cn/#/zh-CN/component/notification
        window.PutMessage = function(options){
            if(window.parent && window !== window.parent && window.parent.PutMessage ){
                window.parent.PutMessage(options);
                return false;
            }
    ...
    $(function(){
        ...
        $('body').delegate('.mouse-move-contain', 'mousemove', function(e){
            ...
            if( $expander.hasClass('move-vertical') ){
                ...
                var _maxW = isNaN($contain.children('.mouse-move-left')
                    .attr('data-max-width')) ? 0 : Number($contain.children('.mouse-move-left').attr('data-max-width'));
                var _minW = isNaN($contain.children('.mouse-move-left')
                    .attr('data-min-width')) ? 0 : Number($contain.children('.mouse-move-left').attr('data-min-width'));
                ...
                var _maxH = isNaN($contain.children('.mouse-move-middle')
                    .attr('data-max-height')) ? 0 : Number($contain.children('.mouse-move-middle')
                    .attr('data-max-height'));
                var _minH = isNaN($contain.children('.mouse-move-middle')
                    .attr('data-min-height')) ? 0 : Number($contain.children('.mouse-move-middle')
    ...
    var tableTool = {
        ...
        //导出
        getExportParams: function($table, fileName, Title, Data_Source){
            ...
            if( Data_Source && Data_Source.length > 0 ){
                ...
                $.each(Data_Source, function(i_t, tr){

                    var row = {},
                        isPush = false;
                    for( var _title in tr ){
                        var text = tr[_title];
                ...
                $table.find('tbody tr').each(function(i_t, tr){
                    ...
                    var _IsExport = $tr.find('td input[type=checkbox].femi-tb-checkbox').prop('checked');
                    if( !_IsExport )
                        return true;
        ...
        postExportParams: function(source, $table){
            var result = [],
                HeadOrder = {};
            var $Export;
            $Export = $table.closest('.femi-tb-scroll').find('table.table');
            if( $Export.length == 0 ){
                $table = $table.closest('.femi-tb-scroll').find('.footerBottom table.table.table-partMultiple');
            } else{
                $table = $Export;
            }
        ...
        showFilterPage: function($tableBd, FilterSource, fn){
            if( window.$page ){
                window.$page.init($tableBd.closest('table'), FilterSource, '', fn);
            }
        },
        ...
        getFilterSourceByLikeString: function(data_list, wLikeString, Formatter_Source, delimiter, not_union){
            ...
            wLikeString = wLikeString.trim();

            if( wLikeString.length <= 0 )
                return data_list;
        ...
        filterByLikeStringData: function($tableBd, data_list, wLikeString, Formatter_Source, delimiter, not_union, fn){
            tableTool.showFilterPage($tableBd, tableTool.getFilterSourceByLikeString(data_list, wLikeString, Formatter_Source, delimiter, not_union), fn);
            $(window).resize();
        },

        //表格行合并
        formatterRowspan: function($tableBd, firstcols, cols){
            ...
            if( !cols )
                cols = 5;
            var _DATA = {};
            $tableBd.children('tr').each(function(i, tr){
                $(tr).children('td[data-title]').each(function(j, td){
                    if( j > cols ){
                        return true;
                    }
                    if( j < firstcols ){
            ...
            for( var p in _DATA ){
                ...
                var tdArray = _DATA[p];
                if( !tdArray || tdArray.length <= 1 )
                    continue;

                var _spanTD = undefined;
                var colspan = 1;

                for( var i = 1; i < tdArray.length; i++ ){

                    if( !tdArray[i - 1] || !tdArray[i - 1].html() || tdArray[i - 1].html().trim().length < 1 ){
                    ...
                    if( !tdArray[i] || !tdArray[i].html() || tdArray[i].html().trim().length < 1 ){
                    ...
                    if( !isNaN(tdArray[i - 1].html().trim()) ){
                    ...
                    if( tdArray[i - 1].html().trim() != tdArray[i].html().trim() ){
                    ...
                    _spanTD.attr('rowSpan', colspan);

                    _spanTD.css('vertical-align', 'middle');
    ...
    var app = {
        ...
        processCode: function(code, msg){
            ...
            switch( code ){
                case 9998:
                    ...
                    if( window.parent && window.parent.AssignUser ){
                        window.parent.AssignUser(window);
                    } else if( window.AssignUser ){
                        window.AssignUser(window);
                    } else{
                        app.tip('身份验证错误，请退出后重新登录!');
                    }
                    break;
        ...
        loginUser: function(username, password, fn, err, context){
            app.loading('登录中...');
        ...
        checkRole: function(functionID){
            var _result = false,
                _RoleList = [];
            if( window.parent && window.parent.User_Info && window.parent.User_Info.RoleList ){
                _RoleList = window.parent.User_Info.RoleList;
            } else if( window.User_Info && window.User_Info.RoleList ){
                _RoleList = window.User_Info.RoleList;
            } else{
                return _result;
            }

            $.each(_RoleList, function(i, item){
        ...
        ajax: function(data, fn, err, context){
            ...
            /*if (!window.login_status) {
                ...
                var query = uri.getUrlQuery((window.location.hash?window.location.hash:"").split('?')[1] || "");
    ...
    var util = ( function(){
        ...
        return assign(util, {
            ...
            configBuild: function(KEYWORDLIST, KEYWORD, TypeSource, Formatter){
                ...
                $.each(KEYWORDLIST, function(i, item){
                    ...
                    KEYWORD[detail[0]] = {
                        index: i,
                        name: detail[1],
                        type: detail.length > 2 && detail[2].length > 0 ? detail[2] : undefined,
                        control: detail.length > 3 && detail[3].length > 0 ? detail[3] : undefined,
                        rule: detail.length > 4 && detail[4].length > 0 ? detail[4] : undefined,
                    };
                    var _column = {
                        field: detail[0],
                        title: detail[1],
                    ...
                    if( detail.length > 2 ){
                        Formatter[detail[0]] = util.getFormatter(TypeSource, detail[0], detail[2]);
                    }
                });
            ...
            fontsizeChange: function(_inSize){
                ...
                if( window.frames && window.frames.length > 0 ){
                    for( var i = 0; i < window.frames.length; i++ ){
                        ...
                        $(window.frames[i].document).find('.femi-font-size-large')
                            .css('font-size', ( 14 + _size ) + 'px');
                        $(window.frames[i].document).find('.femi-font-size-biglarge')
                            .css('font-size', ( 16 + _size ) + 'px');
                        $(window.frames[i].document).find('.femi-font-size-middle')
                            .css('font-size', ( 12 + _size ) + 'px');
                        $(window.frames[i].document).find('.femi-font-size-small')
                            .css('font-size', ( 10 + _size ) + 'px');
                        $(window.frames[i].document).find('.femi-font-size-little')
                            .css('font-size', ( 8 + _size ) + 'px');
                    }
                }
    ...
    var module = ( function(){
        ...
        Model = {
            create: function(component){
                ...
                if( component.VueName && component.VueName.length > 1 ){

                    if( !component.methods ){
                        component.methods = {};
                    }
                    $.extend(component.methods, component.com);

                    window[component.VueName] = new Vue(component);
                }
    ...
    var dynamicModal = {
        ...
        HTML: ( function(){
            return {
                ...
                ItemBool: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                ...
                ItemString: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                              '<label class="m-detail-title">{{title}}</label>',
                              '<input type="text" class="form-control" data-name="{{name}}" placeholder="{{title}}" value="{{value}}" >',
                              '</div>',
                ].join(''),
                ItemStringPassword: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                                      '<label class="m-detail-title">{{title}}</label>',
                                      '<input type="password" class="form-control" data-name="{{name}}" placeholder="{{title}}" value="{{value}}" >',
                                      '</div>',
                ].join(''),
                ItemStringInputArray: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                ...
                ItemFile: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                            '<label class="m-detail-title">{{title}}</label>',
                            '<input type="file" data-multi-max="1" class="form-control" data-value="{{value}}" style="padding: 0" data-type="input-file" data-name="{{name}}" placeholder="{{title}}" value="{{value}}" >',
                            '</div>',
                ].join(''),

                ItemFileArray: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                                 '<label class="m-detail-title">{{title}}</label>',
                                 '<input type="file" data-multi-max="5" multiple="“multiple”" data-value="{{valueString}}" style="padding: 0" class="form-control" data-type="input-file-array" data-name="{{name}}" placeholder="{{title}}" value="{{valueString}}" >',
                                 '</div>',
                ].join(''),
                // ItemFileArray: ['<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                //     '<label class="m-detail-title">{{title}}</label>',
                ...
                ItemStringReadonly: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                                      '<label class="m-detail-title">{{title}}</label>',
                                      '<input type="text" class="form-control" data-name="{{name}}" readonly="readonly" placeholder="{{title}}" value="{{value}}" />',
                                      '</div>',
                ].join(''),

                ItemLongText: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                                '<label class="m-detail-title">{{title}}</label>',
                                '<p contenteditable="true"  style="height: auto;" class="form-control" data-name="{{name}}"  >{{value}}</p>',
                                '</div>',
                ].join(''),
                ItemDateTime: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                                '<label class="m-detail-title">{{title}}</label>',
                ...
                ItemDate: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                            '<label class="m-detail-title">{{title}}</label>',
                ...
                ItemWeek: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                            '<label class="m-detail-title">{{title}}</label>',
                            '<input class="form-control" data-type="week" type="week" data-name="{{name}}" value="{{value}}" placeholder="{{title}}" >',

                            '</div>',
                ].join(''),
                ItemTime: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                            '<label class="m-detail-title">{{title}}</label>',
                ...
                ItemNumber: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                              '<label class="m-detail-title">{{title}}</label>',
                ...
                ItemDropOne: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                               '<label class="m-detail-title">{{title}}</label>',
                ...
                ItemDrop: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                            '<label class="m-detail-title">{{title}}</label>',
                            '<label for="modal_select_{{name}}" ></label>',
                ...
                ItemDropOneControl: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                                      '<label class="m-detail-title">{{title}}</label>',
                                      '<label for="modal_select_{{name}}" ></label>',
                ...
                ItemDropControl: [ '<div class="femi-modal-item" style="min-width:{{MinWidth}};width:{{MinWidth}};" >',
                                   '<label class="m-detail-title">{{title}}</label>',
                                   '<label for="modal_select_{{name}}" ></label>',
        ...
        start: ( function(){
            ...
            $('body').delegate('#femi-modal-confirm', 'click', function(){
                ...
                var str = '';
                $('.femi-modal-body .femi-modal-item').each(function(i, item){
                    var $this = $(item),
                        $name = $this.find('label.m-detail-title').text(),
                        $value = $this.find('input.form-control').val();

                    if( $this.children('input.form-control').length == 0 ){
                        return true;
                    }
                    ...
                    if( $value.length <= 0 ){
                        var cat = $name.substring(0, $name.length - 1);
                        str += cat + ' ';
                    }
                });

                if( str.length > 0 ){
                    $('.modal-femi-alarm').html(str + '为必填项，请填写完成');
                    return false;
                }
                ...
                $('body #femi-modal-contain .femi-modal-body .femi-modal-item').each(function(i, item){
                    ...
                    var name = $($value[0]).attr('data-name'),
                        value,
                        cur = new Date(),
                    ...
                    if( type ){
                        switch( type ){
                            ...
                            case 'input-file':
                                ...
                                break;
                            case 'input-file-array':
                                value = $value.attr('data-value').split(' |,| ');
                                break;
                            default:
                                value = $value.val();
                                break;
                ...
                if( dynamicModal.check_fn ){
                    var msg = dynamicModal.check_fn(_data);

                    if( msg && msg.length > 0 ){
                        $('.modal-femi-alarm').html('数据验证错误 ' + msg);
                        return false;
                    }
            ...
            $('body')
                .delegate('.modal .femi-modal-body .femi-modal-item .input-array-contain .input-array-item-c   input.input-array-item', 'change', function(){
                    var $this = $(this),
            ...
            $('body')
                .delegate('.modal .femi-modal-body .femi-modal-item select.femi-modal-dropOneControl', 'change', function(e){
                    var $this = $(this),
            ...
            $('body').delegate('.modal .femi-modal-body .femi-modal-item input[type=file]', 'change', function(e){

                var self = this,
                    _dataFile = self.files,

                    _maxFileNum = Number($(self).attr('data-multi-max'));
                if( isNaN(_maxFileNum) || _maxFileNum < 1 )
                    _maxFileNum = 1;
                if( !_dataFile || _dataFile.length < 1 ){
                    clearFiles();
                    return false;
                }

                if( _dataFile.length > _maxFileNum ){
                    alert('上传文件数超过' + _maxFileNum + '个！');
                    clearFiles();
                    return false;
                }
                if( _dataFile[0] ){
                    ...
                    $.each(_dataFile, function(i, _data){
                    ...
                    $.ajax({ //
                        url: serviceCon.server + '/api/Upload/Submit',
                        type: 'POST',
                        data: form,
                        processData: false,
                        contentType: false,
                        ...
                        if( data.resultCode === 1000 ){
                            $(self).attr('data-value', data.returnObject.list.join(' |,| '));
                        } else{
                            alert('上传失败，请重新再试');
                            clearFiles();
                        }
        ...
        show: function(data, KEYWORD, title, fn, source, check, format, item_width){
            if( !item_width )
                item_width = {};

            //item_width.No="600px";// 最小 600px
                ...
                for( var p in item ){
                    ...
                    if( o ){
                        _data[Number(o.index)] = {
                            MinWidth: item_width[p] ? item_width[p] : '-',
                            name: p,
                            value: item[p],
                            title: o.name,
                            type: o.type,
                    ...
                    $.each(filter_data, function(i, item){
                        ...
                        if( item.type ){
                            ...
                            switch( item.type ){
                                ...
                                case 'Week':
                                    item.value = util.format('yyyy-MM-dd', item.value);
                                    _data.push(util.template(item, dynamicModal.HTML.ItemWeek));
                                    break;
                                case 'Time':
                                ...
                                case 'String':
                                    ...
                                    break;
                                case 'File':
                                    _data.push(util.template(item, dynamicModal.HTML.ItemFile));
                                    break;
                                case 'FileArray':
                                    ...
                                    _data.push(util.template(item, dynamicModal.HTML.ItemFileArray));
                                    break;
                                case 'Bool':
                                    ...
                                    break;
                                case 'Password':
                                    _data.push(util.template(item, dynamicModal.HTML.ItemStringPassword));
                                    break;
                                default:
                                    _IsOK = false;
                                    break;
            ...
            $('.modal .femi-modal-body .femi-modal-item>.m-detail-title').each(function(i, item){
                var wName = $(item).html();
                if( wName.startsWith('*') ){
                    wName = wName.substr(1);
                } else if( wName.endsWith('*') ){
                    wName = wName.substr(0, wName.length - 1);
                } else{
                    return true;
                }
                ...
                $(item).parent('.femi-modal-item').children('.form-control').attr('placeholder', wName);
            });
            if( $('.modal .femi-modal-body .femi-modal-item').length >= 6 ){
                $('.modal .femi-modal-body').addClass('femi-flex-body');
            }
    ...
    var dynamicPropertyGrid = {
        ...
        start: ( function(){
            ...
            $('body')
                .delegate('.propertyGrid  .femi-property-item .input-array-contain .input-array-item-c   input.input-array-item', 'change', function(){
                    var $this = $(this),
            ...
            $('body')
                .delegate('.propertyGrid  .femi-property-item select.femi-modal-dropOneControl', 'change', function(e){
                    var $this = $(this),
                        name = $this.attr('data-name'),
                        value = $this.val(),
                        $children = $this.closest('.propertyGrid')
                            .find('.femi-property-item select[data-control~=' + name + ']');

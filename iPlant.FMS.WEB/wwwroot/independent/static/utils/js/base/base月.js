define(["../jquery-3.1.1", "./bootstrap"], function ($yang, Bs) {
	...
	var tableTool = {
		...
		getExportParams: function ($table, fileName, Title) {
			...
			$table.find("tbody tr td input[type=checkbox].femi-tb-checkbox")
	...
	var dynamicModal = {
		...
		HTML: (function () {
			return {
				...
				ItemWeek: ['<div class="femi-modal-item">',
					'<label class="m-detail-title">{{title}}</label>',
					'<input class="form-control" data-type="week" type="week" data-name="{{name}}" value="{{value}}" placeholder="{{title}}" >',

					'</div>'
				].join(""),
				ItemMonth: ['<div class="femi-modal-item">',
					'<label class="m-detail-title">{{title}}</label>',
					'<input class="form-control" data-type="month" type="month" data-name="{{name}}" value="{{value}}" placeholder="{{title}}" >',
						...
						if (item.type) {
							...
							switch (item.type) {
								...
								case "Week":
									...
									break;
								case "Month":
									item.value = util.format("yyyy-MM", item.value);
									_data.push(util.template(item, dynamicModal.HTML.ItemMonth));
									break;
								case "Time":

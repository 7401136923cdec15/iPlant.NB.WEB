define(['./jquery-3.1.1', './base/base'], function ($jQ, $com) {
    ...
    var ResfushCanvasOne = function (obj,num) {
        ...
        DrawRoundRect(rectStart_X, rectStart_Y, rectWidth, 25, radius, obj.ColorList, obj.PartNo, obj.APSMessage, num);
    ...
    var CanvasBasic = function () {
        ...
        contexttop.fillText(position.orderinfo.OrderNo + " " + position.orderinfo.ProductNo + " " + position.orderinfo.LineName + " " + position.orderinfo.PartNo, (canvas.width + freedom) / 2, 15, canvas.width);
        contexttop.fill();
        contexttop.beginPath();
    ...
    var DrawRoundRect = function (x, y, width, height, radius, color, Name, str,num) {
        ...
        if (color.length == 0) {
            ...
            context.globalCompositeOperation = "source-over";
            context.arc(x + radius, y + 3 + radius, radius, Math.PI, Math.PI * 3 / 2);
            context.lineTo(width / 2 + x, y + 3);
            //context.arc(width - radius + x, radius + y + 3, radius, Math.PI * 3 / 2, Math.PI * 2);
            context.lineTo(width / 2 + x, height - 6 + y + 3);
            //context.arc(width - radius + x, height - 6 - radius + y + 3, radius, 0, Math.PI * 1 / 2);
            context.lineTo(radius + x, height - 6 + y + 3);
            context.arc(radius + x, height - 6 - radius + y + 3, radius, Math.PI * 1 / 2, Math.PI);
            context.fill();
            ...
            context.moveTo(width / 2 + x, y + 3);
            context.lineTo(width + x - radius, y + 3);
            context.arc(width - radius + x, radius + y + 3, radius, Math.PI * 3 / 2, Math.PI * 2);
            context.lineTo(width + x, height - radius - 6 + y - 3);
            context.arc(width + x - radius, height - 6 - radius + y + 3, radius, 0, Math.PI * 1 / 2);
            context.lineTo(width / 2 + x, height - 6 + y + 3);
            // context.arc(radius + x+width/2, height - 6 - radius + y + 3, radius, Math.PI * 1 / 2, Math.PI);
            context.fill();
        }
        else {
            console.log("请检查颜色数量！");
        ...
        //context.globalCompositeOperation = "source-over";
        //context.arc(x + radius, y + 3 + radius, radius, Math.PI, Math.PI * 3 / 2);
        //context.lineTo(width + x, y + 3);
        ////context.arc(width - radius + x, radius + y + 3, radius, Math.PI * 3 / 2, Math.PI * 2);
        //context.lineTo(width + x, height - 6 + y + 3);
        ////context.arc(width - radius + x, height - 6 - radius + y + 3, radius, 0, Math.PI * 1 / 2);
        //context.lineTo(radius + x, height - 6 + y + 3);
        //context.arc(radius + x, height - 6 - radius + y + 3, radius, Math.PI * 1 / 2, Math.PI);
        //context.fill();

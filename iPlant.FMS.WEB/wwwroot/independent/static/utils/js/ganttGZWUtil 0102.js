define(['./jquery-3.1.1', './base/base'], function ($jQ,$com) {
                        ...
                        $.each(jtem.TaskPartList, function (m, mtem) {
                            if (mtem.UniqueID == wDownRectResult.wIndex) {
                                ...
                                if (wDownRectResult.wIsLeft == 1 && new Date(new Date(mtem.startDate).getTime()
                                    + ((mtem.time + day) * 24 * 3600000)) > new Date(new Date(position.series.data[1]).getTime() + 8 * 3600000))
                                    return;
                                ...
                                for (var i = 0; i < position.Task.data.length; i++) {
                                    $.each(position.Task.data[i].TaskPartList, function (t, ttem) {
                                        $.each(jtem.TaskPartList, function (p, ptem) {
                                            ...
                                            if (ptem.PartNo == mtem.PartNo) {
                                                if (position.effect.Time == 1 && position.effect.Part == 0) {

                                                    if (pTime >= mTime) {
                                                        ptem.startDate = mtem.startDate;
                                                    }
    ...
    var ResfushCanvasOne = function (obj) {
        ...
        if (!(obj.startDate instanceof Date))
            ...
            rectStart_X = freedom + (((obj.startDate.getTime() - new Date(position.series.data[0]).getTime()-8*3600000) / (24 * 3600000)) * space);
        ...
        DrawRoundRect(rectStart_X, rectStart_Y, rectWidth, 25, radius, obj.color, obj.PartNo);
    ...
    var DrawMenu = function (task, y) {
        ...
        context.fillStyle = 'black';
        context.fillText(task, 10, y + 15);
        context.fill();
        context.closePath();
    }; 
    // //画左侧产线       1230
    // var DrawLIne = function (y, obj) {

    //     LineX = 50;

    //     context.beginPath();
    //     context.fillStyle = 'black';
    //     context.fillText(obj.LineID, 25, (TOPHEIGHT - LINEHEIGHT) / 2 + LINEHEIGHT);
    //     context.fill();
    //     context.closePath();
    ...
    //     context.beginPath();
    //     context.strokeStyle = '#E9EEF6';
    //     context.moveTo(LineX, 35);
    //     context.lineTo(LineX, TOPHEIGHT);
    //     context.moveTo(0, TOPHEIGHT);
    //     context.lineTo(freedom, TOPHEIGHT);
    ...
    var DrawPartPoint = function (TOPHEIGHT, jtem) {
        ...
        context.beginPath();
        context.fillStyle = 'black';
        context.fillText(jtem.LineName, 200,(TOPHEIGHT - PREHEIGHT) / 2 + PREHEIGHT);
    ...
    var DrawPart = function (TOPHEIGHT, jtem) {
        ...
        context.beginPath();
        context.fillStyle = 'black';
        context.fillText(jtem.PartName, 70, (TOPHEIGHT - PARTHEIGHT) / 2 + PARTHEIGHT);
        context.fill();
        context.closePath();
        ...
        context.beginPath();
        context.strokeStyle = '#E9EEF6';
        context.moveTo(150, 35);
        context.lineTo(150, TOPHEIGHT);
        context.moveTo(LineX, TOPHEIGHT);
        context.lineTo(freedom, TOPHEIGHT);
        context.stroke();
        context.closePath();
    ...
    //圆角矩形
    var flag = false;
    var DrawRoundRect = function (x, y, width, height, radius, color, Name) {
    ...
    var Tips = function (x, y, index) {
        ...
        var startDate = position.Task.data[index].PartNo,

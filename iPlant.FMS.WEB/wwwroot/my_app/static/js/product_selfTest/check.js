require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base'], function ($yang, $com) {
    ...
    HTML = {
        TR: ['<tr class="table-tr-m" bgcolor="#FFFFFF">',
              '<td width="30%" class="ex_type_choose">{{PartNo}}</td>',
              '<td width="30%" class="ex_dispatch_choose">{{num1}}</td>',
              '<td width="30%" class="ex_confirm_choose">{{num2}}</td>',
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            $("body").delegate(".ex_type_add", "click", function () {
                if (window.JSImpl) {
                    window.QRTEST = function (str) {
                        ...
                        model.com.getScan({ QRCode: str }, function (res) {
                            if (!res.info || !res.info.QRType || !res.info.ID) {
                                ...
                                model.com.getPartNo({ PartNo: str, PartType: 3 }, function (data) {
                                    partString = data.list.PartNo;
                                })
                            }
                        })
                    }
                    ...
                    $("#model_tbody").html($com.util.template(data, HTML.TR));
                } else {
                    var data = [{
                        PartNo: "ST-500001",
                        num1: 1,
            ...
            //表格中模组表的删除
            $("body").delegate(".ex_type_remove", "click", function () {
        ...
        com: {
            ...
            rederItemList: function (data) {
                ...
                model._module = data.module && module.length > 0 ? data.module : [];
                model._module.push(module);
            ...
            changeValueSource: function (source) {
                var sourceString = "",
					sourceArray = [];

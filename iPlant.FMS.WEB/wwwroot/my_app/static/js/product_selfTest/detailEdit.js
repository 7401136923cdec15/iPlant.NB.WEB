require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/base/entry'], function ($yang, $com, iForm) {
    ...
    HTML = {
        ...
        SUB_LIST: ['<li class="list-li">',
			...
			'<textarea rows="2"  class="actual-remark">{{Remark}}</textarea>',
			'</div></div>',
			'</div>', //填表单
			'</li>'].join(""),
        ITEM: {
            Text: [
				...
				'<div class="m-c-body m-c-input clearfix">',
				'<input class="actual-value" type="text" value="{{defaultValueText}}"></input>',
				'</div> </div>',
            ].join(""),
            Combo: [
				'<div class="m-c-panel" mode="n-select-radio">',
            ...
            Number: [
				...
				'<div class="m-c-head">实际值 {{unit}}</div>',
				'<div class="m-c-body m-c-input clearfix">',
				'<input class="actual-value" type="number" value="{{defaultValue}}" ',
    ...
    model = $com.Model.create({
        ...
        run: function () {
            ...
            if (!window.JSImpl) {
                model.com.get({
                    EventID: window._eventID ? window._eventID : 2014,
        ...
        com: {
            ...
            render: function (data) {
                ...
                data.ResuletValue = data.Result == true ? 1 : 2;
                $("#resultS").html(STATUS[data.ResuletValue]);
                $("#resultS").addClass(COLOUR[data.ResuletValue]);

	...
	model = $com.Model.create({
		...
		run : function() {
		    ...
		    this.com.creatReport({
		        ...
		        data.info.Remark = "cccc";
		        data.info.LableID = "w";
		        if (ModelAll.BGMode==1) {
		            window.location = "orderPartLZ.html?id=" + model.query.id + "&mFQTY=" + model.query.mFQTY;
		        }
		        else if (ModelAll.BGMode == 3) {
		            $("#zace-title").html("填写报工单");
		...
		com: {
			...
			addCheckSubmit: function (e, close) {
			    ...
			    model.com.add({
			        ...
			        alert("提交成功");
			        window.location = "reportPaDetailSSPro.html?id=" + data.info.ID;

require([ '../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base' ], function($yang, $com) {
	...
	HTML = {
	    LIST: ['<div class="ms-group clearfix" data-id="{{ID}}"  data-mode="{{BGMode}}"  data-shift="{{FQTYShift}}">',
			...
			'<span class="ms-label">周次:</span><span class="ms-text">{{ShiftID}}</span>',
			'</span>',
			'</div>',
			'<div class="ms-sub-title">',
			'<span class="ms-field">',
	...
	model = $com.Model.create({
		...
		events: function () {
		    ...
		    $("body").delegate(".ms-group", "click", function () {
				...
				window.location = "orderLine.html?id=" + id + "&mFQTY=" + mFQTYShift;
		    });
		...
		com : {
			...
			filter : function(data) {
				...
				$(data).each(function() {
					...
					//this.ShiftText = ShiftText[this.Status];

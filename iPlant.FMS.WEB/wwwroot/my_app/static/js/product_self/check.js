require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base'], function ($yang, $com) {
    ...
    model = $com.Model.create({
        ...
        events: function () {
            if ($com.util.boolean(model.query.sanC) == false) {
                wData = [];
                var data_mr = {
                    PartNo: model.query.part,
                    PackgeNo: model.query.pack,
                }
                wData.push(data_mr);
                $("#model_tbody").html($com.util.template(wData, HTML.TR));
            } else {
                ...
                if (window.JSImpl) {
                    ...
                    $("body").delegate(".ex_type_add", "click", function () {

                        window.QRTEST = function (str) {
                            if (!str || str.length < 1)
                                return false;
                            model.com.getPartNo({ PartNo: str, PartType: 3 }, function (res) {
                                //part = data.info.PartNo ? data.info.PartNo : "MZ-00001";
                                //packge = data.info.PackgeNo?data.info.PackgeNo:"DRB-00001";

                                if (res.info.PartNo == "") {
                                    part = "MZ-00001";
                                } else {
                                    part = res.info.PartNo;
                                }

                                if (res.info.PackgeNo == "") {
                                    packge = "DRB-00001";
                                } else {
                                    packge = res.info.PackgeNo;
                                }
                                var data = {
                                    PartNo: part,
                                    PackgeNo: packge,
                                ...
                                for (var i = 0; i < wData.length; i++) {
                                    if (wData[i].PartNo == data.PartNo) {
                                        alert("该模组编号已存在，请勿重复添加！");
                                        return false;
                                    }
                                }
                                wData.push(data);
                        ...
                        if (window.JSImpl)
                            window.JSImpl.readQRCode('QRTEST', "请扫模组二维码！");
                        else
                            window.QRTEST("J190723D0786");
                    });
                    ...
                    $("body").delegate(".ex_type_add", "click", function () {
                        ...
                        var data = [{
                            PartNo: "J190723D0786",
                            PackgeNo: "160V2019060759",
                        }, {
                            PartNo: "J190723D0784",
                            PackgeNo: "160V2019060795",

                        }, {
                            PartNo: "J190723D0782",
                            PackgeNo: "160V2019060756",
                        }]
                        wData = data;
                        var wPackage = [];
                        for (var i = 0; i < wData.length; i++) {
        ...
        com: {
            ...
            getPartNo: function (data, fn, context) {
                var d = {
            ...
            rederItemList: function (data) {
                ...
                model._module = data.module && data.module.length > 0 ? data.module : [];
                //给module添加数据

                model._list = data.list && data.list.length > 0 ? data.list : [];
                wlist = [];
            ...
            addCheck: function () {
                ...
                if ($com.util.boolean(model.query.sanC) == true) {
                    if (DataModule.length != 3) {
                        alert("请按照三串要求规范操作！");
                        return false;
                    }
                    model._module = DataModule;
                } else {
                    model._module = DataModule;
                }
                var data = {

require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base', '../static/utils/js/base/entry'], function ($yang, $com, iForm) {
    ...
    HTML = {
        TR: ['<tr class="table-tr-m" bgcolor="#FFFFFF">',
             ...
             '<td width="50%" >{{PackgeNo}}</td>',
             //'<td width="30%" >{{PCBNo}}</td>',
             //'<td width="10%" class="ex_type_remove"></td>',
         '</tr>'].join(""),
        TITLE: [
    ...
    model = $com.Model.create({
        ...
        com: {
            ...
            Module:function(Mdata){
                var wPackage = [];
                if (Mdata.length == 3) {
                    for (var i = 0; i < Mdata.length; i++) {
                        var wString = Number(Mdata[i].PackgeNo.substring(10, 14));
                ...
                $("#model_tbody").html($com.util.template(Mdata, HTML.TR));
            },

    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            $("body").delegate("#confirm", "click", function (e) {
                if ($("#confirm").text() == "重新绑定订单") {
                    window.location = "checkCXFQ.html?id=" + model.query.id + "&status=" + model.query.status;
                } else {
                    window.location = "checkSCFQ.html?id=" + model.query.id + "&status=" + model.query.status;
                }
            });
        },
        run: function () {
            ...
            if (model.query.status != "生产已确认" && model.query.status != "已验收驳回") {
                $("#confirm").hide();
            }
            if (model.query.status == "已验收驳回") {
                $("#confirm").text("重新绑定订单");
            }
            model.com.getRoute({
                ...
                model.com.getInfo({
                    ...
                    $("#w-option-content3").text(data.info.BindingProductOrderNo);
        ...
        com: {
            ...
            isShow: function (data) {
                ...
                if (data.BindingProductOrderID && data.BindingProductOrderID != 0) {
                    $("#scPlan").show();
                    $("#order").show();
                    $("#w-option-content3").text(data.BindingProductOrderName);
                }
            }
        }
    });

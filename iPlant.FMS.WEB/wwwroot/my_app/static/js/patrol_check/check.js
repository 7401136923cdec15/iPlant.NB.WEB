require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base'], function ($yang, $com) {
    ...
    HTML = {
        ...
        SUB_LIST: ['<li class="list-li">',
			...
			'<textarea rows="3" placeholder="请填写巡检项备注" class="actual-remark">{{remark}}</textarea>',
			'</div></div>',
			'</div>', //填表单
			'</li>'].join(""),
    ...
    model = $com.Model.create({
        ...
        com: {
            ...
            addCheck: function () {
                ...
                data.list = model.com.getItemValueList();

                if (!data.list && !confirm("未获取到表单数据,是否继续提交？")) {
                    return;
                }
                ...
                data.data.goodParts = goodParts;

                data.data.badParts = badParts;

require(['../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base'], function ($yang, $com) {
    ...
    HTML = {
        ...
        LIST2: ['<tr data-stockID="{{StockID}}"   data-qr="{{qr}}" data-stockLocID="{{StockLocID}}" >',
			...
			'<td>{{D2}}</td>',
			'<td>{{D4}}</td>',
			'<td><input type="number"  data-stockID="{{StockID}}"  data-stockLocID="{{StockLocID}}" max="{{D3}}" min="0"   data-min="0"  data-max="{{D3}}"  placeholder="" /></td>',
			'</tr>'].join(""),
        LIST3: ['<tr>',
			'<td>其他</td>',
			'<td></td>',
    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            //$("#back").click(function () {
            //     window.location = "info.html?id=" + model.query.task_id;
            ...
            $("body .view-order").delegate(".m-table-storage tr input", "input", function () {
               ...
               // model._data.FQTYMargin
                if (total > max) {
                    //input 的值
                    val = max - (total - $this.val());
                    $this.val(val);
                    total = max;
                }
                $node.val(total);
...
*/
            ...
            render: function (data) {
                ...
                for (var i = 0; i < data.list.length; i++) {
                    data.list[i].D5 = zItem.LocationList[i].FQTYPL;
                    sum += zItem.LocationList[i].FQTYPL;
                }                              
                $("#count").val(sum);
                ...
                $(".m-table-storage input").each(function (i, item) {
                    ...
                    $(this).val(zItem.LocationList[i].FQTYPL);
                });

require([ '../static/utils/js/jquery-3.1.1', '../static/utils/js/base/base' ], function($yang, $com) {
	...
	HTML = {
		LIST : [ '<div class="ms-group clearfix" data-id="{{ID}}" >',
			...
			'<div class="ms-title">',
			'<span>{{Name}}</span>',
			'<span class="ms-field femi-rt" >{{ShiftLoginTimeText}}</span>',
			'</div>',
		...
		com : {
			...
			filter : function(data) {
				...
				$.each(data, function(i, item) {
					...
					item.Status = STATUS[item.onShift];
					item.ShiftLoginTimeText = $com.util.format("yyyy-MM-dd hh:mm:ss", item.ShiftLoginTime);

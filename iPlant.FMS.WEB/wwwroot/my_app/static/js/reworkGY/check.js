    ...
    model = $com.Model.create({
        ...
        events: function () {
            ...
            $("body").delegate("#confirm", "click", function (e) {
                if ($("#confirm").text() == "重新发起") {
                    window.location = "checkCXFQ.html?id=" + model.query.id + "&status=" + model.query.status;
                } else {
                    window.location = "checkGYFQ.html?id=" + model.query.id + "&status=" + model.query.status;
                }
            });
        },
        run: function () {
            ...
            if (model.query.status != "质量已确认" && model.query.status != "工艺已驳回" && model.query.status != "生产已驳回") {
                $("#confirm").hide();
            }
            if (model.query.status == "工艺已驳回" || model.query.status == "生产已驳回") {
                $("#confirm").text("重新发起");
            }
            model.com.getInfo({
                ID: model.query.id
        ...
        com: {
            ...
            isShow: function (data) {
                if (data.Status == 6) {
                    ...
                    $("#desc1").val(data.CraftCause);
                }
                if (data.Status == 8) {
                    $("#reseaon").show();
                    $("#head").text("生产班组长备注");
                    $("#desc1").attr("readonly", "readonly");
                    $("#desc1").val(data.ProductionCause);
                }

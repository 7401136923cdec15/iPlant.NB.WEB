...
define([ '../jquery-3.1.1',
	...
	TYPELIST = {
		...
		Single : getNumber,
		Double : getNumber,
		Int32ArrayD1 : getNumber,
		StringArrayD1 : getString,
		DateTime : function(v) {
			return v;
		}
	};
				...
				$.each(data.deviceList,function(i,item){
					...
					if(!shops[item.workShopID]){
						...
						Shops_P.push({
							...
							LineDeviceList:[{LineID:item.lineID,LineName:item.lineName,DeviceList:[item]}]
						});
						...
						for(var key in Shops_P){
							if(Shops_P[key].AreaID!=item.workShopID)
								continue;
							Shops_P[key].LineDeviceList.push({LineID:item.lineID,LineName:item.lineName,DeviceList:[item]});
						} 
					}else{

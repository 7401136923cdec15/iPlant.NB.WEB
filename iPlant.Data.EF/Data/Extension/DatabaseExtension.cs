...
namespace iPlant.Data.EF.Extension
{
    ...
    {
        private static log4net.ILog logger = log4net.LogManager.GetLogger(typeof(DatabasesExtension));
        /// <summary>
        ...
        /// <returns></returns>
        public static Dictionary<string, object> GetPropertyInfo<T>(T entity)
        {
        ...
        public static IQueryable<T> AppendSort<T>(IQueryable<T> tempData, string sort, bool isAsc)
        {
            ...
            for (int index = 0; index < sortArr.Length; index++)
            {
                ...
                var property = typeof(T).GetProperties().Where(p => p.Name.ToLower() == sortField.ToLower()).FirstOrDefault();
        ...
        public static string GetCommandText(this DbCommand dbCommand)
        {
            ...
            foreach (DbParameter parameter in dbCommand.Parameters)
            {
                try
                {
                    ...
                    {
                        case DbType.Date:
                            value = StringUtils.parseDate(parameter.Value).ToString("yyyy-MM-dd HH:mm:ss");
                            break;
                        default:

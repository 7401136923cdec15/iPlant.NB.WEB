    ...
    public partial class ClientBase : IDisposable
    {
        ...
        protected static void ValidateResponse(ResponseHeader header)
        {
            ...
            if (StatusCode.IsBad(header.ServiceResult))
            {
                ...
                Exception e = new ServiceResultException(new ServiceResult(header.ServiceResult, header.ServiceDiagnostics, header.StringTable));


                Utils.Trace(e, "ClientBase ValidateResponse");
                Console.WriteLine("ClientBase ValidateResponse {0}", e.ToString());
                throw e;
            }
        }
        #endregion
